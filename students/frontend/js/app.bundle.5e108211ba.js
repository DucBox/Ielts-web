...
`),n=[];let i=null;const s=()=>{i&&(n.push(`</${i}>`),i=null)};for(const a of e){const o=a.trim();if(!o){s();continue}const l=o.match(/^[-*]\s+(.+)$/),d=o.match(/^\d+\.\s+(.+)$/);if(l||d){const p=l?"ul":"ol";i!==p&&(s(),n.push(`<${p}>`),i=p),n.push(`<li>${renderMarkdownInline((l||d)[1])}</li>`);continue}s();const c=o.match(/^(#{2,4})\s+(.+)$/);c?n.push(`<h5>${renderMarkdownInline(c[2])}</h5>`):n.push(`<p>${renderMarkdownInline(o)}</p>`)}return s(),n.join("")}function btnReset(t){t&&(t.disabled=!1,t.innerHTML=t._origHTML||t.innerHTML)}function toast(t,e="success"){const n=e==="error"?6e3:3500,i=document.createElement("div");i.className=`toast toast-${e}`,i.setAttribute("role","alert");const s=document.createElement("span");s.textContent=t;const a=document.createElement("button");a.className="toast-close",a.setAttribute("aria-label","\u0110\xF3ng th\xF4ng b\xE1o"),a.textContent="\xD7",a.onclick=()=>i.remove(),i.appendChild(s),i.appendChild(a),i.addEventListener("mouseenter",()=>i.classList.add("toast-paused")),i.addEventListener("mouseleave",()=>i.classList.remove("toast-paused")),$("#toast-container").appendChild(i);const o=setTimeout(()=>i.remove(),n);i.addEventListener("mouseenter",()=>clearTimeout(o)),i.addEventListener("mouseleave",()=>setTimeout(()=>i.remove(),1e3))}function setLoading(t="\u0110ang t\u1EA3i..."){$("#app").innerHTML=`
  ...
  `);const t=$("#login-username")?.value?.trim();if(t){const e=$("#forgot-password-username");e&&(e.value=t)}setTimeout(()=>$("#forgot-password-username")?.focus(),50)}async function submitForgotPassword(t){const e=$("#forgot-password-username")?.value?.trim()||$("#login-username")?.value?.trim()||"";if(!e){toast("Vui l\xF2ng nh\u1EADp username \u0111\u1EC3 g\u1EEDi link \u0111\u1EB7t l\u1EA1i m\u1EADt kh\u1EA9u.","error");return}btnLoading(t);try{const n=await api.post("/auth/forgot-password",{username:e});closeModal(),toast(n.message||"Link \u0111\u1EB7t l\u1EA1i m\u1EADt kh\u1EA9u \u0111\xE3 \u0111\u01B0\u1EE3c g\u1EEDi qua Gmail.","success")}catch(n){btnReset(t),toast(n.error||"Kh\xF4ng th\u1EC3 g\u1EEDi link \u0111\u1EB7t l\u1EA1i l\xFAc n\xE0y.","error")}}function showResetPasswordPage(t){$("#app").classList.remove("with-header"),$("#app-header").classList.add("hidden"),$("#app").innerHTML=`
    ...
    </div>`}function buildProgressChartSvg(t,e,n,i,s="profile"){const a=s==="profile"?520:320,o=s==="profile"?220:150,l=s==="profile"?42:30,d=14,c=14,p=s==="profile"?28:24,r=a-l-d,m=o-c-p,u="#dbe4ef",v="#94a3b8",h={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"}[i]||"#0f766e",g=new Date;g.setHours(23,59,59,999);const w=new Date(g);w.setDate(g.getDate()-(n-1)),w.setHours(0,0,0,0);const x=Math.max(1,g.getTime()-w.getTime()),S=y=>l+(y.getTime()-w.getTime())/x*r,b=y=>c+(9-y)/9*m,C=[0,3,6,9].map(y=>{const k=b(y).toFixed(1);return`
      <line x1="${l}" y1="${k}" x2="${a-d}" y2="${k}" stroke="${u}" stroke-width="1"/>
      <text x="${l-8}" y="${Number(k)+4}" text-anchor="end" font-size="10" fill="${v}">${y}</text>`}).join(""),L=Number.isFinite(e)?`
  ...
  `:"",M=t.map(y=>`${S(y.date).toFixed(1)},${b(y.avgScore).toFixed(1)}`).join(" "),A=t.length>=2?`<polyline fill="none" stroke="${h}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" points="${M}"/>`:"",H=t.map(y=>{const k=S(y.date).toFixed(1),E=b(y.avgScore).toFixed(1);return`
    ...
    <svg viewBox="0 0 ${a} ${o}" preserveAspectRatio="none" class="progress-chart-svg" aria-hidden="true">
      ...
      ${A}
      ${H}
      <text x="${l}" y="${o-6}" font-size="10" fill="${v}">${B}</text>
      <text x="${a-d}" y="${o-6}" text-anchor="end" font-size="10" fill="${v}">${f}</text>
    ...
    ${n.map(s=>`
      <a href="#/${s.submission_id?"result":"assignment"}/${s.id}" class="cal-event-item">
        ...
        <span class="cal-event-title">${escapeHtml(s.title)}</span>
        <span class="cal-event-status">${s.submission_id?"\u2705 \u0110\xE3 n\u1ED9p":"\u23F3 H\u1EA1n"}</span>
      </a>`).join("")}`}},renderProfile=function(t,e){e=e||window._cachedProfileData||{fields:[],answers:{}};const n=(e.fields||[]).find(b=>b.field_key==="notification_email"),i=(e.fields||[]).filter(b=>b.field_key!=="notification_email"),s=_student&&typeof _student.email=="string"&&_student.email.trim()||e.student&&typeof e.student.email=="string"&&e.student.email.trim()||(n?String(e.answers?.[n.id]||"").trim():""),a=calculateSubmissionStreak(t,window._cachedVocabSessions||[]),o=getTargetSettings(),l=getSubmittedAssignments(t),d=calculateOverallAverage(t),c=d!==null?d.toFixed(1):"\u2014",p=SKILL_ORDER.map(b=>{const _=getSkillGradedAssignments(t,b).map(C=>Number(C.overall_score)).filter(Number.isFinite);return{sk:b,best:_.length?Math.max(..._):null}}),r=new Set(l.map(b=>toDateKey(b.submitted_at||b.created_at)).filter(Boolean)),m=new Set(a.days),u=[];for(let b=6;b>=0;b--){const _=new Date;_.setDate(_.getDate()-b);const C=toDateKey(_),L=_.toLocaleDateString("vi-VN",{weekday:"short"});u.push(`<div class="home-streak-day ${m.has(C)?"on":""}" title="${C}">
      <div class="streak-dot">${m.has(C)?"\u{1F525}":"\xB7"}</div>
      <div class="streak-day-label">${L}</div>
    </div>`)}const v=(_myVocabCache||[]).length,h=(_student.full_name||"?").split(" ").map(b=>b[0]).join("").slice(0,2).toUpperCase(),g=a.current>=30?"#dc2626":a.current>=7?"#f59e0b":"#6b7280",w={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"},x=[];p.forEach(({sk:b,best:_})=>{_!==null&&x.push(`<div class="ms-card"><div class="ms-icon">${SKILL_ICONS[b]}</div><div class="ms-val">${_}</div><div class="ms-label">Best ${SKILL_LABELS[b]}</div></div>`)}),l.length>0&&x.push(`<div class="ms-card"><div class="ms-icon">\u{1F4DD}</div><div class="ms-val">${l.length}</div><div class="ms-label">B\xE0i \u0111\xE3 n\u1ED9p</div></div>`),a.best>1&&x.push(`<div class="ms-card"><div class="ms-icon">\u{1F525}</div><div class="ms-val">${a.best}</div><div class="ms-label">Streak k\u1EF7 l\u1EE5c</div></div>`),v>0&&x.push(`<div class="ms-card"><div class="ms-icon">\u{1F4D6}</div><div class="ms-val">${v}</div><div class="ms-label">T\u1EEB \u0111\xE3 l\u01B0u</div></div>`);const S=[["profile-info","\u{1F4CB} Th\xF4ng tin"],["profile-streak","\u{1F525} Streak"],["profile-targets","\u{1F3AF} Target"],["profile-progress","\u{1F4C8} Ti\u1EBFn \u0111\u1ED9"],...x.length>0?[["profile-achievements","\u{1F3C6} Th\xE0nh t\xEDch"]]:[],["profile-next-steps","\u{1F680} Ti\u1EBFp t\u1EE5c h\u1ECDc"]];$("#app").innerHTML=`
    <div class="container profile-page">
      <div class="profile-hero">
        <div class="profile-avatar" style="background:${w.reading}">${h}</div>
        <div class="profile-hero-info">
      ...
      <div class="profile-section-title">\u{1F680} Ti\u1EBFp t\u1EE5c h\u1ECDc</div>
      <div class="profile-quick-row">
        <a href="#/my-vocab" class="profile-quick-btn pqb-vocab">\u{1F4D6} T\u1EEB v\u1EF1ng c\u1EE7a t\xF4i <span class="pqb-badge">${v}</span></a>
        <a href="#/vocab-games" class="profile-quick-btn">\u{1F0CF} Luy\u1EC7n t\u1EEB</a>
        <a href="#/history" class="profile-quick-btn">\u{1F4CA} L\u1ECBch s\u1EED</a>
        <a href="#/assignments" class="profile-quick-btn">\u{1F4CB} B\xE0i t\u1EADp</a>
      </div>
    ...
    <div class="container mfc-page">
      <div class="mfc-end">
    ...
    </div>`}window.flipMyFC=flipMyFC,window.mfcAnswer=mfcAnswer;async function showAssignments(){setLoading("\u0110ang t\u1EA3i danh s\xE1ch b\xE0i t\u1EADp...");try{const t=await getAssignments();renderAssignments(t)}catch(t){toast("L\u1ED7i t\u1EA3i b\xE0i t\u1EADp: "+(t.error||t.message),"error"),renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c danh s\xE1ch b\xE0i t\u1EADp",t,"/assignments")}}function renderAssignments(t){const e=_assignmentSkillFilter?t.filter(c=>c.skill===_assignmentSkillFilter):t,n=e.filter(c=>c.skill!=="composite"&&c.rewrite_status==="requested"),i=e.filter(c=>c.skill!=="composite"?!c.submission_id&&c.is_active:c.is_active&&!isCompositeAssignmentDone(c)),s=e.filter(c=>c.skill!=="composite"?!c.submission_id&&!c.is_active:!c.is_active&&!isCompositeAssignmentDone(c)),a=e.filter(c=>c.skill!=="composite"?!!c.submission_id&&c.rewrite_status!=="requested":isCompositeAssignmentDone(c));function o(c){const p=c.skill==="composite",r=isOverdue(c.deadline),m=p?isCompositeAssignmentDone(c):!!c.submission_id,u=!c.is_active&&!m,v=SKILL_ICONS[c.skill]||"\u{1F4DD}",h=!m&&!r&&!u?formatCountdown(c.deadline):null;let g,w;if(p){const C=getCompositeSectionsForAssignment(c),L=C.filter(A=>A.submitted).length,M=getCompositeAssignmentAverageScore(c);u?g='<span class="badge badge-closed">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>':m?g=isCompositeAssignmentFullyGraded(c)?'<span class="badge badge-done">\u2705 \u0110\xE3 c\xF3 \u0111i\u1EC3m</span>':'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>':L>0?g=`<span class="badge badge-pending">${L}/${C.length} ph\u1EA7n</span>`:g='<span class="badge badge-pending">Ch\u01B0a l\xE0m</span>',w=isCompositeAssignmentFullyGraded(c)&&M!==null?`<div class="score-band">${M.toFixed(1)}</div><div class="score-label">Band</div>`:""}else if(u)g='<span class="badge badge-closed">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>',w="";else if(m){const C=c.overall_score!==null&&c.overall_score!==void 0;c.rewrite_status==="requested"?(g='<span class="badge badge-rewrite">\u270F\uFE0F Y\xCAU C\u1EA6U VI\u1EBET L\u1EA0I</span>',w=C?`<div class="score-band rewrite-score">${c.overall_score}</div><div class="score-label">Band</div>`:'<div class="score-pending-icon">\u270F\uFE0F</div>'):(g=C?'<span class="badge badge-done">\u2705 \u0110\xE3 c\xF3 \u0111i\u1EC3m</span>':'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>',w=C?`<div class="score-band">${c.overall_score}</div><div class="score-label">Band</div>`:'<div class="score-pending-icon">\u23F3</div>')}else r&&c.deadline?(g='<span class="badge badge-overdue">\u26A0\uFE0F Qu\xE1 h\u1EA1n</span>',w=""):(g='<span class="badge badge-pending">Ch\u01B0a l\xE0m</span>',w="");if(u)return`
        <div class="assignment-card assignment-card-closed">
          <div class="assignment-card-icon">${v}</div>
        ...
        </div>`;const x=!p&&c.rewrite_status==="requested",S=p?c.is_active&&!isCompositeAssignmentDone(c):!c.submission_id&&c.is_active,b=p?`#/composite/${c.id}`:m?`#/result/${c.id}`:`#/assignment/${c.id}`;return`
      <a class="assignment-card ${x?"rewrite":S?"pending-card":m?"done":""}" href="${b}">
        <div class="assignment-card-icon">${v}</div>
        <div class="assignment-card-body">
          <div class="assignment-card-title">${escapeHtml(c.title)}</div>
          <div class="assignment-card-meta">
    ...
    <div class="audio-submit-processing">
      ...
      <tr>
        ...
        ${g}
      </tr>${w}`}).join(""),l=e.length,d=(t.scoring_scale||"10")==="ielts",c=t.overall_score??(l>0?d?Math.round(i/l*9*10)/10:Math.round(i/l*10*10)/10:0),p=s?5:4,r=t.vocabulary||[],m=r.length===0?"":`
    <div class="section-label" style="margin-top:20px">\u{1F4DA} T\u1EEB v\u1EF1ng trong b\xE0i</div>
    <div class="vocab-result-list">
      ${r.map((u,v)=>{const h=isWordSaved(u.word);return`
        <div class="vocab-result-item">
          <div class="vocab-result-header" onclick="toggleVocabItem(${v})">
            <span class="vocab-result-word">${escapeHtml(u.word)}</span>
            <span class="vocab-result-toggle" id="vocab-toggle-${v}">\u25B6</span>
          ...
          <div class="vocab-result-detail hidden" id="vocab-detail-${v}">
    ...
    <div class="feedback-overall">${escapeHtml(s)}</div>`:"",m=_annColorMap(i),u=_buildAnnSidebar(i,m),v=p?`
    ...
    </div>`;const a=3;let o="";for(const l of i){const d=s[l];if(!d||d.length===0)continue;const c=!!_vocabExpanded[l],p=c?d:d.slice(0,a),r=d.length-a,m=p.map(v=>`
      <a class="vocab-game-card" href="#/vocab-game/${v.id}">
        <div class="vocab-game-card-icon">${SKILL_ICONS[v.skill]}</div>
        <div class="vocab-game-card-body">
          <div class="vocab-game-card-title">${escapeHtml(v.title)}</div>
          <div class="vocab-game-card-meta">
            ${skillBadge(v.skill)}
            <span class="vocab-count-badge">${v.vocab_count} t\u1EEB</span>
          </div>
        </div>
  ...
  `).join(""),a=i.length===0?`<div class="empty-hint">Kh\xF4ng c\xF3 \u0111\u1EC1 n\xE0o${_sharedSkillFilter!=="all"?" cho k\u1EF9 n\u0103ng n\xE0y":""}.</div>`:i.map(o=>{const l=(o.tags||[]).map(p=>`<span class="tag-chip">${escapeHtml(p)}</span>`).join(""),d=o.real_test_count>0&&o.best_score!=null,c=o.real_test_count>0?`<div class="shared-card-stats">
    ...
    <div class="assignment-page">
      ...
      <tr>
        ...
        ${w}
      </tr>${x}`}).join(""),l=e.length,d=l===40,c=t.overall_score??(l>0?d?Math.round(s/l*9*10)/10:Math.round(s/l*10*10)/10:0),p=l-s,r=t.vocabulary||[],m=r.length===0?"":`
    <div class="section-label" style="margin-top:20px">\u{1F4DA} T\u1EEB v\u1EF1ng trong b\xE0i</div>
    <div class="vocab-result-list">
      ${r.map((u,v)=>{const h=isWordSaved(u.word);return`
        <div class="vocab-result-item">
  ...
  `;const i=document.getElementById("profile-name-input");i?.focus(),i?.select(),i?.addEventListener("keydown",s=>{s.key==="Enter"&&saveProfileName(),s.key==="Escape"&&renderProfile(window._cachedAssignments||[],window._cachedProfileData)})}async function saveProfileName(){const t=document.getElementById("profile-name-input");if(!t)return;const e=t.value.trim();if(!e){t.focus();return}t.disabled=!0;let n=!1;try{await api.patch("/student/me",{full_name:e}),n=!0}catch(i){t.disabled=!1,t.focus(),alert(i?.error||"Kh\xF4ng th\u1EC3 c\u1EADp nh\u1EADt t\xEAn")}if(n){updateStudentState({full_name:e});const i=document.getElementById("header-student-name");i&&(i.textContent=e),renderProfile(window._cachedAssignments||[],window._cachedProfileData)}}window.startEditProfileName=startEditProfileName,window.saveProfileName=saveProfileName;let _compositeExam=null,_activeSectionId=null,_sectionTimerInterval=null,_sectionTimerSecsLeft=0;function stopCompositeSectionTimer(){_sectionTimerInterval&&(clearInterval(_sectionTimerInterval),_sectionTimerInterval=null)}function _tickCompositeSectionTimer(){_sectionTimerSecsLeft=Math.max(0,_sectionTimerSecsLeft-1);const t=document.getElementById("csec-countdown");if(t){const e=Math.floor(_sectionTimerSecsLeft/60),n=_sectionTimerSecsLeft%60;t.textContent=`${String(e).padStart(2,"0")}:${String(n).padStart(2,"0")}`,t.className=`assign-countdown${_sectionTimerSecsLeft<60?" timer-urgent":""}`}_sectionTimerSecsLeft<=0&&(stopCompositeSectionTimer(),autoSubmitCurrentCompositeSection())}function startCompositeSectionTimer(t){stopCompositeSectionTimer(),_sectionTimerSecsLeft=Math.floor(t),_sectionTimerInterval=setInterval(_tickCompositeSectionTimer,1e3)}async function showCompositeExam({id:t}){setLoading("\u0110ang t\u1EA3i \u0111\u1EC1 t\u1ED5ng h\u1EE3p...");try{const e=await api.get(`/student/assignments/${t}`);e.sections&&(e.sections=e.sections.map(n=>({...n,question_count:Array.isArray(n.questions_data)?n.questions_data.length:0}))),_compositeExam=e,_activeSectionId=null,stopCompositeSectionTimer(),renderCompositeExam(e)}catch(e){toast("L\u1ED7i t\u1EA3i \u0111\u1EC1: "+(e.error||e.message),"error"),navigate("/assignments")}}window.showCompositeExam=showCompositeExam;function renderCompositeExam(t){stopAssignmentCountdown();const e=t.sections||[],n=e.length>0&&e.every(o=>o.submission_id),i=!t.is_active,s=e.map(o=>{const l=!!o.submission_id,d=SKILL_ICONS[o.skill]||"\u{1F4DD}",c=o.question_count>0?`${o.question_count} c\xE2u`:"",p=o.time_limit_minutes?`\u23F1 ${o.time_limit_minutes} ph\xFAt`:"",r=[c,p].filter(Boolean).join(" \xB7 "),m=l?`<span class="badge badge-done" style="font-size:11px">\u2713 \u0110\xE3 n\u1ED9p${n&&o.score!=null?` \xB7 Band ${o.score}`:""}</span>`:i?'<span class="badge badge-closed" style="font-size:11px">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>':'<span class="badge badge-pending" style="font-size:11px">Ch\u01B0a l\xE0m</span>',u=l?n:!i,v=l?n?`#/composite-result/${t.id}/section/${o.submission_id}`:"":i?"":`#/composite/${t.id}/section/${o.id}`,h=l&&!n?'<div style="font-size:12px;color:var(--gray-400);margin-top:6px">\u{1F512} Ho\xE0n th\xE0nh c\xE1c ph\u1EA7n c\xF2n l\u1EA1i \u0111\u1EC3 xem k\u1EBFt qu\u1EA3</div>':"",g=l&&!n?"\u{1F512}":"\u203A";return`
      ...
      </${u&&v?"a":"div"}>`}).join(""),a=n?`<div style="margin-top:24px;text-align:center">
    ...
    <div class="assignment-toolbar" style="position:sticky;top:0;z-index:10;border-bottom:1px solid var(--border);border-top:2px solid var(--border)">
      ...
      </div>`;const v=c(r),h=v?`<div class="result-score-badge">${v}</div>`:'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>';let g="";return r.skill==="writing"&&r.feedback?g=r.feedback:r.skill==="speaking"?g=r.feedback||(r.audio_url?"\u0110\xE3 n\u1ED9p file ghi \xE2m. B\u1EA5m \u0111\u1EC3 xem transcript v\xE0 nh\u1EADn x\xE9t chi ti\u1EBFt.":""):g=r.score!=null?"M\u1EDF \u0111\u1EC3 xem \u0111\xE1p \xE1n \u0111\xFAng/sai, Explain, Locate v\xE0 luy\u1EC7n l\u1EA1i ph\u1EA7n n\xE0y.":"Ph\u1EA7n n\xE0y \u0111\xE3 n\u1ED9p v\xE0 \u0111ang ch\u1EDD ch\u1EA5m \u0111i\u1EC3m.",`<a href="#/composite-result/${t.id}/section/${r.submission_id}" class="result-section-card composite-result-card composite-result-card-link">
      <div class="composite-result-card-top">
        <div class="composite-result-card-title-wrap">
          <div class="composite-result-card-icon">${e[r.skill]||"\u{1F4DD}"}</div>
          <div>
