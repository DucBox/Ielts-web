  ...
  `:"",C=t.map(S=>`${k(S.date).toFixed(1)},${b(S.avgScore).toFixed(1)}`).join(" "),L=t.length>=2?`<polyline fill="none" stroke="${v}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" points="${C}"/>`:"",E=t.map(S=>{const T=k(S.date).toFixed(1),B=b(S.avgScore).toFixed(1);return`
    ...
    <div class="container profile-page">
        ...
        ${i.length===0?"":`<details class="pi-details">
              ...
              <div class="pi-details-body">
                <div class="pi-fields" id="pi-fields">
                  ${i.map(b=>{const w=e.answers[b.id]||"",x=`pi-field-${b.id}`;let _;if(b.field_type==="textarea")_=`<textarea id="${x}" class="form-input pi-input" data-field-id="${b.id}" rows="3" placeholder="Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u">${escapeHtml(w)}</textarea>`;else if(b.field_type==="select"&&Array.isArray(b.options)){const C=b.options.map(L=>`<option value="${escapeHtml(String(L))}" ${w===String(L)?"selected":""}>${escapeHtml(String(L))}</option>`).join("");_=`<select id="${x}" class="form-input pi-input" data-field-id="${b.id}"><option value="">-- Ch\u01B0a ch\u1ECDn --</option>${C}</select>`}else if(b.field_type==="date")_=`<input id="${x}" class="form-input pi-input" type="date" data-field-id="${b.id}" value="${escapeHtml(w)}" />`;else{const C=b.field_key==="notification_email"?"email":"text",L=b.field_key==="notification_email"?"name@example.com":"Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u";_=`<input id="${x}" class="form-input pi-input" type="${C}" data-field-id="${b.id}" value="${escapeHtml(w)}" placeholder="${L}" />`}return`<div class="pi-field">
                      <label class="pi-label" for="${x}">${escapeHtml(b.label)}</label>
                      ${_}
                    </div>`}).join("")}
                </div>
      ...
      b\u1EA1n b\u1EA5m "B\u1EAFt \u0111\u1EA7u l\xE0m b\xE0i", v\xE0 <b>v\u1EABn ch\u1EA1y ti\u1EBFp k\u1EC3 c\u1EA3 khi b\u1EA1n tho\xE1t ra</b>.
        ...
        <div class="attempt-history-row${a?" is-latest":""}">
      ...
      <div class="attempt-history">
        <div class="attempt-history-note">\u0110i\u1EC3m ch\xEDnh th\u1EE9c lu\xF4n l\xE0 <b>l\u1EA7n m\u1EDBi nh\u1EA5t</b>.</div>
        ${i||'<div class="attempt-history-note">Ch\u01B0a c\xF3 l\u1EA7n n\u1ED9p n\xE0o.</div>'}
      </div>`}catch(e){$("#modal-body").innerHTML=`<p style="color:var(--danger)">${escapeHtml(e.error||e.message||"L\u1ED7i t\u1EA3i l\u1ECBch s\u1EED")}</p>`}}}window.openAttemptHistory=openAttemptHistory;async function showMyVocab(){_myVocabSearch="",_myVocabSort="",setLoading("\u0110ang t\u1EA3i t\u1EEB v\u1EF1ng...");const t=routeToken();await loadMyVocab(),!routeChanged(t)&&renderMyVocabList()}function renderMyVocabList(){const t=_myVocabCache||[],e=_myVocabSearch.toLowerCase();let n=e?t.filter(s=>s.word.toLowerCase().includes(e)||s.definition.toLowerCase().includes(e)):t;_myVocabSort==="az"?n=[...n].sort((s,o)=>s.word.toLowerCase().localeCompare(o.word.toLowerCase())):_myVocabSort==="za"?n=[...n].sort((s,o)=>o.word.toLowerCase().localeCompare(s.word.toLowerCase())):_myVocabSort==="newest"&&(n=[...n].sort((s,o)=>(o.savedAt||"")>(s.savedAt||"")?1:-1));const i=n.length===0?`<div class="empty-state-v2">
    ...
    <div class="container mfc-page">
      <div class="mfc-end">
    ...
    </div>`}window.flipMyFC=flipMyFC,window.mfcAnswer=mfcAnswer;async function showAssignments(){setLoading("\u0110ang t\u1EA3i danh s\xE1ch b\xE0i t\u1EADp...");const t=routeToken();try{const e=await getAssignments();if(routeChanged(t))return;renderAssignments(e)}catch(e){if(routeChanged(t))return;toast("L\u1ED7i t\u1EA3i b\xE0i t\u1EADp: "+(e.error||e.message),"error"),renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c danh s\xE1ch b\xE0i t\u1EADp",e,"/assignments")}}function renderAssignments(t){const e=_assignmentSkillFilter?t.filter(c=>c.skill===_assignmentSkillFilter):t,n=e.filter(c=>c.skill!=="composite"&&c.rewrite_status==="requested"),i=e.filter(c=>c.skill!=="composite"?!c.submission_id&&c.is_active:c.is_active&&!isCompositeAssignmentDone(c)),s=e.filter(c=>c.skill!=="composite"?!c.submission_id&&!c.is_active:!c.is_active&&!isCompositeAssignmentDone(c)),o=e.filter(c=>c.skill!=="composite"?!!c.submission_id&&c.rewrite_status!=="requested":isCompositeAssignmentDone(c));function a(c){const p=c.skill==="composite",d=isOverdue(c.deadline),m=p?isCompositeAssignmentDone(c):!!c.submission_id,u=!c.is_active&&!m,h=SKILL_ICONS[c.skill]||"\u{1F4DD}",v=!m&&!d&&!u?formatCountdown(c.deadline):null;let g,f;if(p){const _=getCompositeSectionsForAssignment(c),C=_.filter(E=>E.submitted).length,L=getCompositeAssignmentAverageScore(c);u?g='<span class="badge badge-closed">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>':m?g=isCompositeAssignmentFullyGraded(c)?'<span class="badge badge-done">\u2705 \u0110\xE3 c\xF3 \u0111i\u1EC3m</span>':'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>':C>0?g=`<span class="badge badge-pending">${C}/${_.length} ph\u1EA7n</span>`:g='<span class="badge badge-pending">Ch\u01B0a l\xE0m</span>',f=isCompositeAssignmentFullyGraded(c)&&L!==null?`<div class="score-band">${L.toFixed(2)}</div><div class="score-label">Band</div>`:""}else if(u)g='<span class="badge badge-closed">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>',f="";else if(m){const _=c.overall_score!==null&&c.overall_score!==void 0,C=c.rewrite_status==="requested";c.retake_attempt_number!=null?(g='<span class="badge badge-retaking">\u{1F3AF} \u0110ang l\xE0m l\u1EA1i</span>',f=_?`<div class="score-band retaking-score">${c.overall_score}</div><div class="score-label">Band c\u0169</div>`:'<div class="score-pending-icon">\u{1F3AF}</div>'):C?(g='<span class="badge badge-rewrite">\u270F\uFE0F Y\xCAU C\u1EA6U L\xC0M L\u1EA0I</span>',f=_?`<div class="score-band rewrite-score">${c.overall_score}</div><div class="score-label">Band</div>`:'<div class="score-pending-icon">\u270F\uFE0F</div>'):(g=_?'<span class="badge badge-done">\u2705 \u0110\xE3 c\xF3 \u0111i\u1EC3m</span>':'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>',f=_?`<div class="score-band">${c.overall_score}</div><div class="score-label">Band</div>`:'<div class="score-pending-icon">\u23F3</div>')}else d&&c.deadline?(g='<span class="badge badge-overdue">\u26A0\uFE0F Qu\xE1 h\u1EA1n</span>',f=""):(g='<span class="badge badge-pending">Ch\u01B0a l\xE0m</span>',f="");if(u)return`
        ...
        </div>`;const y=!p&&c.rewrite_status==="requested",k=!p&&c.retake_attempt_number!=null,b=p?c.is_active&&!isCompositeAssignmentDone(c):!c.submission_id&&c.is_active,w=p?`#/composite/${c.id}`:m&&!k?`#/result/${c.id}`:`#/assignment/${c.id}`;return`
      <a class="assignment-card ${y?"rewrite":k?"retaking":b?"pending-card":m?"done":""}" href="${w}">
        <div class="assignment-card-icon">${h}</div>
        <div class="assignment-card-body">
          <div class="assignment-card-title">${escapeHtml(c.title)}</div>
          <div class="assignment-card-meta">
    ...
    <div class="assignment-page">
      <div class="assignment-toolbar">
        ...
        <div class="toolbar-actions">
          ...
          ${(t.attempt_number||1)>1&&!t.is_composite_section?`<button class="btn-practice btn-practice-history" onclick="openAttemptHistory('${t.assignment_id}')">\u{1F4CB} L\u1ECBch s\u1EED c\xE1c l\u1EA7n</button>`:""}
        </div>
      </div>
      <div class="assignment-content">
