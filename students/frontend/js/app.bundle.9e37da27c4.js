...
`),n=[];let i=null;const s=()=>{i&&(n.push(`</${i}>`),i=null)};for(const a of e){const o=a.trim();if(!o){s();continue}const l=o.match(/^[-*]\s+(.+)$/),d=o.match(/^\d+\.\s+(.+)$/);if(l||d){const p=l?"ul":"ol";i!==p&&(s(),n.push(`<${p}>`),i=p),n.push(`<li>${renderMarkdownInline((l||d)[1])}</li>`);continue}s();const c=o.match(/^(#{2,4})\s+(.+)$/);c?n.push(`<h5>${renderMarkdownInline(c[2])}</h5>`):n.push(`<p>${renderMarkdownInline(o)}</p>`)}return s(),n.join("")}function btnReset(t){t&&(t.disabled=!1,t.innerHTML=t._origHTML||t.innerHTML)}function toast(t,e="success"){const n=e==="error"?6e3:3500,i=document.createElement("div");i.className=`toast toast-${e}`,i.setAttribute("role","alert");const s=document.createElement("span");s.textContent=t;const a=document.createElement("button");a.className="toast-close",a.setAttribute("aria-label","\u0110\xF3ng th\xF4ng b\xE1o"),a.textContent="\xD7",a.onclick=()=>i.remove(),i.appendChild(s),i.appendChild(a),i.addEventListener("mouseenter",()=>i.classList.add("toast-paused")),i.addEventListener("mouseleave",()=>i.classList.remove("toast-paused")),$("#toast-container").appendChild(i);const o=setTimeout(()=>i.remove(),n);i.addEventListener("mouseenter",()=>clearTimeout(o)),i.addEventListener("mouseleave",()=>setTimeout(()=>i.remove(),1e3))}function setLoading(t="\u0110ang t\u1EA3i..."){$("#app").innerHTML=`
  ...
  `);const t=$("#login-username")?.value?.trim();if(t){const e=$("#forgot-password-username");e&&(e.value=t)}setTimeout(()=>$("#forgot-password-username")?.focus(),50)}async function submitForgotPassword(t){const e=$("#forgot-password-username")?.value?.trim()||$("#login-username")?.value?.trim()||"";if(!e){toast("Vui l\xF2ng nh\u1EADp username \u0111\u1EC3 g\u1EEDi link \u0111\u1EB7t l\u1EA1i m\u1EADt kh\u1EA9u.","error");return}btnLoading(t);try{const n=await api.post("/auth/forgot-password",{username:e});closeModal(),toast(n.message||"Link \u0111\u1EB7t l\u1EA1i m\u1EADt kh\u1EA9u \u0111\xE3 \u0111\u01B0\u1EE3c g\u1EEDi qua Gmail.","success")}catch(n){btnReset(t),toast(n.error||"Kh\xF4ng th\u1EC3 g\u1EEDi link \u0111\u1EB7t l\u1EA1i l\xFAc n\xE0y.","error")}}function showResetPasswordPage(t){$("#app").classList.remove("with-header"),$("#app-header").classList.add("hidden"),$("#app").innerHTML=`
    ...
    </div>`}function buildProgressChartSvg(t,e,n,i,s="profile"){const a=s==="profile"?520:320,o=s==="profile"?220:150,l=s==="profile"?42:30,d=14,c=14,p=s==="profile"?28:24,r=a-l-d,v=o-c-p,u="#dbe4ef",m="#94a3b8",h={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"}[i]||"#0f766e",g=new Date;g.setHours(23,59,59,999);const w=new Date(g);w.setDate(g.getDate()-(n-1)),w.setHours(0,0,0,0);const x=Math.max(1,g.getTime()-w.getTime()),S=y=>l+(y.getTime()-w.getTime())/x*r,b=y=>c+(9-y)/9*v,C=[0,3,6,9].map(y=>{const k=b(y).toFixed(1);return`
      <line x1="${l}" y1="${k}" x2="${a-d}" y2="${k}" stroke="${u}" stroke-width="1"/>
      <text x="${l-8}" y="${Number(k)+4}" text-anchor="end" font-size="10" fill="${m}">${y}</text>`}).join(""),L=Number.isFinite(e)?`
    <line x1="${l}" y1="${b(e).toFixed(1)}" x2="${a-d}" y2="${b(e).toFixed(1)}"
      stroke="${h}" stroke-opacity=".38" stroke-width="1.5" stroke-dasharray="5 5"/>
  `:"",M=t.map(y=>`${S(y.date).toFixed(1)},${b(y.avgScore).toFixed(1)}`).join(" "),A=t.length>=2?`<polyline fill="none" stroke="${h}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" points="${M}"/>`:"",H=t.map(y=>{const k=S(y.date).toFixed(1),E=b(y.avgScore).toFixed(1);return`
      <circle cx="${k}" cy="${E}" r="5.5" fill="#fff" stroke="${h}" stroke-width="3"
        onmouseenter="showProgressPointTooltip(event, '${i}', '${y.dateKey}')"
        onmousemove="moveProgressTooltip(event)"
        onmouseleave="hideProgressTooltip()"></circle>`}).join(""),B=toDateKey(w).slice(5).replace("-","/"),f=toDateKey(g).slice(5).replace("-","/");return`
    <svg viewBox="0 0 ${a} ${o}" preserveAspectRatio="none" class="progress-chart-svg" aria-hidden="true">
      ${C}
      ${L}
      ${A}
      ${H}
      <text x="${l}" y="${o-6}" font-size="10" fill="${m}">${B}</text>
      <text x="${a-d}" y="${o-6}" text-anchor="end" font-size="10" fill="${m}">${f}</text>
    ...
    </div>`}function renderSkillChartCard(t,e,n,i="profile"){const a=getTargetSettings()[e],o=getSkillGradedAssignments(t,e),l=getSubmittedAssignments(t).filter(h=>h.skill===e).length,d=aggregateDailyScores(t,e,n),c={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"}[e]||"#0f766e",p=o.length?o.reduce((h,g)=>h+Number(g.overall_score),0)/o.length:null,r=o.length?Number(o[o.length-1].overall_score):null,v=p!==null?p-a:null,u=v===null?"":v>=0?`<span class="chart-skill-delta ok">+${v.toFixed(1)} vs target</span>`:`<span class="chart-skill-delta gap">${v.toFixed(1)} vs target</span>`,m=d.length?buildProgressChartSvg(d,a,n,e,i):`<div class="chart-empty-state">
    ...
    </div>`,a.classList.remove("hidden"),moveProgressTooltip(t)}function moveProgressTooltip(t){const e=ensureProgressTooltip();if(e.classList.contains("hidden"))return;const n=16,i=e.offsetWidth||260,s=e.offsetHeight||120;let a=t.clientX+n,o=t.clientY+n;const l=window.visualViewport?.width??window.innerWidth,d=window.visualViewport?.height??window.innerHeight;a+i>l-12&&(a=t.clientX-i-n),o+s>d-12&&(o=t.clientY-s-n),e.style.left=`${Math.max(12,a)}px`,e.style.top=`${Math.max(12,o)}px`}function hideProgressTooltip(){ensureProgressTooltip().classList.add("hidden")}function setProgressRange(t,e){if(t==="home"){_homeChartRange=e,renderHome(window._cachedAssignments||[]);return}_profileChartRange=e,renderProfile(window._cachedAssignments||[])}function scrollProfileSection(t){const e=document.getElementById(t);e&&e.scrollIntoView({behavior:"smooth",block:"start"})}async function showHome(){setLoading("\u0110ang t\u1EA3i trang ch\u1EE7...");try{const[t,e]=await Promise.all([getAssignments(),api.get("/student/vocab/sessions").catch(()=>[])]);window._cachedVocabSessions=e,renderHome(t)}catch(t){toast("L\u1ED7i t\u1EA3i trang ch\u1EE7: "+(t.error||t.message),"error")}}function renderHome(t){const e=calculateSubmissionStreak(t,window._cachedVocabSessions||[]),n=t.length,i=getSubmittedAssignments(t).length,s=t.filter(r=>!r.submission_id&&r.is_active).length,a=calculateOverallAverage(t),o=toDateKey(new Date),l=t.filter(r=>r.submission_id||!r.is_active||!r.deadline?!1:toDateKey(r.deadline)===o),d=t.filter(r=>!r.submission_id&&r.is_active).sort((r,v)=>r.deadline?v.deadline?new Date(r.deadline)-new Date(v.deadline):-1:1).slice(0,5),c=[],p=new Set(e.days);for(let r=6;r>=0;r--){const v=new Date;v.setDate(v.getDate()-r);const u=toDateKey(v),m=v.toLocaleDateString("vi-VN",{weekday:"short"});c.push(`<div class="home-streak-day ${p.has(u)?"on":""}" title="${u}">
    ...
    </a>`}let _historyFilter={skill:"",minBand:0};async function showHistory(){setLoading("\u0110ang t\u1EA3i l\u1ECBch s\u1EED...");try{const t=await getAssignments();renderHistory(getHistorySourceItems(t))}catch(t){toast("L\u1ED7i t\u1EA3i l\u1ECBch s\u1EED: "+(t.error||t.message),"error")}}function renderHistory(t){let e=t.slice();_historyFilter.skill&&(e=e.filter(i=>i.skill===_historyFilter.skill)),_historyFilter.minBand>0&&(e=e.filter(i=>Number(getHistoryItemScore(i)||0)>=_historyFilter.minBand)),e.sort((i,s)=>new Date(s.submitted_at||s.created_at)-new Date(i.submitted_at||i.created_at));const n=[["","T\u1EA5t c\u1EA3"],...FILTERABLE_ASSIGNMENT_SKILLS.map(i=>[i,`${SKILL_ICONS[i]} ${SKILL_LABELS[i]}`])];$("#app").innerHTML=`
    <div class="container">
    ...
    </a>`}function setHistoryFilter(t,e){_historyFilter[t]=e,renderHistory(getHistorySourceItems(window._cachedAssignments||[]))}let _calMonth=null;async function showCalendar(){setLoading("\u0110ang t\u1EA3i l\u1ECBch...");try{const t=await getAssignments();if(!_calMonth){const e=new Date;_calMonth=[e.getFullYear(),e.getMonth()]}renderCalendar(t)}catch(t){toast("L\u1ED7i t\u1EA3i l\u1ECBch: "+(t.error||t.message),"error")}}function renderCalendar(t){const[e,n]=_calMonth,i=new Date(e,n,1),s=new Date(e,n+1,0),a=(i.getDay()+6)%7,o=s.getDate(),l={};for(const r of t){if(r.deadline){const v=toDateKey(r.deadline);(l[v]||(l[v]={deadlines:[],submissions:[]})).deadlines.push(r)}if(r.submission_id){const v=toDateKey(r.submitted_at||r.created_at);v&&(l[v]||(l[v]={deadlines:[],submissions:[]})).submissions.push(r)}}const d=["T2","T3","T4","T5","T6","T7","CN"].map(r=>`<div class="cal-weekday">${r}</div>`).join("");let c="";for(let r=0;r<a;r++)c+='<div class="cal-cell empty"></div>';for(let r=1;r<=o;r++){const v=`${e}-${String(n+1).padStart(2,"0")}-${String(r).padStart(2,"0")}`,u=l[v],m=new Date().toISOString().slice(0,10),h=v===m;let g="";u&&(u.deadlines.length&&(g+=`<span class="cal-dot deadline" title="${u.deadlines.length} deadline"></span>`),u.submissions.length&&(g+=`<span class="cal-dot submitted" title="${u.submissions.length} \u0111\xE3 n\u1ED9p"></span>`)),c+=`
      <div class="cal-cell${h?" today":""}${u?" has-event":""}" data-day="${v}" onclick="selectCalDay('${v}')">
    ...
    ${n.map(s=>`
      ...
      </a>`).join("")}`}}function profileTargetKey(){return`ielts_target:${_student?.id}`}function getTargetBand(){return parseFloat(localStorage.getItem(profileTargetKey()))||6.5}function setTargetBand(t){localStorage.setItem(profileTargetKey(),String(t))}async function showProfile(){setLoading("\u0110ang t\u1EA3i h\u1ED3 s\u01A1...");try{const[t,e,n]=await Promise.all([getAssignments(),api.get("/student/profile-answers").catch(()=>({fields:[],answers:{}})),api.get("/student/vocab/sessions").catch(()=>[]),loadMyVocab()]);window._cachedProfileData=e,window._cachedVocabSessions=n,updateStudentState({email:getStudentNotificationEmail(e)||null}),renderProfile(t,e)}catch(t){toast("L\u1ED7i: "+(t.error||t.message),"error")}}function renderProfile(t){const e=calculateSubmissionStreak(t,window._cachedVocabSessions||[]),n=getTargetBand(),i=getSubmittedAssignments(t),a=t.filter(f=>f.submission_id&&f.overall_score!=null).map(f=>Number(f.overall_score)).filter(Boolean),o=a.length?(a.reduce((f,y)=>f+y,0)/a.length).toFixed(1):"\u2014",l=["reading","listening","writing","speaking"],d=getIeltsGradedAssignments(t),c=l.map(f=>{const y=d.filter(T=>T.skill===f).sort((T,N)=>new Date(T.created_at)-new Date(N.created_at)),k=y.map(T=>Number(T.overall_score)).filter(T=>T>0),E=k.length?k.reduce((T,N)=>T+N,0)/k.length:null,I=k.length?Math.max(...k):null;return{sk:f,count:y.length,avg:E,best:I,timeline:k}}),p=new Set(i.map(f=>toDateKey(f.submitted_at||f.created_at)).filter(Boolean)),r=new Set(e.days),v=[];for(let f=6;f>=0;f--){const y=new Date;y.setDate(y.getDate()-f);const k=toDateKey(y),E=y.toLocaleDateString("vi-VN",{weekday:"short"});v.push(`<div class="home-streak-day ${r.has(k)?"on":""}" title="${k}">
      <div class="streak-dot">${r.has(k)?"\u{1F525}":"\xB7"}</div>
      <div class="streak-day-label">${E}</div>
    </div>`)}const u=(_myVocabCache||[]).length,m=[5,5.5,6,6.5,7,7.5,8,8.5,9].map(f=>`<option value="${f}" ${n===f?"selected":""}>${f}</option>`).join(""),h=(_student.full_name||"?").split(" ").map(f=>f[0]).join("").slice(0,2).toUpperCase(),g=e.current>=30?"#dc2626":e.current>=7?"#f59e0b":"#6b7280",w={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"},x={reading:"\u{1F4D6}",listening:"\u{1F3A7}",writing:"\u270D\uFE0F",speaking:"\u{1F3A4}"},S={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking"},b=f=>i.filter(y=>y.skill===f).sort((y,k)=>new Date(k.submitted_at||k.created_at)-new Date(y.submitted_at||y.created_at));function _(f,y){if(f.length<2)return"";const k=100,E=28,I=f.map((P,D)=>{const F=D/(f.length-1)*k,q=E-P/9*E;return`${F.toFixed(1)},${q.toFixed(1)}`}).join(" "),T=k,N=E-f[f.length-1]/9*E;return`<svg viewBox="0 0 ${k} ${E}" preserveAspectRatio="none" style="width:100%;height:28px;display:block">
      <polyline fill="none" stroke="${y}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" points="${I}"/>
      <circle cx="${T.toFixed(1)}" cy="${N.toFixed(1)}" r="3" fill="${y}"/>
    </svg>`}function C({sk:f,count:y,avg:k,best:E,timeline:I}){const T=w[f],N=k!==null?Math.min(100,Math.round(k/n*100)):0,P=k===null?"":k>=n?"\u2705":k>=n-.5?"\u{1F7E1}":"\u{1F534}",D=_(I,T),F=k!==null?k>=n?`<span class="spc-vs ok">+${(k-n).toFixed(1)} vs target</span>`:`<span class="spc-vs gap">-${(n-k).toFixed(1)} vs target</span>`:"",q=b(f).length;return`
      <button class="skill-progress-card spc-${f}" type="button"
        onclick="openSkillProgressModal('${f}')">
        <div class="spc-head">
          <span class="spc-icon">${x[f]}</span>
          <span class="spc-name">${S[f]}</span>
          <span>${P}</span>
        </div>
        ...
        <div class="spc-meta">${y===0?"Ch\u01B0a c\xF3 b\xE0i n\xE0o \u0111\u01B0\u1EE3c ch\u1EA5m":`${y} b\xE0i \u0111\xE3 ch\u1EA5m \xB7 Best: ${E}`}</div>
        <div class="spc-footnote">${q>0?`${q} b\xE0i \u0111\xE3 l\xE0m \xB7 Nh\u1EA5n \u0111\u1EC3 xem chi ti\u1EBFt`:"Ch\u01B0a c\xF3 b\xE0i \u0111\xE3 l\xE0m"}</div>
      </button>`}const L=28,M=new Date;M.setDate(M.getDate()-(L-1));const A=(M.getDay()+6)%7,H=Array(A).fill('<div class="act-cell act-pad"></div>');for(let f=L-1;f>=0;f--){const y=new Date;y.setDate(y.getDate()-f);const k=y.toISOString().slice(0,10);H.push(`<div class="act-cell ${p.has(k)?"act-done":"act-none"}" title="${k}"></div>`)}const B=[];c.forEach(({sk:f,best:y})=>{y!==null&&B.push(`<div class="ms-card"><div class="ms-icon">${x[f]}</div><div class="ms-val">${y}</div><div class="ms-label">Best ${S[f]}</div></div>`)}),i.length>0&&B.push(`<div class="ms-card"><div class="ms-icon">\u{1F4DD}</div><div class="ms-val">${i.length}</div><div class="ms-label">B\xE0i \u0111\xE3 n\u1ED9p</div></div>`),e.best>1&&B.push(`<div class="ms-card"><div class="ms-icon">\u{1F525}</div><div class="ms-val">${e.best}</div><div class="ms-label">Streak k\u1EF7 l\u1EE5c</div></div>`),u>0&&B.push(`<div class="ms-card"><div class="ms-icon">\u{1F4D6}</div><div class="ms-val">${u}</div><div class="ms-label">T\u1EEB \u0111\xE3 l\u01B0u</div></div>`),$("#app").innerHTML=`
    <div class="container profile-page">

      <!-- Hero -->
      <div class="profile-hero">
        <div class="profile-avatar" style="background:${w.reading}">${h}</div>
        <div class="profile-hero-info">
          ...
          <div class="profile-meta-row">
            <span class="profile-streak" style="color:${g}">\u{1F525} ${e.current} ng\xE0y</span>
      ...
      <div class="home-streak-card">
        ...
        <div class="home-streak-week">${v.join("")}</div>
      </div>
      ...
      <div class="activity-wrap">
        <div class="activity-day-labels">${["T2","T3","T4","T5","T6","T7","CN"].map(f=>`<span>${f}</span>`).join("")}</div>
    ...
    </a>`},renderCalendar=function(t){const[e,n]=_calMonth,i=new Date(e,n,1),s=new Date(e,n+1,0),a=(i.getDay()+6)%7,o=s.getDate(),l={};for(const r of t){if(r.deadline){const v=toDateKey(r.deadline);(l[v]||(l[v]={deadlines:[],submissions:[]})).deadlines.push(r)}if(r.submission_id){const v=toDateKey(r.submitted_at||r.created_at);v&&(l[v]||(l[v]={deadlines:[],submissions:[]})).submissions.push(r)}}const d=["T2","T3","T4","T5","T6","T7","CN"].map(r=>`<div class="cal-weekday">${r}</div>`).join("");let c="";for(let r=0;r<a;r++)c+='<div class="cal-cell empty"></div>';for(let r=1;r<=o;r++){const v=`${e}-${String(n+1).padStart(2,"0")}-${String(r).padStart(2,"0")}`,u=l[v],m=v===toDateKey(new Date);let h="";u&&(u.deadlines.length&&(h+=`<span class="cal-dot deadline" title="${u.deadlines.length} deadline"></span>`),u.submissions.length&&(h+=`<span class="cal-dot submitted" title="${u.submissions.length} \u0111\xE3 n\u1ED9p"></span>`)),c+=`
      <div class="cal-cell${m?" today":""}${u?" has-event":""}" data-day="${v}" onclick="selectCalDay('${v}')">
        <div class="cal-day-num">${r}</div>
    ...
    ${n.map(s=>`
      <a href="#/${s.submission_id?"result":"assignment"}/${s.id}" class="cal-event-item">
        ...
        <span class="cal-event-title">${escapeHtml(s.title)}</span>
        <span class="cal-event-status">${s.submission_id?"\u2705 \u0110\xE3 n\u1ED9p":"\u23F3 H\u1EA1n"}</span>
      </a>`).join("")}`}},renderProfile=function(t,e){e=e||window._cachedProfileData||{fields:[],answers:{}};const n=(e.fields||[]).find(b=>b.field_key==="notification_email"),i=(e.fields||[]).filter(b=>b.field_key!=="notification_email"),s=_student&&typeof _student.email=="string"&&_student.email.trim()||e.student&&typeof e.student.email=="string"&&e.student.email.trim()||(n?String(e.answers?.[n.id]||"").trim():""),a=calculateSubmissionStreak(t,window._cachedVocabSessions||[]),o=getTargetSettings(),l=getSubmittedAssignments(t),d=calculateOverallAverage(t),c=d!==null?d.toFixed(1):"\u2014",p=SKILL_ORDER.map(b=>{const _=getSkillGradedAssignments(t,b).map(C=>Number(C.overall_score)).filter(Number.isFinite);return{sk:b,best:_.length?Math.max(..._):null}}),r=new Set(l.map(b=>toDateKey(b.submitted_at||b.created_at)).filter(Boolean)),v=new Set(a.days),u=[];for(let b=6;b>=0;b--){const _=new Date;_.setDate(_.getDate()-b);const C=toDateKey(_),L=_.toLocaleDateString("vi-VN",{weekday:"short"});u.push(`<div class="home-streak-day ${v.has(C)?"on":""}" title="${C}">
      <div class="streak-dot">${v.has(C)?"\u{1F525}":"\xB7"}</div>
      <div class="streak-day-label">${L}</div>
    </div>`)}const m=(_myVocabCache||[]).length,h=(_student.full_name||"?").split(" ").map(b=>b[0]).join("").slice(0,2).toUpperCase(),g=a.current>=30?"#dc2626":a.current>=7?"#f59e0b":"#6b7280",w={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"},x=[];p.forEach(({sk:b,best:_})=>{_!==null&&x.push(`<div class="ms-card"><div class="ms-icon">${SKILL_ICONS[b]}</div><div class="ms-val">${_}</div><div class="ms-label">Best ${SKILL_LABELS[b]}</div></div>`)}),l.length>0&&x.push(`<div class="ms-card"><div class="ms-icon">\u{1F4DD}</div><div class="ms-val">${l.length}</div><div class="ms-label">B\xE0i \u0111\xE3 n\u1ED9p</div></div>`),a.best>1&&x.push(`<div class="ms-card"><div class="ms-icon">\u{1F525}</div><div class="ms-val">${a.best}</div><div class="ms-label">Streak k\u1EF7 l\u1EE5c</div></div>`),m>0&&x.push(`<div class="ms-card"><div class="ms-icon">\u{1F4D6}</div><div class="ms-val">${m}</div><div class="ms-label">T\u1EEB \u0111\xE3 l\u01B0u</div></div>`);const S=[["profile-info","\u{1F4CB} Th\xF4ng tin"],["profile-streak","\u{1F525} Streak"],["profile-targets","\u{1F3AF} Target"],["profile-progress","\u{1F4C8} Ti\u1EBFn \u0111\u1ED9"],...x.length>0?[["profile-achievements","\u{1F3C6} Th\xE0nh t\xEDch"]]:[],["profile-next-steps","\u{1F680} Ti\u1EBFp t\u1EE5c h\u1ECDc"]];$("#app").innerHTML=`
    <div class="container profile-page">
      <div class="profile-hero">
        <div class="profile-avatar" style="background:${w.reading}">${h}</div>
        <div class="profile-hero-info">
          ...
          <div class="profile-meta">L\u1EDBp ${escapeHtml(_selectedClass.class_name)}</div>
          <div class="profile-meta-row">
            <span class="profile-streak" style="color:${g}">\u{1F525} ${a.current} ng\xE0y</span>
            <span class="profile-dot">\xB7</span>
      ...
      <div class="profile-nav">
        <div class="profile-nav-list">
          ${S.map(([b,_])=>`
            <button type="button" class="profile-nav-btn" onclick="scrollProfileSection('${b}')">${_}</button>
          `).join("")}
        </div>
      </div>
        ...
        ${i.length===0?"":`<details class="pi-details">
              ...
              <div class="pi-details-body">
                <div class="pi-fields" id="pi-fields">
                  ${i.map(b=>{const _=e.answers[b.id]||"",C=`pi-field-${b.id}`;let L;if(b.field_type==="textarea")L=`<textarea id="${C}" class="form-input pi-input" data-field-id="${b.id}" rows="3" placeholder="Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u">${escapeHtml(_)}</textarea>`;else if(b.field_type==="select"&&Array.isArray(b.options)){const M=b.options.map(A=>`<option value="${escapeHtml(String(A))}" ${_===String(A)?"selected":""}>${escapeHtml(String(A))}</option>`).join("");L=`<select id="${C}" class="form-input pi-input" data-field-id="${b.id}"><option value="">-- Ch\u01B0a ch\u1ECDn --</option>${M}</select>`}else if(b.field_type==="date")L=`<input id="${C}" class="form-input pi-input" type="date" data-field-id="${b.id}" value="${escapeHtml(_)}" />`;else{const M=b.field_key==="notification_email"?"email":"text",A=b.field_key==="notification_email"?"name@example.com":"Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u";L=`<input id="${C}" class="form-input pi-input" type="${M}" data-field-id="${b.id}" value="${escapeHtml(_)}" placeholder="${A}" />`}return`<div class="pi-field">
                      <label class="pi-label" for="${C}">${escapeHtml(b.label)}</label>
                      ${L}
                    </div>`}).join("")}
                </div>
      ...
      ${x.length>0?`
        ...
        <div class="profile-section-title">\u{1F3C6} Th\xE0nh t\xEDch c\xE1 nh\xE2n</div>
        <div class="milestones-row">${x.join("")}</div>
        </section>
      `:""}
    ...
    <div class="container mfc-page">
      <div class="mfc-end">
    ...
    </div>`}window.flipMyFC=flipMyFC,window.mfcAnswer=mfcAnswer;async function showAssignments(){setLoading("\u0110ang t\u1EA3i danh s\xE1ch b\xE0i t\u1EADp...");try{const t=await getAssignments();renderAssignments(t)}catch(t){toast("L\u1ED7i t\u1EA3i b\xE0i t\u1EADp: "+(t.error||t.message),"error")}}function renderAssignments(t){const e=_assignmentSkillFilter?t.filter(c=>c.skill===_assignmentSkillFilter):t,n=e.filter(c=>c.skill!=="composite"&&c.rewrite_status==="requested"),i=e.filter(c=>c.skill!=="composite"?!c.submission_id&&c.is_active:c.is_active&&!isCompositeAssignmentDone(c)),s=e.filter(c=>c.skill!=="composite"?!c.submission_id&&!c.is_active:!c.is_active&&!isCompositeAssignmentDone(c)),a=e.filter(c=>c.skill!=="composite"?!!c.submission_id&&c.rewrite_status!=="requested":isCompositeAssignmentDone(c));function o(c){const p=c.skill==="composite",r=isOverdue(c.deadline),v=p?isCompositeAssignmentDone(c):!!c.submission_id,u=!c.is_active&&!v,m=SKILL_ICONS[c.skill]||"\u{1F4DD}",h=!v&&!r&&!u?formatCountdown(c.deadline):null;let g,w;if(p){const C=getCompositeSectionsForAssignment(c),L=C.filter(A=>A.submitted).length,M=getCompositeAssignmentAverageScore(c);u?g='<span class="badge badge-closed">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>':v?g=isCompositeAssignmentFullyGraded(c)?'<span class="badge badge-done">\u2705 \u0110\xE3 c\xF3 \u0111i\u1EC3m</span>':'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>':L>0?g=`<span class="badge badge-pending">${L}/${C.length} ph\u1EA7n</span>`:g='<span class="badge badge-pending">Ch\u01B0a l\xE0m</span>',w=isCompositeAssignmentFullyGraded(c)&&M!==null?`<div class="score-band">${M.toFixed(1)}</div><div class="score-label">Band</div>`:""}else if(u)g='<span class="badge badge-closed">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>',w="";else if(v){const C=c.overall_score!==null&&c.overall_score!==void 0;c.rewrite_status==="requested"?(g='<span class="badge badge-rewrite">\u270F\uFE0F Y\xCAU C\u1EA6U VI\u1EBET L\u1EA0I</span>',w=C?`<div class="score-band rewrite-score">${c.overall_score}</div><div class="score-label">Band</div>`:'<div class="score-pending-icon">\u270F\uFE0F</div>'):(g=C?'<span class="badge badge-done">\u2705 \u0110\xE3 c\xF3 \u0111i\u1EC3m</span>':'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>',w=C?`<div class="score-band">${c.overall_score}</div><div class="score-label">Band</div>`:'<div class="score-pending-icon">\u23F3</div>')}else r&&c.deadline?(g='<span class="badge badge-overdue">\u26A0\uFE0F Qu\xE1 h\u1EA1n</span>',w=""):(g='<span class="badge badge-pending">Ch\u01B0a l\xE0m</span>',w="");if(u)return`
        <div class="assignment-card assignment-card-closed">
          <div class="assignment-card-icon">${m}</div>
        ...
        </div>`;const x=!p&&c.rewrite_status==="requested",S=p?c.is_active&&!isCompositeAssignmentDone(c):!c.submission_id&&c.is_active,b=p?`#/composite/${c.id}`:v?`#/result/${c.id}`:`#/assignment/${c.id}`;return`
      <a class="assignment-card ${x?"rewrite":S?"pending-card":v?"done":""}" href="${b}">
        <div class="assignment-card-icon">${m}</div>
        <div class="assignment-card-body">
          <div class="assignment-card-title">${escapeHtml(c.title)}</div>
          <div class="assignment-card-meta">
          ...
          <div class="assignment-card-deadline-row">
            ...
            <span class="assignment-card-deadline ${r&&!v?"overdue":""}">
    ...
    <div class="audio-submit-processing">
      ...
      <tr>
        ...
        <td>${escapeHtml(m)||'<em style="color:var(--gray-400)">B\u1ECF tr\u1ED1ng</em>'}</td>
        <td>${escapeHtml(u.answers.join(" / "))}</td>
        <td class="${h?"result-correct":"result-wrong"}">${h?"\u2713":"\u2717"}</td>
        ${g}
      </tr>${w}`}).join(""),l=e.length,d=(t.scoring_scale||"10")==="ielts",c=t.overall_score??(l>0?d?Math.round(i/l*9*10)/10:Math.round(i/l*10*10)/10:0),p=s?5:4,r=t.vocabulary||[],v=r.length===0?"":`
    <div class="section-label" style="margin-top:20px">\u{1F4DA} T\u1EEB v\u1EF1ng trong b\xE0i</div>
    <div class="vocab-result-list">
      ${r.map((u,m)=>{const h=isWordSaved(u.word);return`
        <div class="vocab-result-item">
          <div class="vocab-result-header" onclick="toggleVocabItem(${m})">
            <span class="vocab-result-word">${escapeHtml(u.word)}</span>
            <span class="vocab-result-toggle" id="vocab-toggle-${m}">\u25B6</span>
          ...
          <div class="vocab-result-detail hidden" id="vocab-detail-${m}">
    ...
    </div>`}function renderWritingFeedback(t,e=null){const n=t.teacher_feedback||{},i=(n.annotations||[]).sort((g,w)=>g.start-w.start),s=n.overall||"",a=t.overall_score??n.score,o=countWords(t.writing_content||""),l=e?[...e].sort((g,w)=>(w.attempt_number||1)-(g.attempt_number||1)):null,c=(l?l[0]:t).rewrite_status==="requested",p=e&&e.length>1,r=s?`
    <div class="section-label">Nh\u1EADn x\xE9t t\u1ED5ng th\u1EC3</div>
    <div class="feedback-overall">${escapeHtml(s)}</div>`:"",v=_annColorMap(i),u=_buildAnnSidebar(i,v),m=p?`
    <div class="version-selector">
      <span class="version-selector-label">Xem k\u1EBFt qu\u1EA3:</span>
      ${[...e].sort((g,w)=>(g.attempt_number||1)-(w.attempt_number||1)).map(g=>`
        <button class="version-btn${g.id===t.id?" active":""}"
          onclick="switchWritingVersion('${g.id}','${t.assignment_id}')">
    ...
    <div class="assignment-page">
      ...
      </div>`}).join("")}</div>`}function _annColorMap(t){if(!t||!t.length)return new Map;const e=t.filter(s=>(s.type||"highlight")==="highlight");if(!e.length)return new Map;const n=new Map(e.map(s=>[s.id,0])),i=[...e].sort((s,a)=>s.end-s.start-(a.end-a.start));for(const s of i)for(const a of e)a!==s&&a.start<=s.start&&a.end>=s.end&&n.set(a.id,Math.max(n.get(a.id),n.get(s.id)+1));return new Map(e.map(s=>[s.id,Math.min(n.get(s.id),ANN_COLORS.length-1)]))}function buildAnnotatedHtml(t,e){if(!t)return'<span style="color:var(--gray-400)">(Tr\u1ED1ng)</span>';if(!e||e.length===0)return escapeHtml(t);const n=_annColorMap(e),i=[...e].sort((d,c)=>d.start-c.start),s=new Map(i.map((d,c)=>[d.id,c+1]));let a=0;const o=new Map(i.map(d=>(d.type||"highlight")==="highlight"?[d.id,++a]:[d.id,null]));function l(d,c,p){if(d>=c)return"";if(!p.length)return escapeHtml(t.slice(d,c));const r=[...p].sort((h,g)=>h.start-g.start||g.end-g.start-(h.end-h.start));let v="",u=d;const m=new Set;for(const h of r){if(m.has(h.id))continue;const g=Math.max(h.start,u);if(g>=h.end)continue;v+=escapeHtml(t.slice(u,g));const w=r.filter(S=>!m.has(S.id)&&S!==h&&S.start>=h.start&&S.end<=h.end);w.forEach(S=>m.add(S.id)),m.add(h.id);const x=s.get(h.id)-1;if((h.type||"highlight")==="delete")v+=`<span class="ann-delete" id="ann-mark-${x}" title="${escapeHtml(h.comment)}">`,v+=l(g,h.end,w),v+="</span>";else{const S=ANN_COLORS[n.get(h.id)];v+=`<mark class="ann-highlight ${S}" id="ann-mark-${x}" title="${escapeHtml(h.comment)}">`,v+=l(g,h.end,w),v+=`<sup class="ann-marker ann-marker-c${n.get(h.id)}">${o.get(h.id)}</sup>`,v+="</mark>"}u=h.end}return v+=escapeHtml(t.slice(u,c)),v}return l(0,t.length,e)}function scrollToFeedbackMark(t){const e=document.getElementById(`ann-mark-${t}`);if(!e)return;const n=document.getElementById("feedback-content-pane");if(n){const i=e.getBoundingClientRect().top-n.getBoundingClientRect().top;n.scrollTop+=i-n.clientHeight/3}else e.scrollIntoView({behavior:"smooth",block:"center"});e.classList.add("ann-flash"),setTimeout(()=>e.classList.remove("ann-flash"),1500)}function renderSpeakingResult(t){const e=t.teacher_feedback;t.overall_score!=null||e?.annotations?.length>0||e?.overall?renderSpeakingFeedback(t):renderSpeakingPending(t)}function renderSpeakingPending(t){$("#app").innerHTML=`
    <div class="container">
      <button class="btn-back" onclick="navigate('${getResultBackHref()}')">${getResultBackLabel()}</button>
      <div class="result-header">
        <div>${skillBadge(t.skill)}</div>
    ...
    </div>`}function renderSpeakingFeedback(t){const e=t.teacher_feedback||{},n=(e.annotations||[]).sort((d,c)=>d.start-c.start),i=e.overall||"",s=t.overall_score??e.score,a=i?`
    <div class="section-label">Nh\u1EADn x\xE9t t\u1ED5ng th\u1EC3</div>
    <div class="feedback-overall">${escapeHtml(i)}</div>`:"",o=_annColorMap(n),l=_buildAnnSidebar(n,o);$("#app").innerHTML=`
    <div class="assignment-page">
      <div class="assignment-toolbar">
        <button class="btn-back" onclick="navigate('${getResultBackHref()}')">${getResultBackLabel()}</button>
        <div class="assignment-toolbar-title">${skillBadge(t.skill)} ${escapeHtml(t.assignment_title||"")}</div>
        <div class="score-chip">
          <span class="score-chip-val">${s??"\u2014"}</span>
          <span class="score-chip-label">/9.0</span>
        </div>
      </div>
      <div class="assignment-content">
        <div class="content-pane" id="feedback-content-pane">
          ${a}
          ${(()=>{const d=Array.isArray(t.speaking_audio_urls)&&t.speaking_audio_urls.length>0?t.speaking_audio_urls:t.speaking_audio_url?[{url:t.speaking_audio_url}]:[];return d.length?`<div class="section-label"${i?' style="margin-top:20px"':""}>Audio ghi \xE2m c\u1EE7a b\u1EA1n</div>
              <div style="margin-bottom:16px">
                ${d.map((c,p)=>`
                  ${d.length>1?`<div style="font-size:12px;color:var(--gray-400);margin-bottom:4px">${escapeHtml(c.name||"Ph\u1EA7n "+(p+1))}</div>`:""}
                  <audio controls src="${escapeHtml(c.url||"")}" style="width:100%;height:36px;outline:none;margin-bottom:${p<d.length-1?"10px":"0"}"></audio>
                `).join("")}
              </div>`:""})()}
          <div class="section-label">Transcript (AI Generated)
            ${n.length>0?'<span class="feedback-hint">M\xE0u highlight = nh\u1EADn x\xE9t \xB7 B\u1EA5m s\u1ED1 \u0111\u1EC3 xem</span>':""}
          </div>
          <div class="submitted-content feedback-essay">${buildAnnotatedHtml(t.speaking_script||"",n)}</div>
        </div>
        <div class="answer-pane">
          ${l}
        </div>
      </div>
    </div>`}const _vocabExpanded={};let _vocabSearch="";async function showVocabGames(){setLoading("\u0110ang t\u1EA3i t\u1EEB v\u1EF1ng...");const t=!window._cachedAssignments&&_student&&_selectedClass;await Promise.all([loadMyVocab(),t?getAssignments().then(e=>{window._cachedAssignments=e}).catch(()=>{}):Promise.resolve()]),_vocabSearch="",renderVocabGames()}function buildVocabTeacherHtml(t){const e=_vocabSearch.toLowerCase().trim(),n=e?t.filter(l=>l.title.toLowerCase().includes(e)||l.skill.toLowerCase().includes(e)):t,i=["reading","listening","writing","speaking"],s={};for(const l of i)s[l]=[];for(const l of n)s[l.skill]?s[l.skill].push(l):s[l.skill]=[l];if(n.length===0)return`<div class="vocab-empty-mini">
    ...
    </div>`;const a=3;let o="";for(const l of i){const d=s[l];if(!d||d.length===0)continue;const c=!!_vocabExpanded[l],p=c?d:d.slice(0,a),r=d.length-a,v=p.map(m=>`
      <a class="vocab-game-card" href="#/vocab-game/${m.id}">
        <div class="vocab-game-card-icon">${SKILL_ICONS[m.skill]}</div>
        <div class="vocab-game-card-body">
          <div class="vocab-game-card-title">${escapeHtml(m.title)}</div>
          <div class="vocab-game-card-meta">
            ${skillBadge(m.skill)}
            <span class="vocab-count-badge">${m.vocab_count} t\u1EEB</span>
          </div>
        </div>
      ...
      <div class="vocab-skill-group">
        ...
        <div class="vocab-game-list">${v}</div>
        ${u}
    ...
    </div>`}function vocabToggleSkill(t){_vocabExpanded[t]=!_vocabExpanded[t],renderVocabGames()}window.vocabToggleSkill=vocabToggleSkill,window.renderVocabGames=renderVocabGames;async function showVocabGame({id:t}){setLoading("\u0110ang t\u1EA3i t\u1EEB v\u1EF1ng...");try{const e=await api.get(`/assignments/${t}/vocabulary`);if(_vocabGameId=t,_vocabGameData=e,!e.vocabulary?.length){toast("B\xE0i n\xE0y ch\u01B0a c\xF3 t\u1EEB v\u1EF1ng","error"),navigate("/vocab-games");return}renderVocabGameMenu()}catch(e){toast("L\u1ED7i t\u1EA3i t\u1EEB v\u1EF1ng: "+(e.error||e.message),"error"),navigate("/vocab-games")}}function renderVocabGameMenu(){if(!_vocabGameData)return;const t=_vocabGameData.vocabulary.length,e=Math.min(t,10);$("#app").innerHTML=`
    <div class="container">
      <div class="page-header">
  ...
  `).join(""),a=i.length===0?`<div class="empty-hint">Kh\xF4ng c\xF3 \u0111\u1EC1 n\xE0o${_sharedSkillFilter!=="all"?" cho k\u1EF9 n\u0103ng n\xE0y":""}.</div>`:i.map(o=>{const l=(o.tags||[]).map(p=>`<span class="tag-chip">${escapeHtml(p)}</span>`).join(""),d=o.real_test_count>0&&o.best_score!=null,c=o.real_test_count>0?`<div class="shared-card-stats">
    ...
    <div class="shared-card" onclick="navigate('/shared-pool/${o.id}')">
      ...
      ${l?`<div class="shared-card-tags">${l}</div>`:""}
      ${c}
    </div>`}).join("")}}window.setSharedSkillFilter=setSharedSkillFilter,window.filterSharedPool=filterSharedPool;async function showSharedQuestion({id:t}){setLoading("\u0110ang t\u1EA3i \u0111\u1EC1...");try{const[e,n]=await Promise.all([api.get(`/student/shared-pool/${t}`),api.get(`/student/shared-pool/${t}/attempts`)]);_renderSharedDetail(e,n)}catch(e){toast("L\u1ED7i t\u1EA3i \u0111\u1EC1: "+(e.error||e.message),"error"),navigate("/shared-pool")}}function _renderSharedDetail(t,e){const n=(t.tags||[]).map(l=>`<span class="tag-chip">${escapeHtml(l)}</span>`).join(""),i=e.filter(l=>l.mode==="real_test"),s=!!t.time_limit_minutes,a=e.length===0?'<div class="empty-hint" style="padding:12px 0">Ch\u01B0a c\xF3 l\u1EA7n th\u1EED n\xE0o.</div>':e.slice(0,5).map(l=>`
    ...
    <div class="assignment-page">
      ...
      <tr>
        ...
        ${w}
      </tr>${x}`}).join(""),l=e.length,d=l===40,c=t.overall_score??(l>0?d?Math.round(s/l*9*10)/10:Math.round(s/l*10*10)/10:0),p=l-s,r=t.vocabulary||[],v=r.length===0?"":`
    <div class="section-label" style="margin-top:20px">\u{1F4DA} T\u1EEB v\u1EF1ng trong b\xE0i</div>
    <div class="vocab-result-list">
      ${r.map((u,m)=>{const h=isWordSaved(u.word);return`
        <div class="vocab-result-item">
    ...
    </div>`}async function showSharedPractice({id:t}){setLoading("\u0110ang t\u1EA3i...");const e=t.indexOf("?"),n=e>=0?t.slice(0,e):t,s=new URLSearchParams(e>=0?t.slice(e+1):"").get("attempt_id");try{const[a,o]=await Promise.all([api.get(`/student/shared-pool/${n}`),s?api.get(`/student/shared-attempts/${s}`):Promise.resolve(null)]);if(a.skill!=="reading"&&a.skill!=="listening"){toast("Ch\u1EBF \u0111\u1ED9 luy\u1EC7n t\u1EADp ch\u1EC9 h\u1ED7 tr\u1EE3 Reading v\xE0 Listening.","warning"),navigate(s?`/shared-attempt/${s}`:`/shared-pool/${n}`);return}const l=o?.student_answers||[];let d=a.questions_data||[];if(o&&(d=d.filter(r=>{const u=(l.find(m=>m.q_no===r.q_no)?.answer||"").trim().toLowerCase();return!r.answers?.some(m=>m.toLowerCase().trim()===u)})),d.length===0){toast("Kh\xF4ng c\xF3 c\xE2u sai n\xE0o \u0111\u1EC3 l\xE0m l\u1EA1i! \u{1F389}","success"),navigate(s?`/shared-attempt/${s}`:`/shared-pool/${n}`);return}let c="";for(const r of d)c+=`
        <div class="answer-row">
          <span class="q-label">Q${r.q_no}</span>
          <input class="answer-input" id="sprac-${r.q_no}" type="text" placeholder="\u0110\xE1p \xE1n c\xE2u ${r.q_no}" />
        </div>`;const p=s?`/shared-attempt/${s}`:`/shared-pool/${n}`;$("#app").innerHTML=`
      <div class="assignment-page">
        <div class="assignment-toolbar">
          <button class="btn-back" onclick="navigate('${p}')">\u2190 K\u1EBFt qu\u1EA3</button>
          <div class="assignment-toolbar-title">${skillBadge(a.skill)} ${escapeHtml(a.title)} \u2014 L\xE0m l\u1EA1i c\xE2u sai (${d.length} c\xE2u)</div>
          <button class="btn btn-primary btn-sm" id="submit-btn"
            onclick="submitSharedPractice(${JSON.stringify(d.map(r=>r.q_no))}, '${p}', this)">Ki\u1EC3m tra</button>
        </div>
        <div style="background:#fef3c7;border-bottom:1px solid #fbbf24;padding:8px 16px;font-size:12px;color:#92400e;display:flex;align-items:center;gap:6px">
        ...
        <div class="assignment-content">
          ...
          <div class="answer-pane">
            <div style="font-size:12px;color:var(--gray-400);margin-bottom:12px">\u0110i\u1EC1n \u0111\xE1p \xE1n cho ${d.length} c\xE2u v\xE0 b\u1EA5m <strong>Ki\u1EC3m tra</strong>.</div>
            <div class="answer-grid">${c}</div>
          </div>
        </div>
      </div>`,bindReadingTextInteractions("practice-reading-text")}catch(a){toast("L\u1ED7i t\u1EA3i b\xE0i: "+(a.error||a.message),"error"),navigate("/shared-pool")}}async function submitSharedPractice(t,e,n){const i=_sharedCtx?.poolQ,s=i?i.questions_data.filter(c=>t.includes(c.q_no)):t.map(c=>({q_no:c,answers:[]})),a=t.map(c=>({q_no:c,answer:($(`#sprac-${c}`)?.value||"").trim()}));btnLoading(n);let o=0;const l=t.map(c=>{const p=s.find(u=>u.q_no===c)||{q_no:c,answers:[]},r=(a.find(u=>u.q_no===c)?.answer||"").trim(),v=(p.answers||[]).some(u=>u.toLowerCase().trim()===r.toLowerCase());return v&&o++,`
      <tr>
  ...
  `;const i=document.getElementById("profile-name-input");i?.focus(),i?.select(),i?.addEventListener("keydown",s=>{s.key==="Enter"&&saveProfileName(),s.key==="Escape"&&renderProfile(window._cachedAssignments||[],window._cachedProfileData)})}async function saveProfileName(){const t=document.getElementById("profile-name-input");if(!t)return;const e=t.value.trim();if(!e){t.focus();return}t.disabled=!0;let n=!1;try{await api.patch("/student/me",{full_name:e}),n=!0}catch(i){t.disabled=!1,t.focus(),alert(i?.error||"Kh\xF4ng th\u1EC3 c\u1EADp nh\u1EADt t\xEAn")}if(n){updateStudentState({full_name:e});const i=document.getElementById("header-student-name");i&&(i.textContent=e),renderProfile(window._cachedAssignments||[],window._cachedProfileData)}}window.startEditProfileName=startEditProfileName,window.saveProfileName=saveProfileName;let _compositeExam=null,_activeSectionId=null,_sectionTimerInterval=null,_sectionTimerSecsLeft=0;function stopCompositeSectionTimer(){_sectionTimerInterval&&(clearInterval(_sectionTimerInterval),_sectionTimerInterval=null)}function _tickCompositeSectionTimer(){_sectionTimerSecsLeft=Math.max(0,_sectionTimerSecsLeft-1);const t=document.getElementById("csec-countdown");if(t){const e=Math.floor(_sectionTimerSecsLeft/60),n=_sectionTimerSecsLeft%60;t.textContent=`${String(e).padStart(2,"0")}:${String(n).padStart(2,"0")}`,t.className=`assign-countdown${_sectionTimerSecsLeft<60?" timer-urgent":""}`}_sectionTimerSecsLeft<=0&&(stopCompositeSectionTimer(),autoSubmitCurrentCompositeSection())}function startCompositeSectionTimer(t){stopCompositeSectionTimer(),_sectionTimerSecsLeft=Math.floor(t),_sectionTimerInterval=setInterval(_tickCompositeSectionTimer,1e3)}async function showCompositeExam({id:t}){setLoading("\u0110ang t\u1EA3i \u0111\u1EC1 t\u1ED5ng h\u1EE3p...");try{const e=await api.get(`/student/assignments/${t}`);e.sections&&(e.sections=e.sections.map(n=>({...n,question_count:Array.isArray(n.questions_data)?n.questions_data.length:0}))),_compositeExam=e,_activeSectionId=null,stopCompositeSectionTimer(),renderCompositeExam(e)}catch(e){toast("L\u1ED7i t\u1EA3i \u0111\u1EC1: "+(e.error||e.message),"error"),navigate("/assignments")}}window.showCompositeExam=showCompositeExam;function renderCompositeExam(t){stopAssignmentCountdown();const e=t.sections||[],n=e.length>0&&e.every(o=>o.submission_id),i=!t.is_active,s=e.map(o=>{const l=!!o.submission_id,d=SKILL_ICONS[o.skill]||"\u{1F4DD}",c=o.question_count>0?`${o.question_count} c\xE2u`:"",p=o.time_limit_minutes?`\u23F1 ${o.time_limit_minutes} ph\xFAt`:"",r=[c,p].filter(Boolean).join(" \xB7 "),v=l?`<span class="badge badge-done" style="font-size:11px">\u2713 \u0110\xE3 n\u1ED9p${n&&o.score!=null?` \xB7 Band ${o.score}`:""}</span>`:i?'<span class="badge badge-closed" style="font-size:11px">\u{1F512} \u0110\xE3 \u0111\xF3ng</span>':'<span class="badge badge-pending" style="font-size:11px">Ch\u01B0a l\xE0m</span>',u=l?n:!i,m=l?n?`#/composite-result/${t.id}/section/${o.submission_id}`:"":i?"":`#/composite/${t.id}/section/${o.id}`,h=l&&!n?'<div style="font-size:12px;color:var(--gray-400);margin-top:6px">\u{1F512} Ho\xE0n th\xE0nh c\xE1c ph\u1EA7n c\xF2n l\u1EA1i \u0111\u1EC3 xem k\u1EBFt qu\u1EA3</div>':"",g=l&&!n?"\u{1F512}":"\u203A";return`
    ...
    <div class="assignment-page" style="max-width:700px;margin:0 auto">
      ...
      <div style="padding:16px 0 32px">
    ...
    </div>`}window.renderCompositeExam=renderCompositeExam;async function showCompositeSectionExam({id:t,sectionId:e}){setLoading("\u0110ang t\u1EA3i...");try{if(!_compositeExam||_compositeExam.id!==t){const a=await api.get(`/student/assignments/${t}`);a.sections&&(a.sections=a.sections.map(o=>({...o,question_count:Array.isArray(o.questions_data)?o.questions_data.length:0}))),_compositeExam=a}const n=_compositeExam.sections?.find(a=>a.id===e);if(!n){toast("Kh\xF4ng t\xECm th\u1EA5y ph\u1EA7n thi","error"),navigate(`/composite/${t}`);return}const i=(_compositeExam.sections||[]).length>0&&_compositeExam.sections.every(a=>a.submission_id);if(n.submission_id){if(!i){toast("Ho\xE0n th\xE0nh c\xE1c ph\u1EA7n c\xF2n l\u1EA1i \u0111\u1EC3 xem k\u1EBFt qu\u1EA3 c\u1EE7a ph\u1EA7n n\xE0y.","warning"),navigate(`/composite/${t}`);return}navigate(`/composite-result/${t}/section/${n.submission_id}`);return}if(!_compositeExam.is_active){toast("\u0110\u1EC1 \u0111\xE3 \u0111\xF3ng","error"),navigate(`/composite/${t}`);return}let s=n.time_limit_minutes?n.time_limit_minutes*60:null;if(_compositeExam.mode==="exam"&&n.time_limit_minutes){try{const a=await api.post("/exam-sessions",{ref_type:"composite_section",ref_id:e,assignment_id:t}),o=(Date.now()-new Date(a.started_at).getTime())/1e3;s=Math.max(0,n.time_limit_minutes*60-o)}catch{s=n.time_limit_minutes*60}if(s<=0){await _autoSubmitCompositeSectionAndBack(t,e);return}}_activeSectionId=e,_renderCompositeSectionFullScreen(t,n,s),s!==null&&(_installExamBeforeUnload(),startCompositeSectionTimer(s))}catch(n){toast("L\u1ED7i: "+(n.error||n.message),"error"),navigate(`/composite/${t}`)}}window.showCompositeSectionExam=showCompositeSectionExam;function _renderCompositeSectionFullScreen(t,e,n){const i=e.id,s=`${t}_${i}`;_activeAssignmentId=s;const a=loadDraft(s,"flags");_flaggedSet=new Set(Array.isArray(a?.data)?a.data:[]);const o=e.question_count||0,l=n!==null?'<div class="assign-countdown-wrap"><span class="assign-countdown-label">\u23F1 C\xF2n l\u1EA1i</span><span class="assign-countdown" id="assign-countdown">--:--</span></div>':"";let d="",c=null;if(e.skill==="reading"){let p="";for(let r=1;r<=o;r++){const v=_flaggedSet.has(r)?" flagged":"";p+=`<div class="answer-row">
        ...
        <button class="q-flag-btn${v}" data-flag-q="${r}" onclick="toggleFlag(${r})" title="\u0110\xE1nh d\u1EA5u xem l\u1EA1i">\u{1F6A9}</button>
      </div>`}d=`
      <div class="assignment-content">
        <div class="content-pane" id="reading-content-pane">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:16px">
      ...
      </div>`,c=()=>{restoreAnswerDraft(s,o),bindReadingTextInteractions(),updateNavigatorState(),startAutoSave(()=>persistAnswerDraft(s,o))}}else if(e.skill==="listening"){let p="";for(let v=1;v<=o;v++){const u=_flaggedSet.has(v)?" flagged":"";p+=`<div class="answer-row">
        <span class="q-label">Q${v}</span>
        <input class="answer-input" id="ans-${v}" type="text" placeholder="\u0110\xE1p \xE1n c\xE2u ${v}"
          oninput="updateNavigatorState();scheduleAnswerDraftSave('${s}',${o})" />
        <button class="q-flag-btn${u}" data-flag-q="${v}" onclick="toggleFlag(${v})" title="\u0110\xE1nh d\u1EA5u xem l\u1EA1i">\u{1F6A9}</button>
      </div>`}d=`
      <div class="assignment-content">
        <div class="content-pane">
    ...
    <div class="assignment-toolbar" style="position:sticky;top:0;z-index:10;border-bottom:1px solid var(--border);border-top:2px solid var(--border)">
    ...
    </div>`;let d="",c=null;if(e.skill==="reading"){let p="";for(let r=1;r<=a;r++){const v=_flaggedSet.has(r)?" flagged":"";p+=`<div class="answer-row">
        <span class="q-label">Q${r}</span>
        <input class="answer-input" id="ans-${r}" type="text" placeholder="\u0110\xE1p \xE1n c\xE2u ${r}"
      ...
      </div>`,c=()=>{restoreAnswerDraft(t,a),bindReadingTextInteractions(),updateNavigatorState(),startAutoSave(()=>persistAnswerDraft(t,a))}}else if(e.skill==="listening"){let p="";for(let v=1;v<=a;v++){const u=_flaggedSet.has(v)?" flagged":"";p+=`<div class="answer-row">
        <span class="q-label">Q${v}</span>
        <input class="answer-input" id="ans-${v}" type="text" placeholder="\u0110\xE1p \xE1n c\xE2u ${v}"
          oninput="updateNavigatorState();scheduleAnswerDraftSave('${t}',${a})" />
        <button class="q-flag-btn${u}" data-flag-q="${v}" onclick="toggleFlag(${v})" title="\u0110\xE1nh d\u1EA5u xem l\u1EA1i">\u{1F6A9}</button>
    ...
    </div>`,document.body.appendChild(n),window._csecResolve=e})}async function showCompositeResult({id:t}){setLoading("\u0110ang t\u1EA3i k\u1EBFt qu\u1EA3...");try{const e=await api.get(`/student/assignments/${t}`);if(!((e.sections||[]).length>0&&e.sections.every(i=>i.submission_id))){toast("B\u1EA1n c\u1EA7n ho\xE0n th\xE0nh to\xE0n b\u1ED9 \u0111\u1EC1 t\u1ED5ng h\u1EE3p tr\u01B0\u1EDBc khi xem k\u1EBFt qu\u1EA3.","warning"),navigate(`/composite/${t}`);return}renderCompositeResult(e)}catch(e){toast("L\u1ED7i: "+(e.error||e.message),"error"),navigate("/assignments")}}window.showCompositeResult=showCompositeResult;async function showCompositeSectionResult({id:t,submissionId:e}){setLoading("\u0110ang t\u1EA3i k\u1EBFt qu\u1EA3...");try{const n=await api.get(`/student/assignments/${t}`);if(!((n.sections||[]).length>0&&n.sections.every(a=>a.submission_id))){toast("Ho\xE0n th\xE0nh to\xE0n b\u1ED9 \u0111\u1EC1 t\u1ED5ng h\u1EE3p tr\u01B0\u1EDBc khi xem k\u1EBFt qu\u1EA3 chi ti\u1EBFt.","warning"),navigate(`/composite/${t}`);return}const s=await api.get(`/student/composite-section-submissions/${e}`);setResultNavContext({backHref:`/composite-result/${t}`,backLabel:"\u2190 K\u1EBFt qu\u1EA3 t\u1ED5ng h\u1EE3p"}),renderResult(s)}catch(n){toast("L\u1ED7i t\u1EA3i k\u1EBFt qu\u1EA3: "+(n.error||n.message),"error"),navigate(`/composite-result/${t}`)}}window.showCompositeSectionResult=showCompositeSectionResult;function renderCompositeResult(t){const e={reading:"\u{1F4D6}",listening:"\u{1F3A7}",writing:"\u270D\uFE0F",speaking:"\u{1F3A4}"},n=t.scoring_scale||"10",i=t.sections.length,s=t.sections.filter(r=>r.submission_id).length,o=t.sections.filter(r=>r.submission_id&&r.score!=null).length,l=Math.max(s-o,0),d=s===i&&l===0?'<span class="badge badge-done">\u2713 \u0110\xE3 ch\u1EA5m xong</span>':l>0?`<span class="badge badge-waiting">\u23F3 ${l} ph\u1EA7n ch\u1EDD ch\u1EA5m</span>`:`<span class="badge badge-pending">${s}/${i} ph\u1EA7n \u0111\xE3 n\u1ED9p</span>`;function c(r){return r.score==null?null:r.skill==="writing"||r.skill==="speaking"||n==="ielts"?`Band ${Number(r.score).toFixed(1)}/9`:`${Number(r.score).toFixed(1)}/10`}const p=t.sections.map(r=>{const v=SKILL_LABELS[r.skill]||r.skill,u=!!r.submission_id;if(!r.submission_id)return`<div class="result-section-card composite-result-card composite-result-card-locked">
        <div class="composite-result-card-top">
          <div class="composite-result-card-title-wrap">
            ...
            <div>
              ...
              <div class="composite-result-card-subtitle">${escapeHtml(v)} \xB7 Ch\u01B0a n\u1ED9p ph\u1EA7n n\xE0y</div>
            </div>
          </div>
      ...
      </div>`;const m=c(r),h=m?`<div class="result-score-badge">${m}</div>`:'<span class="badge badge-waiting">\u23F3 Ch\u1EDD ch\u1EA5m</span>';let g="";return r.skill==="writing"&&r.feedback?g=r.feedback:r.skill==="speaking"?g=r.feedback||(r.audio_url?"\u0110\xE3 n\u1ED9p file ghi \xE2m. B\u1EA5m \u0111\u1EC3 xem transcript v\xE0 nh\u1EADn x\xE9t chi ti\u1EBFt.":""):g=r.score!=null?"M\u1EDF \u0111\u1EC3 xem \u0111\xE1p \xE1n \u0111\xFAng/sai, Explain, Locate v\xE0 luy\u1EC7n l\u1EA1i ph\u1EA7n n\xE0y.":"Ph\u1EA7n n\xE0y \u0111\xE3 n\u1ED9p v\xE0 \u0111ang ch\u1EDD ch\u1EA5m \u0111i\u1EC3m.",`<a href="#/composite-result/${t.id}/section/${r.submission_id}" class="result-section-card composite-result-card composite-result-card-link">
      <div class="composite-result-card-top">
        <div class="composite-result-card-title-wrap">
          <div class="composite-result-card-icon">${e[r.skill]||"\u{1F4DD}"}</div>
          <div>
            <div class="composite-result-card-title">${escapeHtml(r.label)}</div>
            <div class="composite-result-card-subtitle">${escapeHtml(v)} \xB7 ${u?"\u0110\xE3 n\u1ED9p b\xE0i":"Ch\u01B0a n\u1ED9p"}</div>
          </div>
        </div>
      ...
      <div class="composite-result-hero">
        ...
        <div class="composite-result-hero-main">
          ...
          <div class="composite-result-skill-scores">
            ${t.sections.map(r=>{const v=c(r);return`<div class="composite-skill-score-chip">
                ...
                <div class="composite-skill-score-info">
                  ...
                  <span class="composite-skill-score-val">${v??(r.submission_id?"\u23F3":"\u2014")}</span>
                </div>
              </div>`}).join("")}
          </div>
        </div>
