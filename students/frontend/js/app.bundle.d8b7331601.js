...
`),n=[];let i=null;const s=()=>{i&&(n.push(`</${i}>`),i=null)};for(const a of e){const o=a.trim();if(!o){s();continue}const l=o.match(/^[-*]\s+(.+)$/),d=o.match(/^\d+\.\s+(.+)$/);if(l||d){const p=l?"ul":"ol";i!==p&&(s(),n.push(`<${p}>`),i=p),n.push(`<li>${renderMarkdownInline((l||d)[1])}</li>`);continue}s();const c=o.match(/^(#{2,4})\s+(.+)$/);c?n.push(`<h5>${renderMarkdownInline(c[2])}</h5>`):n.push(`<p>${renderMarkdownInline(o)}</p>`)}return s(),n.join("")}function btnReset(t){t&&(t.disabled=!1,t.innerHTML=t._origHTML||t.innerHTML)}function toast(t,e="success"){const n=e==="error"?6e3:3500,i=document.createElement("div");i.className=`toast toast-${e}`,i.setAttribute("role","alert");const s=document.createElement("span");s.textContent=t;const a=document.createElement("button");a.className="toast-close",a.setAttribute("aria-label","\u0110\xF3ng th\xF4ng b\xE1o"),a.textContent="\xD7",a.onclick=()=>i.remove(),i.appendChild(s),i.appendChild(a),i.addEventListener("mouseenter",()=>i.classList.add("toast-paused")),i.addEventListener("mouseleave",()=>i.classList.remove("toast-paused")),$("#toast-container").appendChild(i);const o=setTimeout(()=>i.remove(),n);i.addEventListener("mouseenter",()=>clearTimeout(o)),i.addEventListener("mouseleave",()=>setTimeout(()=>i.remove(),1e3))}function setLoading(t="\u0110ang t\u1EA3i..."){$("#app").innerHTML=`
  ...
  `);const t=$("#login-username")?.value?.trim();if(t){const e=$("#forgot-password-username");e&&(e.value=t)}setTimeout(()=>$("#forgot-password-username")?.focus(),50)}async function submitForgotPassword(t){const e=$("#forgot-password-username")?.value?.trim()||$("#login-username")?.value?.trim()||"";if(!e){toast("Vui l\xF2ng nh\u1EADp username \u0111\u1EC3 g\u1EEDi link \u0111\u1EB7t l\u1EA1i m\u1EADt kh\u1EA9u.","error");return}btnLoading(t);try{const n=await api.post("/auth/forgot-password",{username:e});closeModal(),toast(n.message||"Link \u0111\u1EB7t l\u1EA1i m\u1EADt kh\u1EA9u \u0111\xE3 \u0111\u01B0\u1EE3c g\u1EEDi qua Gmail.","success")}catch(n){btnReset(t),toast(n.error||"Kh\xF4ng th\u1EC3 g\u1EEDi link \u0111\u1EB7t l\u1EA1i l\xFAc n\xE0y.","error")}}function showResetPasswordPage(t){$("#app").classList.remove("with-header"),$("#app-header").classList.add("hidden"),$("#app").innerHTML=`
    ...
    </div>`}function buildProgressChartSvg(t,e,n,i,s="profile"){const a=s==="profile"?520:320,o=s==="profile"?220:150,l=s==="profile"?42:30,d=14,c=14,p=s==="profile"?28:24,r=a-l-d,m=o-c-p,u="#dbe4ef",g="#94a3b8",h={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"}[i]||"#0f766e",v=new Date;v.setHours(23,59,59,999);const y=new Date(v);y.setDate(v.getDate()-(n-1)),y.setHours(0,0,0,0);const _=Math.max(1,v.getTime()-y.getTime()),S=w=>l+(w.getTime()-y.getTime())/_*r,b=w=>c+(9-w)/9*m,C=[0,3,6,9].map(w=>{const k=b(w).toFixed(1);return`
  ...
  `:"",M=t.map(w=>`${S(w.date).toFixed(1)},${b(w.avgScore).toFixed(1)}`).join(" "),A=t.length>=2?`<polyline fill="none" stroke="${h}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" points="${M}"/>`:"",H=t.map(w=>{const k=S(w.date).toFixed(1),E=b(w.avgScore).toFixed(1);return`
      <circle cx="${k}" cy="${E}" r="5.5" fill="#fff" stroke="${h}" stroke-width="3"
        ...
        onmouseleave="hideProgressTooltip()"></circle>`}).join(""),B=toDateKey(y).slice(5).replace("-","/"),f=toDateKey(v).slice(5).replace("-","/");return`
    <svg viewBox="0 0 ${a} ${o}" preserveAspectRatio="none" class="progress-chart-svg" aria-hidden="true">
      ...
      <text x="${a-d}" y="${o-6}" text-anchor="end" font-size="10" fill="${g}">${f}</text>
    ...
    ${n.map(s=>`
      ...
      </a>`).join("")}`}}function profileTargetKey(){return`ielts_target:${_student?.id}`}function getTargetBand(){return parseFloat(localStorage.getItem(profileTargetKey()))||6.5}function setTargetBand(t){localStorage.setItem(profileTargetKey(),String(t))}async function showProfile(){setLoading("\u0110ang t\u1EA3i h\u1ED3 s\u01A1...");try{const[t,e,n]=await Promise.all([getAssignments(),api.get("/student/profile-answers").catch(()=>({fields:[],answers:{}})),api.get("/student/vocab/sessions").catch(()=>[]),loadMyVocab()]);window._cachedProfileData=e,window._cachedVocabSessions=n,updateStudentState({email:getStudentNotificationEmail(e)||null}),renderProfile(t,e)}catch(t){toast("L\u1ED7i: "+(t.error||t.message),"error"),renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c h\u1ED3 s\u01A1",t,"/profile")}}function renderProfile(t){const e=calculateSubmissionStreak(t,window._cachedVocabSessions||[]),n=getTargetBand(),i=getSubmittedAssignments(t),a=t.filter(f=>f.submission_id&&f.overall_score!=null).map(f=>Number(f.overall_score)).filter(Boolean),o=a.length?(a.reduce((f,w)=>f+w,0)/a.length).toFixed(1):"\u2014",l=["reading","listening","writing","speaking"],d=getIeltsGradedAssignments(t),c=l.map(f=>{const w=d.filter(T=>T.skill===f).sort((T,N)=>new Date(T.created_at)-new Date(N.created_at)),k=w.map(T=>Number(T.overall_score)).filter(T=>T>0),E=k.length?k.reduce((T,N)=>T+N,0)/k.length:null,I=k.length?Math.max(...k):null;return{sk:f,count:w.length,avg:E,best:I,timeline:k}}),p=new Set(i.map(f=>toDateKey(f.submitted_at||f.created_at)).filter(Boolean)),r=new Set(e.days),m=[];for(let f=6;f>=0;f--){const w=new Date;w.setDate(w.getDate()-f);const k=toDateKey(w),E=w.toLocaleDateString("vi-VN",{weekday:"short"});m.push(`<div class="home-streak-day ${r.has(k)?"on":""}" title="${k}">
      <div class="streak-dot">${r.has(k)?"\u{1F525}":"\xB7"}</div>
      <div class="streak-day-label">${E}</div>
    </div>`)}const u=(_myVocabCache||[]).length,g=[5,5.5,6,6.5,7,7.5,8,8.5,9].map(f=>`<option value="${f}" ${n===f?"selected":""}>${f}</option>`).join(""),h=(_student.full_name||"?").split(" ").map(f=>f[0]).join("").slice(0,2).toUpperCase(),v=e.current>=30?"#dc2626":e.current>=7?"#f59e0b":"#6b7280",y={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"},_={reading:"\u{1F4D6}",listening:"\u{1F3A7}",writing:"\u270D\uFE0F",speaking:"\u{1F3A4}"},S={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking"},b=f=>i.filter(w=>w.skill===f).sort((w,k)=>new Date(k.submitted_at||k.created_at)-new Date(w.submitted_at||w.created_at));function x(f,w){if(f.length<2)return"";const k=100,E=28,I=f.map((P,D)=>{const F=D/(f.length-1)*k,q=E-P/9*E;return`${F.toFixed(1)},${q.toFixed(1)}`}).join(" "),T=k,N=E-f[f.length-1]/9*E;return`<svg viewBox="0 0 ${k} ${E}" preserveAspectRatio="none" style="width:100%;height:28px;display:block">
    ...
    </svg>`}function C({sk:f,count:w,avg:k,best:E,timeline:I}){const T=y[f],N=k!==null?Math.min(100,Math.round(k/n*100)):0,P=k===null?"":k>=n?"\u2705":k>=n-.5?"\u{1F7E1}":"\u{1F534}",D=x(I,T),F=k!==null?k>=n?`<span class="spc-vs ok">+${(k-n).toFixed(1)} vs target</span>`:`<span class="spc-vs gap">-${(n-k).toFixed(1)} vs target</span>`:"",q=b(f).length;return`
      <button class="skill-progress-card spc-${f}" type="button"
        onclick="openSkillProgressModal('${f}')">
        <div class="spc-head">
          <span class="spc-icon">${_[f]}</span>
      ...
      </button>`}const L=28,M=new Date;M.setDate(M.getDate()-(L-1));const A=(M.getDay()+6)%7,H=Array(A).fill('<div class="act-cell act-pad"></div>');for(let f=L-1;f>=0;f--){const w=new Date;w.setDate(w.getDate()-f);const k=w.toISOString().slice(0,10);H.push(`<div class="act-cell ${p.has(k)?"act-done":"act-none"}" title="${k}"></div>`)}const B=[];c.forEach(({sk:f,best:w})=>{w!==null&&B.push(`<div class="ms-card"><div class="ms-icon">${_[f]}</div><div class="ms-val">${w}</div><div class="ms-label">Best ${S[f]}</div></div>`)}),i.length>0&&B.push(`<div class="ms-card"><div class="ms-icon">\u{1F4DD}</div><div class="ms-val">${i.length}</div><div class="ms-label">B\xE0i \u0111\xE3 n\u1ED9p</div></div>`),e.best>1&&B.push(`<div class="ms-card"><div class="ms-icon">\u{1F525}</div><div class="ms-val">${e.best}</div><div class="ms-label">Streak k\u1EF7 l\u1EE5c</div></div>`),u>0&&B.push(`<div class="ms-card"><div class="ms-icon">\u{1F4D6}</div><div class="ms-val">${u}</div><div class="ms-label">T\u1EEB \u0111\xE3 l\u01B0u</div></div>`),$("#app").innerHTML=`
    ...
    ${n.map(s=>`
      <a href="#/${s.submission_id?"result":"assignment"}/${s.id}" class="cal-event-item">
        ...
        <span class="cal-event-title">${escapeHtml(s.title)}</span>
        <span class="cal-event-status">${s.submission_id?"\u2705 \u0110\xE3 n\u1ED9p":"\u23F3 H\u1EA1n"}</span>
      </a>`).join("")}`}},renderProfile=function(t,e){e=e||window._cachedProfileData||{fields:[],answers:{}};const n=(e.fields||[]).find(b=>b.field_key==="notification_email"),i=(e.fields||[]).filter(b=>b.field_key!=="notification_email"),s=_student&&typeof _student.email=="string"&&_student.email.trim()||e.student&&typeof e.student.email=="string"&&e.student.email.trim()||(n?String(e.answers?.[n.id]||"").trim():""),a=calculateSubmissionStreak(t,window._cachedVocabSessions||[]),o=getTargetSettings(),l=getSubmittedAssignments(t),d=calculateOverallAverage(t),c=d!==null?d.toFixed(1):"\u2014",p=SKILL_ORDER.map(b=>{const x=getSkillGradedAssignments(t,b).map(C=>Number(C.overall_score)).filter(Number.isFinite);return{sk:b,best:x.length?Math.max(...x):null}}),r=new Set(l.map(b=>toDateKey(b.submitted_at||b.created_at)).filter(Boolean)),m=new Set(a.days),u=[];for(let b=6;b>=0;b--){const x=new Date;x.setDate(x.getDate()-b);const C=toDateKey(x),L=x.toLocaleDateString("vi-VN",{weekday:"short"});u.push(`<div class="home-streak-day ${m.has(C)?"on":""}" title="${C}">
      <div class="streak-dot">${m.has(C)?"\u{1F525}":"\xB7"}</div>
      <div class="streak-day-label">${L}</div>
    </div>`)}const g=(_myVocabCache||[]).length,h=(_student.full_name||"?").split(" ").map(b=>b[0]).join("").slice(0,2).toUpperCase(),v=a.current>=30?"#dc2626":a.current>=7?"#f59e0b":"#6b7280",y={reading:"#0f766e",listening:"#7c3aed",writing:"#d97706",speaking:"#dc2626"},_=[];p.forEach(({sk:b,best:x})=>{x!==null&&_.push(`<div class="ms-card"><div class="ms-icon">${SKILL_ICONS[b]}</div><div class="ms-val">${x}</div><div class="ms-label">Best ${SKILL_LABELS[b]}</div></div>`)}),l.length>0&&_.push(`<div class="ms-card"><div class="ms-icon">\u{1F4DD}</div><div class="ms-val">${l.length}</div><div class="ms-label">B\xE0i \u0111\xE3 n\u1ED9p</div></div>`),a.best>1&&_.push(`<div class="ms-card"><div class="ms-icon">\u{1F525}</div><div class="ms-val">${a.best}</div><div class="ms-label">Streak k\u1EF7 l\u1EE5c</div></div>`),g>0&&_.push(`<div class="ms-card"><div class="ms-icon">\u{1F4D6}</div><div class="ms-val">${g}</div><div class="ms-label">T\u1EEB \u0111\xE3 l\u01B0u</div></div>`);const S=[["profile-info","\u{1F4CB} Th\xF4ng tin"],["profile-streak","\u{1F525} Streak"],["profile-targets","\u{1F3AF} Target"],["profile-progress","\u{1F4C8} Ti\u1EBFn \u0111\u1ED9"],..._.length>0?[["profile-achievements","\u{1F3C6} Th\xE0nh t\xEDch"]]:[],["profile-next-steps","\u{1F680} Ti\u1EBFp t\u1EE5c h\u1ECDc"]];$("#app").innerHTML=`
    <div class="container profile-page">
      <div class="profile-hero">
        <div class="profile-avatar" style="background:${y.reading}">${h}</div>
        <div class="profile-hero-info">
      ...
      <div class="profile-nav">
        <div class="profile-nav-list">
          ${S.map(([b,x])=>`
            <button type="button" class="profile-nav-btn" onclick="scrollProfileSection('${b}')">${x}</button>
          `).join("")}
        </div>
      </div>
        ...
        ${i.length===0?"":`<details class="pi-details">
              ...
              <div class="pi-details-body">
                <div class="pi-fields" id="pi-fields">
                  ${i.map(b=>{const x=e.answers[b.id]||"",C=`pi-field-${b.id}`;let L;if(b.field_type==="textarea")L=`<textarea id="${C}" class="form-input pi-input" data-field-id="${b.id}" rows="3" placeholder="Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u">${escapeHtml(x)}</textarea>`;else if(b.field_type==="select"&&Array.isArray(b.options)){const M=b.options.map(A=>`<option value="${escapeHtml(String(A))}" ${x===String(A)?"selected":""}>${escapeHtml(String(A))}</option>`).join("");L=`<select id="${C}" class="form-input pi-input" data-field-id="${b.id}"><option value="">-- Ch\u01B0a ch\u1ECDn --</option>${M}</select>`}else if(b.field_type==="date")L=`<input id="${C}" class="form-input pi-input" type="date" data-field-id="${b.id}" value="${escapeHtml(x)}" />`;else{const M=b.field_key==="notification_email"?"email":"text",A=b.field_key==="notification_email"?"name@example.com":"Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u";L=`<input id="${C}" class="form-input pi-input" type="${M}" data-field-id="${b.id}" value="${escapeHtml(x)}" placeholder="${A}" />`}return`<div class="pi-field">
    ...
    <div class="container mfc-page">
      <div class="mfc-end">
        ...
        </div>`;const _=!p&&c.rewrite_status==="requested",S=p?c.is_active&&!isCompositeAssignmentDone(c):!c.submission_id&&c.is_active,b=p?`#/composite/${c.id}`:m?`#/result/${c.id}`:`#/assignment/${c.id}`;return`
      <a class="assignment-card ${_?"rewrite":S?"pending-card":m?"done":""}" href="${b}">
        <div class="assignment-card-icon">${g}</div>
        <div class="assignment-card-body">
          <div class="assignment-card-title">${escapeHtml(c.title)}</div>
          <div class="assignment-card-meta">
    ...
    <div class="audio-submit-processing">
      ...
      <tr>
        ...
        ${v}
      </tr>${y}`}).join(""),l=e.length,d=(t.scoring_scale||"10")==="ielts",c=t.overall_score??(l>0?d?Math.round(i/l*9*10)/10:Math.round(i/l*10*10)/10:0),p=s?5:4,r=t.vocabulary||[],m=r.length===0?"":`
    ...
    </div>`}function renderWritingFeedback(t,e=null){const n=t.teacher_feedback||{},i=(n.annotations||[]).sort((v,y)=>v.start-y.start),s=n.overall||"",a=t.overall_score??n.score,o=countWords(t.writing_content||""),l=e?[...e].sort((v,y)=>(y.attempt_number||1)-(v.attempt_number||1)):null,c=(l?l[0]:t).rewrite_status==="requested",p=e&&e.length>1,r=s?`
    <div class="section-label">Nh\u1EADn x\xE9t t\u1ED5ng th\u1EC3</div>
    <div class="feedback-overall">${escapeHtml(s)}</div>`:"",m=_annColorMap(i),u=_buildAnnSidebar(i,m),g=p?`
    <div class="version-selector">
      <span class="version-selector-label">Xem k\u1EBFt qu\u1EA3:</span>
      ${[...e].sort((v,y)=>(v.attempt_number||1)-(y.attempt_number||1)).map(v=>`
        <button class="version-btn${v.id===t.id?" active":""}"
          onclick="switchWritingVersion('${v.id}','${t.assignment_id}')">
