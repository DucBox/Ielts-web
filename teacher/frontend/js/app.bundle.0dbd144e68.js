...
`),n=[];let i=null;const s=()=>{i&&(n.push(`</${i}>`),i=null)};for(const o of e){const a=o.trim();if(!a){s();continue}const l=a.match(/^[-*]\s+(.+)$/),r=a.match(/^\d+\.\s+(.+)$/);if(l||r){const d=l?"ul":"ol";i!==d&&(s(),n.push(`<${d}>`),i=d),n.push(`<li>${renderMarkdownInline((l||r)[1])}</li>`);continue}s();const c=a.match(/^(#{2,4})\s+(.+)$/);c?n.push(`<h5>${renderMarkdownInline(c[2])}</h5>`):n.push(`<p>${renderMarkdownInline(a)}</p>`)}return s(),n.join("")}function btnReset(t){t&&(t.disabled=!1,t.innerHTML=t._origHTML||t.innerHTML)}function toast(t,e="success"){const n=e==="error"?6e3:3500,i=document.createElement("div");i.className=`toast toast-${e}`,i.setAttribute("role","alert");const s=document.createElement("span");s.textContent=t;const o=document.createElement("button");o.className="toast-close",o.setAttribute("aria-label","\u0110\xF3ng th\xF4ng b\xE1o"),o.textContent="\xD7",o.onclick=()=>i.remove(),i.appendChild(s),i.appendChild(o),i.addEventListener("mouseenter",()=>i.classList.add("toast-paused")),i.addEventListener("mouseleave",()=>i.classList.remove("toast-paused")),$("#toast-container").appendChild(i);const a=setTimeout(()=>i.remove(),n);i.addEventListener("mouseenter",()=>clearTimeout(a)),i.addEventListener("mouseleave",()=>setTimeout(()=>i.remove(),1e3))}function setLoading(t="\u0110ang t\u1EA3i..."){$("#app").innerHTML=`
    ...
    `);const a=$("#modal-overlay"),l=a?.querySelector('[data-confirm-action="cancel"]'),r=a?.querySelector('[data-confirm-action="confirm"]');let c=!1;const d=u=>{c||(c=!0,closeModal(),o(u))};l.onclick=()=>d(!1),r.onclick=()=>d(!0),a.onclick=u=>{u.target===a&&d(!1)}})}let _oneTimeStudentCredentials=null;function closeModal(t){const e=$("#modal-overlay");t&&t.target!==e||(_oneTimeStudentCredentials=null,e&&(e.onclick=n=>closeModal(n)),e.classList.add("hidden"),$("#modal-body").innerHTML="",_modalPreviousFocus&&(_modalPreviousFocus.focus(),_modalPreviousFocus=null))}document.addEventListener("keydown",t=>{const e=$("#modal-overlay");if(!e||e.classList.contains("hidden"))return;const n=e.querySelector(".modal")||e;if(t.key==="Escape"){closeModal();return}if(t.key==="Tab"){const i=[...n.querySelectorAll(FOCUSABLE)];if(!i.length)return;const s=i[0],o=i[i.length-1];(t.shiftKey?document.activeElement===s:document.activeElement===o)&&(t.preventDefault(),(t.shiftKey?o:s).focus())}});function csvEscape(t){return`"${String(t??"").replace(/"/g,'""')}"`}function downloadCsvFile(t,e,n){const i=[e,...n].map(l=>l.map(csvEscape).join(",")).join(`
`),s=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=t,a.click(),URL.revokeObjectURL(o)}function buildStudentCredentialsFilename(t="student_accounts"){const e=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");return`${t}_${e}.csv`}function openStudentCredentialsModal(t,e,n="student_accounts"){const i=(Array.isArray(e)?e:[]).map(s=>({full_name:String(s?.full_name||"").trim(),username:String(s?.username||"").trim(),password:String(s?.password||"")})).filter(s=>s.full_name&&s.username&&s.password);_oneTimeStudentCredentials=i.length>0?{rows:i,fileName:buildStudentCredentialsFilename(n)}:null,openModal(t,`
    ...
    </div>`}window.showHistogramStudents=showHistogramStudents;function renderStatsTab(t,e){destroyStatsCharts();const{timeline:n,per_student:i,per_assignment:s}=e,o=_statsSkillFilter,a=_statsStatusFilter,l=_statsModeFilter,r=_statsScaleFilter,c=s.filter(p=>!(o&&p.skill!==o||a==="active"&&!p.is_active||a==="closed"&&p.is_active||l&&p.mode!==l||r&&(p.scoring_scale||"10")!==r)),d=new Set(c.map(p=>p.id)),u=p=>p.length?p.reduce((f,b)=>f+b,0)/p.length:null,m=i.length,g=c.length,h=c.filter(p=>p.is_active).length,v=g-h,T=new Map;i.forEach(p=>{p.submissions.forEach(f=>{if(!d.has(f.assignment_id))return;const b=`${p.id||p.student_id}:${f.assignment_id}`,y=T.get(b);(!y||(f.attempt_number||1)>(y.attempt_number||1))&&T.set(b,{...f,student_name:p.name})})});const w=Array.from(T.values()),L=w.filter(p=>p.overall_score!==null);_statsAllScoredSubs=L;const E=u(L.map(p=>Number(p.overall_score))),I=g*m,N=I>0?Math.round(w.length/I*100):0,H=[0,0,0,0,0];for(const p of L){const f=Number(p.overall_score),b=f>=9?4:Math.min(4,Math.floor(f/2));H[b]++}const F=["reading","listening","writing","speaking"],j={},O={};for(const p of F){const f=L.filter(x=>x.skill===p);j[p]=u(f.map(x=>Number(x.overall_score)));const b=c.filter(x=>x.skill===p),y=w.filter(x=>x.skill===p),M=b.length*m;O[p]={count:b.length,submitted:y.length,pct:M>0?Math.round(y.length/M*100):0}}let P=i.map(p=>{const f=p.submissions.filter(S=>d.has(S.assignment_id)),b=new Map;f.forEach(S=>{const k=b.get(S.assignment_id);(!k||(S.attempt_number||1)>(k.attempt_number||1))&&b.set(S.assignment_id,S)});const y=Array.from(b.values()),M=y.filter(S=>S.overall_score!==null),x=S=>u(M.filter(k=>k.skill===S).map(k=>Number(k.overall_score))),_=y.filter(S=>!S.is_active&&S.deadline),C=_.filter(S=>S.on_time).length;return{...p,submitted:y.length,total:g,avg_score:o?x(o):u(M.map(S=>Number(S.overall_score))),avg_reading:x("reading"),avg_listening:x("listening"),avg_writing:x("writing"),avg_speaking:x("speaking"),on_time:C,closed_total:_.length,on_time_rate:_.length>0?C/_.length:null}});_statsSortCol&&(P=[...P].sort((p,f)=>{let b=p[_statsSortCol],y=f[_statsSortCol];return b==null?1:y==null?-1:typeof b=="string"?_statsSortDir==="asc"?b.localeCompare(y):y.localeCompare(b):_statsSortDir==="asc"?b-y:y-b}));const q=p=>p!=null?Number(p).toFixed(1):"\u2014",W=(p,f)=>f>0?Math.round(p/f*100):0,B=p=>_statsSortCol!==p?'<span class="sort-icon">\u2195</span>':`<span class="sort-icon active">${_statsSortDir==="asc"?"\u2191":"\u2193"}</span>`,Q={reading:"#3b82f6",listening:"#f59e0b",writing:"#8b5cf6",speaking:"#22c55e"},z=["#3b82f6","#ef4444","#22c55e","#f59e0b","#8b5cf6","#06b6d4","#ec4899","#14b8a6","#f97316","#a3e635"],X={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking",composite:"T\u1ED5ng h\u1EE3p"},Y=`
    <div class="stats-cards-grid">
      ...
      <div class="stats-card">
        ...
        <div class="stats-card-body">
          ...
          <div class="stats-card-label">${h} \u0111ang m\u1EDF \xB7 ${v} \u0111\xE3 \u0111\xF3ng</div>
        </div>
      </div>
      <div class="stats-card">
    ...
    </div>`,K=c.filter(p=>!p.is_active&&p.deadline),R=_closedAssignsSearch?K.filter(p=>p.title.toLowerCase().includes(_closedAssignsSearch)):K,U=3,D=_closedAssignsExpanded?R:R.slice(0,U),it=R.length-D.length,st=K.length===0?"":`
    <div class="stats-section-card">
    ...
    <div class="stats-section-card">
      ...
      ${P.length===0?'<div class="empty-state" style="padding:20px">Kh\xF4ng c\xF3 d\u1EEF li\u1EC7u</div>':`<div class="table-wrap">
          <table class="stats-table">
            ...
            <tbody>
              ${P.map(p=>{const f=W(p.submitted,p.total),b=p.on_time_rate!==null?Math.round(p.on_time_rate*100):null;return`<tr>
                    ...
                    <div class="stats-expand-body">
                      <table class="stats-sub-table">
                            <thead id="stats-sub-thead-${p.id}"><tr>
                              <th class="sortable" onclick="sortStatsSubTable('${p.id}','title')">B\xE0i t\u1EADp ${makeSortIcon("title",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${p.id}','skill')">K\u1EF9 n\u0103ng ${makeSortIcon("skill",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${p.id}','score')">\u0110i\u1EC3m ${makeSortIcon("score",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${p.id}','submitted_at')">Ng\xE0y n\u1ED9p ${makeSortIcon("submitted_at",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${p.id}','on_time')">\u0110\xFAng h\u1EA1n ${makeSortIcon("on_time",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th>Overtime</th>
                            </tr></thead>
                            <tbody id="stats-sub-tbody-${p.id}">${buildStatsSubRows(p.submissions.filter(y=>d.has(y.assignment_id)))}</tbody>
                          </table>
                    </div>
                  </td>
                </tr>`}).join("")}
    ...
    </div>`,A=p=>makeSortIcon(p,_assignTableSortCol,_assignTableSortDir),G=[...c];_assignTableSortCol&&G.sort((p,f)=>{let b,y;switch(_assignTableSortCol){case"title":b=p.title.toLowerCase(),y=f.title.toLowerCase();break;case"skill":b=p.skill||"",y=f.skill||"";break;case"mode":b=p.mode||"",y=f.mode||"";break;case"submitted_rate":b=p.total?p.submitted/p.total:0,y=f.total?f.submitted/f.total:0;break;case"avg_score":b=p.avg_score??-1,y=f.avg_score??-1;break;case"on_time":b=p.on_time??-1,y=f.on_time??-1;break;case"late":b=p.late??-1,y=f.late??-1;break;case"missing":b=p.missing??-1,y=f.missing??-1;break;case"is_active":b=p.is_active?1:0,y=f.is_active?1:0;break;default:return 0}return b<y?_assignTableSortDir==="asc"?-1:1:b>y?_assignTableSortDir==="asc"?1:-1:0});const ct=`
    <div class="stats-section-card">
