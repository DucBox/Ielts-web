...
`),s=new Blob(["\uFEFF"+o],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(s),r=document.createElement("a");r.href=a,r.download=`${t.title.replace(/[^a-zA-Z0-9_\-]/g,"_")}_diem.csv`,r.click(),URL.revokeObjectURL(a)}window.exportSubmissionsCSV=exportSubmissionsCSV;function buildSubmissionRows(t,e){return t.length===0?`<tr><td colspan="5">
    ...
    </div>`}function renderSubmissionModal(t,e,n=null,i=null){const o=renderAttemptSwitcher(n,i,e),s="";if(e==="reading"||e==="listening"){const a={};(t.questions_data||[]).forEach(d=>{a[d.q_no]=d.answers||[]});const r=(t.student_answers||[]).map(d=>{const u=a[d.q_no]||[],m=u.some(h=>h.toLowerCase().trim()===(d.answer||"").toLowerCase().trim());return`
        <tr>
          ...
          <td style="text-align:center;font-size:16px">${m?"\u2705":"\u274C"}</td>
        </tr>`}).join(""),l=(t.student_answers||[]).filter(d=>(a[d.q_no]||[]).some(m=>m.toLowerCase().trim()===(d.answer||"").toLowerCase().trim())).length,c=(t.questions_data||[]).length;$("#modal-title").textContent=`B\xE0i l\xE0m \u2014 ${skillBadge(e).replace(/<[^>]+>/g,"")}`,$("#modal-body").innerHTML=`
      ${o}
          ...
          ${u.map((h,g)=>`
            <div style="${m?"margin-bottom:10px":""}">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
                ...
                <button type="button" class="btn-icon" title="T\u1EA3i xu\u1ED1ng" aria-label="T\u1EA3i xu\u1ED1ng" onclick="downloadAudioFile('${escapeJsAttr(h.url||"")}', '${escapeJsAttr((t.student_name||"speaking")+"_"+(h.name||"phan"+(g+1)))}', this)">\u{1F4E5}</button>
              </div>
              ${g===0?'<div id="waveform-container" class="waveform-container"><div class="waveform-loading">\u0110ang t\u1EA3i waveform...</div></div>':""}
              ${audioPlayerBlock(`<audio ${g===0?'id="waveform-audio"':`id="track-audio-${g}"`} controls src="${escapeHtml(h.url||"")}" preload="metadata" style="width:100%;height:36px;outline:none;${g===0?"margin-top:6px":""}"></audio>`)}
            </div>`).join("")}
        </div>`)}$("#app").innerHTML=`
    <nav class="breadcrumb">
      <a class="breadcrumb-item" onclick="navigate('/classes')">L\u1EDBp h\u1ECDc</a>
