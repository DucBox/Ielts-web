...
`),o=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),s=URL.createObjectURL(o),a=document.createElement("a");a.href=s,a.download=t,a.click(),URL.revokeObjectURL(s)}async function downloadAudioFile(t,e,n){if(!t)return;const i=n?n.innerHTML:"";try{n&&(n.disabled=!0,n.innerHTML="\u23F3");const o=await fetch(t,{cache:"no-store"});if(!o.ok)throw new Error("fetch failed");const s=await o.blob(),a=(t.split("?")[0].match(/\.([a-zA-Z0-9]+)$/)||[])[1],r=(s.type.split("/")[1]||"").split(";")[0],l=a||r||"webm",c=(e||"audio").replace(/\.[a-zA-Z0-9]{2,5}$/,""),d=URL.createObjectURL(s),u=document.createElement("a");u.href=d,u.download=`${c}.${l}`,document.body.appendChild(u),u.click(),u.remove(),URL.revokeObjectURL(d)}catch(o){toast("T\u1EA3i file th\u1EA5t b\u1EA1i: "+(o.message||"l\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"),"error")}finally{n&&(n.disabled=!1,n.innerHTML=i)}}window.downloadAudioFile=downloadAudioFile;const AUDIO_SKIP_SECONDS=10,AUDIO_SPEEDS=[1,1.25,1.5,2,2.5,3];function _audioBlockTrack(t){return t?.closest?.(".audio-block")?.querySelector("audio")||null}function nudgeAudio(t,e){const n=_audioBlockTrack(t);if(!n)return;const i=Number.isFinite(n.duration)?n.duration:1/0;n.currentTime=Math.min(i,Math.max(0,n.currentTime+e))}window.nudgeAudio=nudgeAudio;function setAudioRate(t){const e=_audioBlockTrack(t);e&&(e.playbackRate=Number(t.value)||1)}window.setAudioRate=setAudioRate;function audioPlayerBlock(t){return`<div class="audio-block">
    ...
    </div>`,j=c.filter(p=>!p.is_active&&p.deadline),D=_closedAssignsSearch?j.filter(p=>p.title.toLowerCase().includes(_closedAssignsSearch)):j,W=3,R=_closedAssignsExpanded?D:D.slice(0,W),it=D.length-R.length,ot=j.length===0?"":`
    <div class="stats-section-card">
          ...
          <table class="stats-table">
            ...
            <tbody>
              ${F.map(p=>{const v=U(p.submitted,p.total),y=p.on_time_rate!==null?Math.round(p.on_time_rate*100):null;return`<tr>
                  ...
                  <td><span class="stats-score-badge">${N(p.avg_score)}</span></td>
                  ${s?"":`
                    <td style="color:#3b82f6">${N(p.avg_reading)}</td>
                    <td style="color:#f59e0b">${N(p.avg_listening)}</td>
                    <td style="color:#8b5cf6">${N(p.avg_writing)}</td>
                    <td style="color:#22c55e">${N(p.avg_speaking)}</td>`}
...
QUY T\u1EAEC T\u1EEANG C\u1ED8T:
- STT: s\u1ED1 th\u1EE9 t\u1EF1 c\xE2u h\u1ECFi, gi\u1EEF \u0111\xFAng nh\u01B0 trong \u0111\u1EC1 t\xF4i d\xE1n b\xEAn d\u01B0\u1EDBi.
...
`;function showAiPromptHelper(t){openModal(t==="vocab"?"\u2728 Prompt t\u1EA1o CSV t\u1EEB v\u1EF1ng b\u1EB1ng AI":"\u2728 Prompt t\u1EA1o CSV \u0111\xE1p \xE1n b\u1EB1ng AI",`
    <div style="display:flex;flex-direction:column;gap:12px">
      <div style="font-size:13px;color:var(--text-muted)">${t==="vocab"?"Sao ch\xE9p prompt n\xE0y, d\xE1n v\xE0o ChatGPT/Claude k\xE8m theo b\xE0i \u0111\u1ECDc ho\u1EB7c audio script c\u1EE7a b\u1EA1n, r\u1ED3i t\u1EA3i file CSV m\xE0 AI tr\u1EA3 v\u1EC1 v\xE0 import v\xE0o \u0111\xE2y.":"Sao ch\xE9p prompt n\xE0y, d\xE1n v\xE0o ChatGPT/Claude k\xE8m b\xE0i \u0111\u1ECDc/audio script, \u0111\u1EC1 c\xE2u h\u1ECFi v\xE0 \u0111\xE1p \xE1n (n\u1EBFu c\xF3), r\u1ED3i t\u1EA3i file CSV m\xE0 AI t\u1EA1o ra v\xE0 import v\xE0o \u0111\xE2y."}</div>
      <textarea id="ai-prompt-helper-text" class="form-textarea" rows="20" readonly style="font-family:ui-monospace,monospace;font-size:12px;white-space:pre-wrap">${escapeHtml(t==="vocab"?VOCAB_IMPORT_AI_PROMPT:ANSWER_IMPORT_AI_PROMPT)}</textarea>
      <div class="modal-footer" style="display:flex;justify-content:flex-end;gap:8px">
