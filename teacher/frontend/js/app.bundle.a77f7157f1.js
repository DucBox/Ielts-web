...
`),s=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=t,a.click(),URL.revokeObjectURL(o)}async function downloadAudioFile(t,e,n){if(!t)return;const i=n?n.innerHTML:"";try{n&&(n.disabled=!0,n.innerHTML="\u23F3");const s=await fetch(t,{cache:"no-store"});if(!s.ok)throw new Error("fetch failed");const o=await s.blob(),a=(t.split("?")[0].match(/\.([a-zA-Z0-9]+)$/)||[])[1],r=(o.type.split("/")[1]||"").split(";")[0],l=a||r||"webm",c=(e||"audio").replace(/\.[a-zA-Z0-9]{2,5}$/,""),d=URL.createObjectURL(o),u=document.createElement("a");u.href=d,u.download=`${c}.${l}`,document.body.appendChild(u),u.click(),u.remove(),URL.revokeObjectURL(d)}catch(s){toast("T\u1EA3i file th\u1EA5t b\u1EA1i: "+(s.message||"l\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"),"error")}finally{n&&(n.disabled=!1,n.innerHTML=i)}}window.downloadAudioFile=downloadAudioFile;function buildStudentCredentialsFilename(t="student_accounts"){const e=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");return`${t}_${e}.csv`}function openStudentCredentialsModal(t,e,n="student_accounts"){const i=(Array.isArray(e)?e:[]).map(s=>({full_name:String(s?.full_name||"").trim(),username:String(s?.username||"").trim(),password:String(s?.password||"")})).filter(s=>s.full_name&&s.username&&s.password);_oneTimeStudentCredentials=i.length>0?{rows:i,fileName:buildStudentCredentialsFilename(n)}:null,openModal(t,`
    ...
    </div>`}window.showHistogramStudents=showHistogramStudents;function renderStatsTab(t,e){destroyStatsCharts();const{timeline:n,per_student:i,per_assignment:s}=e,o=_statsSkillFilter,a=_statsStatusFilter,r=_statsModeFilter,l=_statsScaleFilter,c=s.filter(p=>!(o&&p.skill!==o||a==="active"&&!p.is_active||a==="closed"&&p.is_active||r&&p.mode!==r||l&&(p.scoring_scale||"10")!==l)),d=new Set(c.map(p=>p.id)),u=p=>p.length?p.reduce((f,b)=>f+b,0)/p.length:null,m=i.length,h=c.length,g=c.filter(p=>p.is_active).length,v=h-g,k=new Map;i.forEach(p=>{p.submissions.forEach(f=>{if(!d.has(f.assignment_id))return;const b=`${p.id||p.student_id}:${f.assignment_id}`,w=k.get(b);(!w||(f.attempt_number||1)>(w.attempt_number||1))&&k.set(b,{...f,student_name:p.name})})});const y=Array.from(k.values()),L=y.filter(p=>p.overall_score!==null);_statsAllScoredSubs=L;const E=u(L.map(p=>Number(p.overall_score))),I=h*m,H=I>0?Math.round(y.length/I*100):0,q=[0,0,0,0,0];for(const p of L){const f=Number(p.overall_score),b=f>=9?4:Math.min(4,Math.floor(f/2));q[b]++}const F=["reading","listening","writing","speaking"],j={},O={};for(const p of F){const f=L.filter(_=>_.skill===p);j[p]=u(f.map(_=>Number(_.overall_score)));const b=c.filter(_=>_.skill===p),w=y.filter(_=>_.skill===p),B=b.length*m;O[p]={count:b.length,submitted:w.length,pct:B>0?Math.round(w.length/B*100):0}}let P=i.map(p=>{const f=p.submissions.filter(C=>d.has(C.assignment_id)),b=new Map;f.forEach(C=>{const x=b.get(C.assignment_id);(!x||(C.attempt_number||1)>(x.attempt_number||1))&&b.set(C.assignment_id,C)});const w=Array.from(b.values()),B=w.filter(C=>C.overall_score!==null),_=C=>u(B.filter(x=>x.skill===C).map(x=>Number(x.overall_score))),S=w.filter(C=>!C.is_active&&C.deadline),T=S.filter(C=>C.on_time).length;return{...p,submitted:w.length,total:h,avg_score:o?_(o):u(B.map(C=>Number(C.overall_score))),avg_reading:_("reading"),avg_listening:_("listening"),avg_writing:_("writing"),avg_speaking:_("speaking"),on_time:T,closed_total:S.length,on_time_rate:S.length>0?T/S.length:null}});_statsSortCol&&(P=[...P].sort((p,f)=>{let b=p[_statsSortCol],w=f[_statsSortCol];return b==null?1:w==null?-1:typeof b=="string"?_statsSortDir==="asc"?b.localeCompare(w):w.localeCompare(b):_statsSortDir==="asc"?b-w:w-b}));const D=p=>p!=null?Number(p).toFixed(1):"\u2014",U=(p,f)=>f>0?Math.round(p/f*100):0,M=p=>_statsSortCol!==p?'<span class="sort-icon">\u2195</span>':`<span class="sort-icon active">${_statsSortDir==="asc"?"\u2191":"\u2193"}</span>`,V={reading:"#3b82f6",listening:"#f59e0b",writing:"#8b5cf6",speaking:"#22c55e"},z=["#3b82f6","#ef4444","#22c55e","#f59e0b","#8b5cf6","#06b6d4","#ec4899","#14b8a6","#f97316","#a3e635"],W={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking",composite:"T\u1ED5ng h\u1EE3p"},Y=`
    ...
    </div>`,A=p=>makeSortIcon(p,_assignTableSortCol,_assignTableSortDir),G=[...c];_assignTableSortCol&&G.sort((p,f)=>{let b,w;switch(_assignTableSortCol){case"title":b=p.title.toLowerCase(),w=f.title.toLowerCase();break;case"skill":b=p.skill||"",w=f.skill||"";break;case"mode":b=p.mode||"",w=f.mode||"";break;case"submitted_rate":b=p.total?p.submitted/p.total:0,w=f.total?f.submitted/f.total:0;break;case"avg_score":b=p.avg_score??-1,w=f.avg_score??-1;break;case"on_time":b=p.on_time??-1,w=f.on_time??-1;break;case"late":b=p.late??-1,w=f.late??-1;break;case"missing":b=p.missing??-1,w=f.missing??-1;break;case"is_active":b=p.is_active?1:0,w=f.is_active?1:0;break;default:return 0}return b<w?_assignTableSortDir==="asc"?-1:1:b>w?_assignTableSortDir==="asc"?1:-1:0});const ct=`
    <div class="stats-section-card">
