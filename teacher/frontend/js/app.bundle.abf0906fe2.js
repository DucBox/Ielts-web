...
`),n=[];let i=null;const s=()=>{i&&(n.push(`</${i}>`),i=null)};for(const o of e){const a=o.trim();if(!a){s();continue}const r=a.match(/^[-*]\s+(.+)$/),l=a.match(/^\d+\.\s+(.+)$/);if(r||l){const d=r?"ul":"ol";i!==d&&(s(),n.push(`<${d}>`),i=d),n.push(`<li>${renderMarkdownInline((r||l)[1])}</li>`);continue}s();const c=a.match(/^(#{2,4})\s+(.+)$/);c?n.push(`<h5>${renderMarkdownInline(c[2])}</h5>`):n.push(`<p>${renderMarkdownInline(a)}</p>`)}return s(),n.join("")}function btnReset(t){t&&(t.disabled=!1,t.innerHTML=t._origHTML||t.innerHTML)}function toast(t,e="success"){const n=e==="error"?6e3:3500,i=document.createElement("div");i.className=`toast toast-${e}`,i.setAttribute("role","alert");const s=document.createElement("span");s.textContent=t;const o=document.createElement("button");o.className="toast-close",o.setAttribute("aria-label","\u0110\xF3ng th\xF4ng b\xE1o"),o.textContent="\xD7",o.onclick=()=>i.remove(),i.appendChild(s),i.appendChild(o),i.addEventListener("mouseenter",()=>i.classList.add("toast-paused")),i.addEventListener("mouseleave",()=>i.classList.remove("toast-paused")),$("#toast-container").appendChild(i);const a=setTimeout(()=>i.remove(),n);i.addEventListener("mouseenter",()=>clearTimeout(a)),i.addEventListener("mouseleave",()=>setTimeout(()=>i.remove(),1e3))}function setLoading(t="\u0110ang t\u1EA3i..."){$("#app").innerHTML=`
    ...
    `);const l=$("#modal-overlay"),c=l?.querySelector("#prompt-action-input"),d=l?.querySelector("#prompt-action-error"),u=l?.querySelector('[data-prompt-action="cancel"]'),m=l?.querySelector('[data-prompt-action="confirm"]');let h=!1;const g=w=>{h||(h=!0,window._modalCloseCallback=null,closeModal(),r(w))};window._modalCloseCallback=()=>g(null);const v=()=>{const w=c?.value??"",y=w.trim(),_=typeof a=="function"?a(y,w):"";if(_){d&&(d.textContent=_),c?.focus(),c?.select?.();return}g(y)};u.onclick=()=>g(null),m.onclick=v,c?.addEventListener("keydown",w=>{w.key==="Enter"&&(w.preventDefault(),v())}),l.onclick=w=>{w.target===l&&g(null)},requestAnimationFrame(()=>{c?.focus(),c?.select?.()})})}let _oneTimeStudentCredentials=null;function closeModal(t){const e=$("#modal-overlay");t&&t.target!==e||(_oneTimeStudentCredentials=null,e&&(e.onclick=n=>closeModal(n)),window._modalCloseCallback&&(window._modalCloseCallback(),window._modalCloseCallback=null),e.classList.add("hidden"),$("#modal-body").innerHTML="",_modalPreviousFocus&&(_modalPreviousFocus.focus(),_modalPreviousFocus=null))}document.addEventListener("keydown",t=>{const e=$("#modal-overlay");if(!e||e.classList.contains("hidden"))return;const n=e.querySelector(".modal")||e;if(t.key==="Escape"){closeModal();return}if(t.key==="Tab"){const i=[...n.querySelectorAll(FOCUSABLE)];if(!i.length)return;const s=i[0],o=i[i.length-1];(t.shiftKey?document.activeElement===s:document.activeElement===o)&&(t.preventDefault(),(t.shiftKey?o:s).focus())}});function csvEscape(t){return`"${String(t??"").replace(/"/g,'""')}"`}function downloadCsvFile(t,e,n){const i=[e,...n].map(r=>r.map(csvEscape).join(",")).join(`
`),s=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=t,a.click(),URL.revokeObjectURL(o)}async function downloadAudioFile(t,e,n){if(!t)return;const i=n?n.innerHTML:"";try{n&&(n.disabled=!0,n.innerHTML="\u23F3");const s=await fetch(t,{cache:"no-store"});if(!s.ok)throw new Error("fetch failed");const o=await s.blob(),a=(t.split("?")[0].match(/\.([a-zA-Z0-9]+)$/)||[])[1],r=(o.type.split("/")[1]||"").split(";")[0],l=a||r||"webm",c=(e||"audio").replace(/\.[a-zA-Z0-9]{2,5}$/,""),d=URL.createObjectURL(o),u=document.createElement("a");u.href=d,u.download=`${c}.${l}`,document.body.appendChild(u),u.click(),u.remove(),URL.revokeObjectURL(d)}catch(s){toast("T\u1EA3i file th\u1EA5t b\u1EA1i: "+(s.message||"l\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"),"error")}finally{n&&(n.disabled=!1,n.innerHTML=i)}}window.downloadAudioFile=downloadAudioFile;function buildStudentCredentialsFilename(t="student_accounts"){const e=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");return`${t}_${e}.csv`}function openStudentCredentialsModal(t,e,n="student_accounts"){const i=(Array.isArray(e)?e:[]).map(s=>({full_name:String(s?.full_name||"").trim(),username:String(s?.username||"").trim(),password:String(s?.password||"")})).filter(s=>s.full_name&&s.username&&s.password);_oneTimeStudentCredentials=i.length>0?{rows:i,fileName:buildStudentCredentialsFilename(n)}:null,openModal(t,`
    ...
    </div>`}window.showHistogramStudents=showHistogramStudents;function renderStatsTab(t,e){destroyStatsCharts();const{timeline:n,per_student:i,per_assignment:s}=e,o=_statsSkillFilter,a=_statsStatusFilter,r=_statsModeFilter,l=_statsScaleFilter,c=s.filter(p=>!(o&&p.skill!==o||a==="active"&&!p.is_active||a==="closed"&&p.is_active||r&&p.mode!==r||l&&(p.scoring_scale||"10")!==l)),d=new Set(c.map(p=>p.id)),u=p=>p.length?p.reduce((f,b)=>f+b,0)/p.length:null,m=i.length,h=c.length,g=c.filter(p=>p.is_active).length,v=h-g,w=new Map;i.forEach(p=>{p.submissions.forEach(f=>{if(!d.has(f.assignment_id))return;const b=`${p.id||p.student_id}:${f.assignment_id}`,k=w.get(b);(!k||(f.attempt_number||1)>(k.attempt_number||1))&&w.set(b,{...f,student_name:p.name})})});const y=Array.from(w.values()),_=y.filter(p=>p.overall_score!==null);_statsAllScoredSubs=_;const E=u(_.map(p=>Number(p.overall_score))),I=h*m,H=I>0?Math.round(y.length/I*100):0,D=[0,0,0,0,0];for(const p of _){const f=Number(p.overall_score),b=f>=9?4:Math.min(4,Math.floor(f/2));D[b]++}const F=["reading","listening","writing","speaking"],j={},O={};for(const p of F){const f=_.filter(S=>S.skill===p);j[p]=u(f.map(S=>Number(S.overall_score)));const b=c.filter(S=>S.skill===p),k=y.filter(S=>S.skill===p),B=b.length*m;O[p]={count:b.length,submitted:k.length,pct:B>0?Math.round(k.length/B*100):0}}let P=i.map(p=>{const f=p.submissions.filter(T=>d.has(T.assignment_id)),b=new Map;f.forEach(T=>{const x=b.get(T.assignment_id);(!x||(T.attempt_number||1)>(x.attempt_number||1))&&b.set(T.assignment_id,T)});const k=Array.from(b.values()),B=k.filter(T=>T.overall_score!==null),S=T=>u(B.filter(x=>x.skill===T).map(x=>Number(x.overall_score))),C=k.filter(T=>!T.is_active&&T.deadline),L=C.filter(T=>T.on_time).length;return{...p,submitted:k.length,total:h,avg_score:o?S(o):u(B.map(T=>Number(T.overall_score))),avg_reading:S("reading"),avg_listening:S("listening"),avg_writing:S("writing"),avg_speaking:S("speaking"),on_time:L,closed_total:C.length,on_time_rate:C.length>0?L/C.length:null}});_statsSortCol&&(P=[...P].sort((p,f)=>{let b=p[_statsSortCol],k=f[_statsSortCol];return b==null?1:k==null?-1:typeof b=="string"?_statsSortDir==="asc"?b.localeCompare(k):k.localeCompare(b):_statsSortDir==="asc"?b-k:k-b}));const q=p=>p!=null?Number(p).toFixed(1):"\u2014",U=(p,f)=>f>0?Math.round(p/f*100):0,M=p=>_statsSortCol!==p?'<span class="sort-icon">\u2195</span>':`<span class="sort-icon active">${_statsSortDir==="asc"?"\u2191":"\u2193"}</span>`,V={reading:"#3b82f6",listening:"#f59e0b",writing:"#8b5cf6",speaking:"#22c55e"},z=["#3b82f6","#ef4444","#22c55e","#f59e0b","#8b5cf6","#06b6d4","#ec4899","#14b8a6","#f97316","#a3e635"],X={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking",composite:"T\u1ED5ng h\u1EE3p"},Y=`
    <div class="stats-cards-grid">
      ...
      <div class="stats-card">
        ...
        <div class="stats-card-body">
          ...
          <div class="stats-card-label">${g} \u0111ang m\u1EDF \xB7 ${v} \u0111\xE3 \u0111\xF3ng</div>
        </div>
      </div>
      <div class="stats-card">
      ...
      <div class="stats-card">
        ...
        <div class="stats-card-body">
          ...
          <div class="stats-card-label">\u0110i\u1EC3m TB l\u1EDBp (${_.length} b\xE0i \u0111\xE3 ch\u1EA5m)</div>
        </div>
      </div>
    </div>`,Z=`
      ...
      <div class="stats-skill-chart">
        ${["reading","listening","writing","speaking"].map(p=>{const f=O[p];return!f||f.count===0?"":`<div class="stats-skill-row">
            ...
            <div class="stats-bar-wrap">
              <div class="stats-bar-fill" style="width:${f.pct}%;background:${V[p]}"></div>
            </div>
    ...
    <div class="stats-section-card stats-chart-card">
      <div class="stats-section-title">
        Ph\xE2n b\u1ED5 \u0111i\u1EC3m (${_.length} b\xE0i ch\u1EA5m)
    ...
    </div>`,K=c.filter(p=>!p.is_active&&p.deadline),N=_closedAssignsSearch?K.filter(p=>p.title.toLowerCase().includes(_closedAssignsSearch)):K,W=3,R=_closedAssignsExpanded?N:N.slice(0,W),it=N.length-R.length,st=K.length===0?"":`
    <div class="stats-section-card">
      ...
      ${N.length===0?'<p style="color:var(--gray-400);font-size:13px;padding:8px 0 4px">Kh\xF4ng t\xECm th\u1EA5y b\xE0i t\u1EADp ph\xF9 h\u1EE3p</p>':`<canvas id="chart-ontime" height="${Math.max(80,R.length*38)}"></canvas>`}
      ${N.length>W?`
    ...
    <div class="stats-section-card">
      ...
      ${P.length===0?'<div class="empty-state" style="padding:20px">Kh\xF4ng c\xF3 d\u1EEF li\u1EC7u</div>':`<div class="table-wrap">
          <table class="stats-table">
            ...
            <tbody>
              ${P.map(p=>{const f=U(p.submitted,p.total),b=p.on_time_rate!==null?Math.round(p.on_time_rate*100):null;return`<tr>
    ...
    </div>`,A=p=>makeSortIcon(p,_assignTableSortCol,_assignTableSortDir),G=[...c];_assignTableSortCol&&G.sort((p,f)=>{let b,k;switch(_assignTableSortCol){case"title":b=p.title.toLowerCase(),k=f.title.toLowerCase();break;case"skill":b=p.skill||"",k=f.skill||"";break;case"mode":b=p.mode||"",k=f.mode||"";break;case"submitted_rate":b=p.total?p.submitted/p.total:0,k=f.total?f.submitted/f.total:0;break;case"avg_score":b=p.avg_score??-1,k=f.avg_score??-1;break;case"on_time":b=p.on_time??-1,k=f.on_time??-1;break;case"late":b=p.late??-1,k=f.late??-1;break;case"missing":b=p.missing??-1,k=f.missing??-1;break;case"is_active":b=p.is_active?1:0,k=f.is_active?1:0;break;default:return 0}return b<k?_assignTableSortDir==="asc"?-1:1:b>k?_assignTableSortDir==="asc"?1:-1:0});const ct=`
    <div class="stats-section-card">
      ...
      ${c.length===0?'<div class="empty-state" style="padding:20px">Kh\xF4ng c\xF3 b\xE0i t\u1EADp ph\xF9 h\u1EE3p</div>':`<div class="table-wrap">
          <table class="stats-table">
            ...
            <tbody>
              ${G.map(p=>{const f=U(p.submitted,p.total);return`<tr>
    ...
    ${Z}
    ${lt}
    ${ct}`,ensureChartJsLoaded().then(()=>requestAnimationFrame(()=>{const p=document.getElementById("chart-skill-score");if(p){const S=["reading","listening","writing","speaking"].filter(C=>j[C]!==null);if(S.length>0){const C=new Chart(p.getContext("2d"),{type:"bar",data:{labels:S.map(L=>X[L]),datasets:[{data:S.map(L=>Number(j[L]).toFixed(2)),backgroundColor:S.map(L=>V[L]+"cc"),borderColor:S.map(L=>V[L]),borderWidth:1,borderRadius:6}]},options:{indexAxis:"y",responsive:!0,plugins:{legend:{display:!1}},scales:{x:{min:0,max:9,grid:{color:"#f3f4f6"},ticks:{font:{size:11}}},y:{ticks:{font:{size:12}}}}}});_statsCharts.push(C)}else p.insertAdjacentHTML("afterend",'<p style="color:var(--gray-400);font-size:13px;padding:20px 0">Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u \u0111i\u1EC3m</p>'),p.remove()}const f=document.getElementById("chart-score-dist");if(f)if(D.reduce((C,L)=>C+L,0)>0){const C=new Chart(f.getContext("2d"),{type:"bar",data:{labels:["0 \u2013 2","2 \u2013 4","4 \u2013 6","6 \u2013 8","8 \u2013 9"],datasets:[{label:"S\u1ED1 b\xE0i",data:D,backgroundColor:["#fca5a5","#fcd34d","#86efac","#67e8f9","#6ee7b7"],hoverBackgroundColor:["#f87171","#fbbf24","#4ade80","#22d3ee","#34d399"],borderRadius:6}]},options:{responsive:!0,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,ticks:{stepSize:1,font:{size:11}},grid:{color:"#f3f4f6"}},x:{ticks:{font:{size:11}}}},onClick:(L,T)=>{T.length&&showHistogramStudents(T[0].index)},onHover:(L,T)=>{L.native.target.style.cursor=T.length?"pointer":"default"}}});_statsCharts.push(C)}else f.insertAdjacentHTML("afterend",'<p style="color:var(--gray-400);font-size:13px;padding:20px 0">Ch\u01B0a c\xF3 b\xE0i n\xE0o \u0111\u01B0\u1EE3c ch\u1EA5m \u0111i\u1EC3m</p>'),f.remove();const b=document.getElementById("chart-trend");if(b){const S=[...s].reverse().filter(x=>!(_statsTrendSkill&&x.skill!==_statsTrendSkill||_statsModeFilter&&x.mode!==_statsModeFilter||_statsScaleFilter&&(x.scoring_scale||"10")!==_statsScaleFilter)),C=S.map(x=>x.title.length>18?x.title.slice(0,16)+"\u2026":x.title),L=i.map((x,dt)=>{const J=z[dt%z.length];return{label:x.name,_studentId:x.id,data:S.map(ut=>{const Q=x.submissions.find(pt=>pt.assignment_id===ut.id);return Q&&Q.overall_score!==null?Number(Q.overall_score):null}),borderColor:J,backgroundColor:J+"22",borderWidth:2,pointRadius:5,pointHoverRadius:7,fill:!1,tension:.3,spanGaps:!0}});if(S.length<1){b.style.display="none";const x=document.getElementById("trend-empty-msg");x&&(x.style.display="")}const T=new Chart(b.getContext("2d"),{type:"line",data:{labels:C,datasets:L},options:{responsive:!0,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!1},tooltip:{filter:x=>x.raw!==null,callbacks:{title:x=>x[0]?.label||"",label:x=>` ${x.dataset.label}: ${x.raw!==null?Number(x.raw).toFixed(1):"\u2014"}`}}},scales:{y:{min:0,max:9,ticks:{font:{size:11}},grid:{color:"#f3f4f6"}},x:{ticks:{font:{size:10},maxRotation:30}}}}});_statsCharts.push(T)}const k=document.getElementById("chart-timeline");if(k&&n.length>=2){const S=new Chart(k.getContext("2d"),{type:"line",data:{labels:n.map(C=>{const L=new Date(C.week);return`${L.getDate()}/${L.getMonth()+1}`}),datasets:[{label:"L\u01B0\u1EE3t n\u1ED9p",data:n.map(C=>C.count),borderColor:"#0f766e",backgroundColor:"#0f766e22",borderWidth:2,pointRadius:4,fill:!0,tension:.3}]},options:{responsive:!0,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,ticks:{stepSize:1,font:{size:11}},grid:{color:"#f3f4f6"}},x:{ticks:{font:{size:11}}}}}});_statsCharts.push(S)}const B=document.getElementById("chart-ontime");if(B&&R.length>0){const S=new Chart(B.getContext("2d"),{type:"bar",data:{labels:R.map(C=>C.title.length>22?C.title.slice(0,20)+"\u2026":C.title),datasets:[{label:"\u0110\xFAng h\u1EA1n",data:R.map(C=>C.on_time),backgroundColor:"#86efac",borderRadius:4},{label:"N\u1ED9p mu\u1ED9n",data:R.map(C=>C.late),backgroundColor:"#fcd34d",borderRadius:4},{label:"Ch\u01B0a n\u1ED9p",data:R.map(C=>C.missing||0),backgroundColor:"#fca5a5",borderRadius:4}]},options:{indexAxis:"y",responsive:!0,plugins:{legend:{position:"bottom",labels:{font:{size:11}}}},scales:{x:{stacked:!0,ticks:{stepSize:1,font:{size:11}},grid:{color:"#f3f4f6"}},y:{stacked:!0,ticks:{font:{size:11}}}}}});_statsCharts.push(S)}})).catch(()=>{const p=t.querySelector(".stats-charts-row");p&&p.insertAdjacentHTML("beforebegin",'<div class="empty-state" style="margin-bottom:16px"><p>Kh\xF4ng th\u1EC3 t\u1EA3i bi\u1EC3u \u0111\u1ED3 l\xFAc n\xE0y. B\u1EA1n v\u1EABn c\xF3 th\u1EC3 xem d\u1EEF li\u1EC7u d\u1EA1ng b\u1EA3ng b\xEAn d\u01B0\u1EDBi.</p></div>')})}function toggleStudentStatsRow(t){const e=document.getElementById(`stats-row-${t}`);e&&(e.style.display=e.style.display==="none"?"":"none")}window.toggleStudentStatsRow=toggleStudentStatsRow;function buildStatsSubRows(t){return t.length===0?'<tr><td colspan="5" style="color:var(--gray-400);padding:12px">Ch\u01B0a n\u1ED9p b\xE0i n\xE0o</td></tr>':(_statsSubSortCol?[...t].sort((n,i)=>{let s,o;if(_statsSubSortCol==="title")s=(n.assignment_title||"").toLowerCase(),o=(i.assignment_title||"").toLowerCase();else if(_statsSubSortCol==="skill")s=n.skill||"",o=i.skill||"";else if(_statsSubSortCol==="score")s=n.overall_score??-1,o=i.overall_score??-1;else if(_statsSubSortCol==="submitted_at")s=n.submitted_at||"",o=i.submitted_at||"";else if(_statsSubSortCol==="on_time")s=n.on_time===null?-1:n.on_time?1:0,o=i.on_time===null?-1:i.on_time?1:0;else return 0;return s<o?_statsSubSortDir==="asc"?-1:1:s>o?_statsSubSortDir==="asc"?1:-1:0}):t).map(n=>`<tr>
...
`),o=new Blob(["\uFEFF"+s],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(o),r=document.createElement("a");r.href=a,r.download=`${t.title.replace(/[^a-zA-Z0-9_\-]/g,"_")}_diem.csv`,r.click(),URL.revokeObjectURL(a)}window.exportSubmissionsCSV=exportSubmissionsCSV;function buildSubmissionRows(t,e){return t.length===0?`<tr><td colspan="5">
  ...
  `)}window.showMicErrorModal=showMicErrorModal;async function confirmReloadForMicPermission(){closeModal(),await confirmAction({title:"T\u1EA3i l\u1EA1i trang?",message:"Nh\u1EADn x\xE9t/\u0111i\u1EC3m ch\u01B0a l\u01B0u tr\xEAn trang ch\u1EA5m b\xE0i s\u1EBD b\u1ECB m\u1EA5t. C\xE1c b\u1EA3n ghi \xE2m \u0111\xE3 upload xong (tr\u1EA1ng th\xE1i \u2713) th\xEC v\u1EABn an to\xE0n.",confirmText:"T\u1EA3i l\u1EA1i trang",danger:!0})&&location.reload()}window.confirmReloadForMicPermission=confirmReloadForMicPermission;function stopVoiceNoteRecording(){clearInterval(_voiceNoteRecordTimer),_voiceNoteMediaRecorder?.stop()}window.stopVoiceNoteRecording=stopVoiceNoteRecording;function _onVoiceNoteRecordingDone(t,e,n){const i=_gradingVoiceNotes[t];if(_voiceNoteRecordIdx=-1,!i)return;if(!e||e.size===0){i.status="idle",renderVoiceNotesList(),toast("B\u1EA3n ghi r\u1ED7ng \u2014 vui l\xF2ng th\u1EED ghi \xE2m l\u1EA1i","error");return}const s=String(n||"audio/webm").split(";")[0].trim(),o=extFromAudioMime(s),a=new File([e],`voice-note-${t+1}.${o}`,{type:s});i.file=a,i.name=a.name,i.size=e.size,i.localUrl=URL.createObjectURL(e),i.status="uploading",i.pct=0,renderVoiceNotesList(),_uploadVoiceNoteSlot(t)}function pickAudioRecorderMime(){const t=["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg"];for(const e of t)try{if(window.MediaRecorder&&MediaRecorder.isTypeSupported(e))return e}catch{}return""}function extFromAudioMime(t){const e=String(t||"").split(";")[0].trim().toLowerCase();return e.includes("webm")?"webm":e.includes("ogg")?"ogg":e.includes("mp4")||e.includes("m4a")||e.includes("aac")?"m4a":e.includes("mpeg")||e.includes("mp3")?"mp3":e.includes("wav")?"wav":e.includes("flac")?"flac":"webm"}const BROWSER_PLAYABLE_AUDIO_EXTS=new Set(["mp3","mpeg","mpga","mp4","m4a","aac","wav","wave","ogg","oga","webm","flac"]);function probeAudioPlayable(t){return new Promise(e=>{const n=URL.createObjectURL(t),i=new Audio;let s=!1;const o=r=>{s||(s=!0,clearTimeout(a),i.removeAttribute("src"),URL.revokeObjectURL(n),e(r))},a=setTimeout(()=>o(!1),8e3);i.preload="metadata",i.onloadedmetadata=()=>o(isFinite(i.duration)&&i.duration>0),i.onerror=()=>o(!1),i.src=n})}async function onVoiceNoteFileSelected(t,e){const n=t.files?.[0];if(!n||!_gradingVoiceNotes[e])return;t.value="";const i=(n.name.split(".").pop()||"").toLowerCase();if(!BROWSER_PLAYABLE_AUDIO_EXTS.has(i)){showFeedbackAudioUnsupported(n.name,i);return}_gradingVoiceNotes[e].status="checking",renderVoiceNotesList();const s=await probeAudioPlayable(n);if(_gradingVoiceNotes[e]){if(!s){_gradingVoiceNotes[e].status="idle",renderVoiceNotesList(),showFeedbackAudioUnsupported(n.name,i,!0);return}_gradingVoiceNotes[e].file=n,_gradingVoiceNotes[e].name=n.name,_gradingVoiceNotes[e].size=n.size,_gradingVoiceNotes[e].localUrl=URL.createObjectURL(n),_gradingVoiceNotes[e].status="uploading",_gradingVoiceNotes[e].pct=0,renderVoiceNotesList(),_uploadVoiceNoteSlot(e)}}window.onVoiceNoteFileSelected=onVoiceNoteFileSelected;function showFeedbackAudioUnsupported(t,e,n=!1){openModal("\u26A0\uFE0F File audio kh\xF4ng ph\xE1t \u0111\u01B0\u1EE3c",`
    ...
    `).join("")}
  `}function setAssignTagFilter(t){_assignTagFilter=t||"",renderAssignPicker(_assignSkillFilter)}function selectQuestion(t,e){_selectedQuestionId=t,document.querySelectorAll(".question-picker-item").forEach(s=>s.classList.remove("selected")),e.classList.add("selected"),e.querySelector("input[type=radio]").checked=!0;const n=e.dataset.skill||"",i=$("#assign-scale-group");i&&(i.style.display=n==="reading"||n==="listening"||n==="composite"?"":"none")}function _syncAssignTimeLimitVisibility(){const t=document.querySelector('input[name="assign-mode"]:checked'),e=!t||t.value!=="practice",n=$("#assign-time-limit-group");n&&(n.style.display=e?"":"none")}async function submitAssign(t,e=!1){const n=$("#assign-title")?.value.trim(),i=$("#assign-deadline")?.value,s=i?new Date(i).toISOString():null,a=document.querySelector('input[name="assign-mode"]:checked')?.value==="practice"?"practice":"exam",r=$("#assign-time-limit")?.value.trim(),l=a==="exam"&&r?Number(r):null,d=document.querySelector('input[name="assign-scale"]:checked')?.value||null;if(!n){toast("Vui l\xF2ng nh\u1EADp t\xEAn b\xE0i t\u1EADp","error");return}if(!_selectedQuestionId){toast("Vui l\xF2ng ch\u1ECDn m\u1ED9t \u0111\u1EC1 t\u1EEB kho","error");return}if(!e&&d==="ielts"){const u=_questions.find(v=>v.id===_selectedQuestionId),m=u?.question_count??0,h=u?.skill??"";if((h==="reading"||h==="listening")&&m!==40){let v=document.getElementById("assign-scale-warn");v||(v=document.createElement("div"),v.id="assign-scale-warn",document.getElementById("modal-body")?.appendChild(v)),v.innerHTML=`
...
`),a=document.createDocumentFragment();o.forEach((l,c)=>{c>0&&a.appendChild(document.createElement("br")),l&&a.appendChild(document.createTextNode(l))}),s.insertNode(a),s.collapse(!1),i.removeAllRanges(),i.addRange(s);const r=document.getElementById("content-composer-host");r&&normalizeIndentTokensInElement(r),syncContentBlocksFromEditor(),saveComposerRange()}async function showQuestionDetail({id:t}){setLoading("\u0110ang t\u1EA3i \u0111\u1EC1...");const e=routeToken();try{const n=await api.get(`/questions/${t}`);if(routeChanged(e))return;renderQuestionDetail(n)}catch(n){if(routeChanged(e))return;toast("L\u1ED7i t\u1EA3i \u0111\u1EC1: "+(n.error||n.message),"error"),renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c \u0111\u1EC1",n,`/questions/${t}`)}}function renderQuestionDetail(t){_audioFile=null,_editingVocabIndex=-1,_vocabItems=Array.isArray(t.vocabulary)?[...t.vocabulary]:[];let e="";if(t.skill==="composite")_cqSections=Array.isArray(t.sections)?t.sections.map(s=>({label:s.label||"",skill:s.skill||"",time_limit_minutes:s.time_limit_minutes??null,questions_data:s.questions_data||[],content_blocks:s.content_blocks||[],content_text:s.content_text||"",content_url:s.content_url||null,content_urls:s.content_urls||[],script:s.script||"",vocabulary:s.vocabulary||[],_id:s.id})):[],_cqEditingIdx=-1,e='<div id="cq-sections-ui"></div>';else if(t.skill==="reading")e=`
    ...
    </div>`}function vocabImportBoxHtml(){return`
  ...
  `,_buildSharedStatsChart(n)}function _buildSharedStatsChart(t){if(_sharedStatsChart){try{_sharedStatsChart.destroy()}catch{}_sharedStatsChart=null}const e=document.getElementById("sp-stats-chart");if(!e)return;const n=t.map(a=>_sharedStatsMode==="avg"?a.avg:a.max).filter(a=>a!=null).map(Number),i=[];for(let a=1;a<=9;a+=.5)i.push(a);const s=new Array(i.length).fill(0);for(const a of n){const r=Math.round(a*2)/2,l=i.indexOf(Math.min(9,Math.max(1,r)));l>=0&&s[l]++}const o=i.map(a=>a>=7?{bg:"#16a34a99",border:"#16a34a"}:a>=5?{bg:"#ca8a0499",border:"#ca8a04"}:{bg:"#dc262699",border:"#dc2626"});ensureChartJsLoaded().then(()=>{_sharedStatsChart=new Chart(e,{type:"bar",data:{labels:i.map(a=>a%1===0?String(a):a.toFixed(1)),datasets:[{label:_sharedStatsMode==="avg"?"\u0110i\u1EC3m TB / HS":"\u0110i\u1EC3m cao nh\u1EA5t / HS",data:s,backgroundColor:o.map(a=>a.bg),borderColor:o.map(a=>a.border),borderWidth:1,borderRadius:4}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1},tooltip:{callbacks:{title:a=>`Band ${a[0].label}`,label:a=>`${a.raw} h\u1ECDc sinh`}}},scales:{x:{title:{display:!0,text:"Band IELTS"},grid:{display:!1}},y:{title:{display:!0,text:"S\u1ED1 h\u1ECDc sinh"},beginAtZero:!0,ticks:{stepSize:1}}}}})}).catch(()=>{const a=e.parentElement;a&&(a.innerHTML='<div class="empty-state" style="height:100%;display:flex;align-items:center;justify-content:center"><p>Kh\xF4ng th\u1EC3 t\u1EA3i bi\u1EC3u \u0111\u1ED3 l\xFAc n\xE0y.</p></div>')})}function setSharedStatsMode(t){_sharedStatsMode=t,document.querySelectorAll(".sp-stats-mode-btn").forEach(e=>{e.classList.toggle("active",e.textContent.includes(t==="avg"?"Trung b\xECnh":"Cao nh\u1EA5t"))}),_buildSharedStatsChart(_groupSharedStatsByStudent(_sharedStatsRows))}function toggleSharedStudentDetail(t){const e=document.getElementById(`sp-detail-${t}`),n=document.querySelector(`.sp-stats-student-row[data-sid="${t}"] .sp-expand-btn`);if(!e)return;const i=e.classList.toggle("hidden");n&&(n.textContent=i?"Chi ti\u1EBFt \u25BE":"Thu g\u1ECDn \u25B4")}window.showSharedPoolStats=showSharedPoolStats,window.closeSharedStatsModal=closeSharedStatsModal,window.setSharedStatsMode=setSharedStatsMode,window.toggleSharedStudentDetail=toggleSharedStudentDetail;let _sharedEditingId=null;async function showSharedPoolForm(){_sharedEditingId=null,_vocabItems=[],_contentBlocks=[createTextBlock("")],_audioSlots=[_newAudioSlot()],_audioFiles=_audioSlots,_audioFile=null,_audioUploadUrl=null,_audioUploadKey=null,_audioUploadName="",_audioUploadSize=0,_audioUploading=!1,_editingVocabIndex=-1,renderSharedPoolFormPage("T\u1EA1o \u0111\u1EC1 luy\u1EC7n t\u1EADp m\u1EDBi",null);const t=restoreSpDraftIntoForm("new");startSpDraftAutosave("new"),t&&toast("\u0110\xE3 kh\xF4i ph\u1EE5c b\u1EA3n nh\xE1p ch\u01B0a l\u01B0u trong 15 ph\xFAt g\u1EA7n nh\u1EA5t.","info")}async function showSharedPoolDetail({id:t}){const e=routeToken();let n;try{n=await api.get(`/shared-pool/${t}`)}catch(s){if(routeChanged(e))return;renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c \u0111\u1EC1",s,"/shared-pool");return}if(routeChanged(e))return;_sharedEditingId=t,_vocabItems=Array.isArray(n.vocabulary)?[...n.vocabulary]:[],_contentBlocks=Array.isArray(n.content_blocks)&&n.content_blocks.length?n.content_blocks:[createTextBlock(n.content_text||"")],_audioSlots=[_newAudioSlot()],_audioFiles=_audioSlots,_audioFile=null,_audioUploadUrl=n.content_url||null,_audioUploadKey=null,_audioUploadName="",_audioUploadSize=0,_audioUploading=!1,_editingVocabIndex=-1,renderSharedPoolFormPage("S\u1EEDa \u0111\u1EC1 luy\u1EC7n t\u1EADp",n);const i=restoreSpDraftIntoForm("edit",t,n.skill);startSpDraftAutosave("edit",t,n.skill),i&&toast("\u0110\xE3 kh\xF4i ph\u1EE5c b\u1EA3n nh\xE1p ch\u01B0a l\u01B0u trong 15 ph\xFAt g\u1EA7n nh\u1EA5t.","info");try{const s=await api.get(`/shared-pool/${t}/stats`);if(routeChanged(e))return;renderSharedPoolStats(s,t)}catch{}}function renderSharedPoolFormPage(t,e){const n=e?.skill||"";$("#app").innerHTML=`
    ...
    ${t.length===0?'<p style="color:var(--gray-400)">Ch\u01B0a c\xF3 h\u1ECDc sinh n\xE0o l\xE0m \u0111\u1EC1 n\xE0y.</p>':`<div class="table-wrap"><table>
      ...
      <div style="display:flex;gap:6px">
        ...
        <div id="location-pick-hint" class="location-pick-hint hidden"></div>
        ${answerGridHtml()}
        ${vocabSectionHtml()}`:s.skill==="listening"?o=`<div class="form-group"><label class="form-label">File Audio <span style="color:var(--danger)">*</span></label>${audioUploadHtml()}</div>
        <div class="form-group" id="script-section">
          <label class="form-label">Script Listening</label>
          ${sttSelectorHtml()}
        ...
        <div id="location-pick-hint" class="location-pick-hint hidden"></div>
        ${answerGridHtml()}
        ${vocabSectionHtml()}`:s.skill==="writing"?o=`${contentComposerHtml("\u0110\u1EC1 b\xE0i Writing")}`:s.skill==="speaking"&&(o=`${contentComposerHtml("C\xE2u h\u1ECFi / Cue Card")}`),i=`
