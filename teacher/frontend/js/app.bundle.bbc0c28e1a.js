...
`),s=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=t,a.click(),URL.revokeObjectURL(o)}async function downloadAudioFile(t,e,n){if(!t)return;const i=n?n.innerHTML:"";try{n&&(n.disabled=!0,n.innerHTML="\u23F3");const s=await fetch(t,{cache:"no-store"});if(!s.ok)throw new Error("fetch failed");const o=await s.blob(),a=(t.split("?")[0].match(/\.([a-zA-Z0-9]+)$/)||[])[1],r=(o.type.split("/")[1]||"").split(";")[0],l=a||r||"webm",c=(e||"audio").replace(/\.[a-zA-Z0-9]{2,5}$/,""),d=URL.createObjectURL(o),u=document.createElement("a");u.href=d,u.download=`${c}.${l}`,document.body.appendChild(u),u.click(),u.remove(),URL.revokeObjectURL(d)}catch(s){toast("T\u1EA3i file th\u1EA5t b\u1EA1i: "+(s.message||"l\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"),"error")}finally{n&&(n.disabled=!1,n.innerHTML=i)}}window.downloadAudioFile=downloadAudioFile;function buildStudentCredentialsFilename(t="student_accounts"){const e=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");return`${t}_${e}.csv`}function openStudentCredentialsModal(t,e,n="student_accounts"){const i=(Array.isArray(e)?e:[]).map(s=>({full_name:String(s?.full_name||"").trim(),username:String(s?.username||"").trim(),password:String(s?.password||"")})).filter(s=>s.full_name&&s.username&&s.password);_oneTimeStudentCredentials=i.length>0?{rows:i,fileName:buildStudentCredentialsFilename(n)}:null,openModal(t,`
    ...
    </div>`}window.showHistogramStudents=showHistogramStudents;function renderStatsTab(t,e){destroyStatsCharts();const{timeline:n,per_student:i,per_assignment:s}=e,o=_statsSkillFilter,a=_statsStatusFilter,r=_statsModeFilter,l=_statsScaleFilter,c=s.filter(p=>!(o&&p.skill!==o||a==="active"&&!p.is_active||a==="closed"&&p.is_active||r&&p.mode!==r||l&&(p.scoring_scale||"10")!==l)),d=new Set(c.map(p=>p.id)),u=p=>p.length?p.reduce((v,b)=>v+b,0)/p.length:null,m=i.length,h=c.length,g=c.filter(p=>p.is_active).length,f=h-g,w=new Map;i.forEach(p=>{p.submissions.forEach(v=>{if(!d.has(v.assignment_id))return;const b=`${p.id||p.student_id}:${v.assignment_id}`,k=w.get(b);(!k||(v.attempt_number||1)>(k.attempt_number||1))&&w.set(b,{...v,student_name:p.name})})});const y=Array.from(w.values()),_=y.filter(p=>p.overall_score!==null);_statsAllScoredSubs=_;const E=u(_.map(p=>Number(p.overall_score))),I=h*m,H=I>0?Math.round(y.length/I*100):0,D=[0,0,0,0,0];for(const p of _){const v=Number(p.overall_score),b=v>=9?4:Math.min(4,Math.floor(v/2));D[b]++}const F=["reading","listening","writing","speaking"],j={},O={};for(const p of F){const v=_.filter(S=>S.skill===p);j[p]=u(v.map(S=>Number(S.overall_score)));const b=c.filter(S=>S.skill===p),k=y.filter(S=>S.skill===p),B=b.length*m;O[p]={count:b.length,submitted:k.length,pct:B>0?Math.round(k.length/B*100):0}}let P=i.map(p=>{const v=p.submissions.filter(T=>d.has(T.assignment_id)),b=new Map;v.forEach(T=>{const x=b.get(T.assignment_id);(!x||(T.attempt_number||1)>(x.attempt_number||1))&&b.set(T.assignment_id,T)});const k=Array.from(b.values()),B=k.filter(T=>T.overall_score!==null),S=T=>u(B.filter(x=>x.skill===T).map(x=>Number(x.overall_score))),C=k.filter(T=>!T.is_active&&T.deadline),L=C.filter(T=>T.on_time).length;return{...p,submitted:k.length,total:h,avg_score:o?S(o):u(B.map(T=>Number(T.overall_score))),avg_reading:S("reading"),avg_listening:S("listening"),avg_writing:S("writing"),avg_speaking:S("speaking"),on_time:L,closed_total:C.length,on_time_rate:C.length>0?L/C.length:null}});_statsSortCol&&(P=[...P].sort((p,v)=>{let b=p[_statsSortCol],k=v[_statsSortCol];return b==null?1:k==null?-1:typeof b=="string"?_statsSortDir==="asc"?b.localeCompare(k):k.localeCompare(b):_statsSortDir==="asc"?b-k:k-b}));const q=p=>p!=null?Number(p).toFixed(1):"\u2014",U=(p,v)=>v>0?Math.round(p/v*100):0,M=p=>_statsSortCol!==p?'<span class="sort-icon">\u2195</span>':`<span class="sort-icon active">${_statsSortDir==="asc"?"\u2191":"\u2193"}</span>`,V={reading:"#3b82f6",listening:"#f59e0b",writing:"#8b5cf6",speaking:"#22c55e"},z=["#3b82f6","#ef4444","#22c55e","#f59e0b","#8b5cf6","#06b6d4","#ec4899","#14b8a6","#f97316","#a3e635"],W={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking",composite:"T\u1ED5ng h\u1EE3p"},Y=`
    ...
    <div class="stats-section-card">
      ...
      <div class="stats-skill-chart">
        ${["reading","listening","writing","speaking"].map(p=>{const v=O[p];return!v||v.count===0?"":`<div class="stats-skill-row">
    ...
    </div>`,K=c.filter(p=>!p.is_active&&p.deadline),N=_closedAssignsSearch?K.filter(p=>p.title.toLowerCase().includes(_closedAssignsSearch)):K,X=3,R=_closedAssignsExpanded?N:N.slice(0,X),it=N.length-R.length,st=K.length===0?"":`
    <div class="stats-section-card">
