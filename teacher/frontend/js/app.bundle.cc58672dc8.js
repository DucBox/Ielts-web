...
`),s=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=t,a.click(),URL.revokeObjectURL(o)}function buildStudentCredentialsFilename(t="student_accounts"){const e=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");return`${t}_${e}.csv`}function openStudentCredentialsModal(t,e,n="student_accounts"){const i=(Array.isArray(e)?e:[]).map(s=>({full_name:String(s?.full_name||"").trim(),username:String(s?.username||"").trim(),password:String(s?.password||"")})).filter(s=>s.full_name&&s.username&&s.password);_oneTimeStudentCredentials=i.length>0?{rows:i,fileName:buildStudentCredentialsFilename(n)}:null,openModal(t,`
    ...
    <div class="table-wrap">
      <table>
        <thead><tr>
          <th class="sortable" data-inbox-col="skill" onclick="sortInbox('skill')">K\u1EF9 n\u0103ng ${t("skill")}</th>
          <th class="sortable" data-inbox-col="student_name" onclick="sortInbox('student_name')">H\u1ECDc sinh ${t("student_name")}</th>
          <th>B\xE0i t\u1EADp</th>
          <th class="sortable" data-inbox-col="class_name" onclick="sortInbox('class_name')">L\u1EDBp ${t("class_name")}</th>
          <th class="sortable" data-inbox-col="submitted_at" onclick="sortInbox('submitted_at')">Th\u1EDDi gian n\u1ED9p ${t("submitted_at")}</th>
          <th></th>
        </tr></thead>
    ...
    <div class="table-wrap">
      <table>
        <thead><tr>
          <th class="sortable" data-graded-col="skill" onclick="sortInboxGraded('skill')">K\u1EF9 n\u0103ng ${e("skill")}</th>
          <th class="sortable" data-graded-col="student_name" onclick="sortInboxGraded('student_name')">H\u1ECDc sinh ${e("student_name")}</th>
          <th>B\xE0i t\u1EADp</th>
          <th class="sortable" data-graded-col="class_name" onclick="sortInboxGraded('class_name')">L\u1EDBp ${e("class_name")}</th>
          <th class="sortable" data-graded-col="submitted_at" onclick="sortInboxGraded('submitted_at')">Th\u1EDDi gian n\u1ED9p ${e("submitted_at")}</th>
          <th class="sortable" data-graded-col="overall_score" onclick="sortInboxGraded('overall_score')">\u0110i\u1EC3m ${e("overall_score")}</th>
          <th></th>
        </tr></thead>
    ...
    </div>`,i=e.map(r=>{const c=r.student_count>0?Math.round(r.submitted_student_count/r.student_count*100):0,d=r.upcoming_deadline_count>0?`<span class="card-deadline-chip">\u26A0\uFE0F ${r.upcoming_deadline_count} b\xE0i s\u1EAFp h\u1EA1n</span>`:"",p=r.pending_grading_count>0?`<span class="card-pending-chip" title="B\xE0i Writing/Speaking ch\u01B0a ch\u1EA5m">\u{1F4DD} ${r.pending_grading_count} c\u1EA7n ch\u1EA5m</span>`:"";return`
    ...
    </div>`}window.showHistogramStudents=showHistogramStudents;function renderStatsTab(t,e){destroyStatsCharts();const{timeline:n,per_student:i,per_assignment:s}=e,o=_statsSkillFilter,a=_statsStatusFilter,l=_statsModeFilter,r=_statsScaleFilter,c=s.filter(u=>!(o&&u.skill!==o||a==="active"&&!u.is_active||a==="closed"&&u.is_active||l&&u.mode!==l||r&&(u.scoring_scale||"10")!==r)),d=new Set(c.map(u=>u.id)),p=u=>u.length?u.reduce((f,v)=>f+v,0)/u.length:null,m=i.length,g=c.length,h=c.filter(u=>u.is_active).length,b=g-h,T=new Map;i.forEach(u=>{u.submissions.forEach(f=>{if(!d.has(f.assignment_id))return;const v=`${u.id||u.student_id}:${f.assignment_id}`,y=T.get(v);(!y||(f.attempt_number||1)>(y.attempt_number||1))&&T.set(v,{...f,student_name:u.name})})});const w=Array.from(T.values()),L=w.filter(u=>u.overall_score!==null);_statsAllScoredSubs=L;const E=p(L.map(u=>Number(u.overall_score))),I=g*m,N=I>0?Math.round(w.length/I*100):0,H=[0,0,0,0,0];for(const u of L){const f=Number(u.overall_score),v=f>=9?4:Math.min(4,Math.floor(f/2));H[v]++}const F=["reading","listening","writing","speaking"],j={},O={};for(const u of F){const f=L.filter(x=>x.skill===u);j[u]=p(f.map(x=>Number(x.overall_score)));const v=c.filter(x=>x.skill===u),y=w.filter(x=>x.skill===u),M=v.length*m;O[u]={count:v.length,submitted:y.length,pct:M>0?Math.round(y.length/M*100):0}}let P=i.map(u=>{const f=u.submissions.filter(S=>d.has(S.assignment_id)),v=new Map;f.forEach(S=>{const k=v.get(S.assignment_id);(!k||(S.attempt_number||1)>(k.attempt_number||1))&&v.set(S.assignment_id,S)});const y=Array.from(v.values()),M=y.filter(S=>S.overall_score!==null),x=S=>p(M.filter(k=>k.skill===S).map(k=>Number(k.overall_score))),_=y.filter(S=>!S.is_active&&S.deadline),C=_.filter(S=>S.on_time).length;return{...u,submitted:y.length,total:g,avg_score:o?x(o):p(M.map(S=>Number(S.overall_score))),avg_reading:x("reading"),avg_listening:x("listening"),avg_writing:x("writing"),avg_speaking:x("speaking"),on_time:C,closed_total:_.length,on_time_rate:_.length>0?C/_.length:null}});_statsSortCol&&(P=[...P].sort((u,f)=>{let v=u[_statsSortCol],y=f[_statsSortCol];return v==null?1:y==null?-1:typeof v=="string"?_statsSortDir==="asc"?v.localeCompare(y):y.localeCompare(v):_statsSortDir==="asc"?v-y:y-v}));const q=u=>u!=null?Number(u).toFixed(1):"\u2014",W=(u,f)=>f>0?Math.round(u/f*100):0,B=u=>_statsSortCol!==u?'<span class="sort-icon">\u2195</span>':`<span class="sort-icon active">${_statsSortDir==="asc"?"\u2191":"\u2193"}</span>`,Q={reading:"#3b82f6",listening:"#f59e0b",writing:"#8b5cf6",speaking:"#22c55e"},z=["#3b82f6","#ef4444","#22c55e","#f59e0b","#8b5cf6","#06b6d4","#ec4899","#14b8a6","#f97316","#a3e635"],U={reading:"Reading",listening:"Listening",writing:"Writing",speaking:"Speaking",composite:"T\u1ED5ng h\u1EE3p"},Y=`
    <div class="stats-cards-grid">
      ...
      <div class="stats-card">
        ...
        <div class="stats-card-body">
          ...
          <div class="stats-card-label">${h} \u0111ang m\u1EDF \xB7 ${b} \u0111\xE3 \u0111\xF3ng</div>
        </div>
      </div>
      <div class="stats-card">
        ...
        <div class="stats-card-body">
          ...
          <div class="stats-card-label">${w.length} / ${I} l\u01B0\u1EE3t n\u1ED9p</div>
        </div>
      </div>
      <div class="stats-card">
    ...
    <div class="stats-section-card">
      ...
      <div class="stats-skill-chart">
        ${["reading","listening","writing","speaking"].map(u=>{const f=O[u];return!f||f.count===0?"":`<div class="stats-skill-row">
            <div class="stats-skill-label">${skillBadge(u)}</div>
            <div class="stats-bar-wrap">
              <div class="stats-bar-fill" style="width:${f.pct}%;background:${Q[u]}"></div>
            </div>
    ...
    </div>`,K=c.filter(u=>!u.is_active&&u.deadline),R=_closedAssignsSearch?K.filter(u=>u.title.toLowerCase().includes(_closedAssignsSearch)):K,X=3,D=_closedAssignsExpanded?R:R.slice(0,X),it=R.length-D.length,st=K.length===0?"":`
    <div class="stats-section-card">
      ...
      <div class="stats-trend-filters">
        <div class="stats-filter-group">
          ...
          <div class="stats-filter-pills">
            ${[["reading","Reading"],["listening","Listening"],["writing","Writing"],["speaking","Speaking"]].map(([u,f])=>`
              <button class="stats-filter-pill trend-skill-pill${_statsTrendSkill===u?" active":""}"
                data-skill="${u}" onclick="filterTrendSkill('${u}')">
                ${f}
              </button>`).join("")}
          </div>
        </div>
        ...
        <div class="stats-student-toggles" id="stats-trend-toggles" style="margin-bottom:0">
          ${i.map((u,f)=>`
            <button class="stats-student-toggle active"
              ...
              onclick="toggleTrendStudent('${u.id}')">
              ${escapeHtml(u.name)}
            </button>`).join("")}
        </div>
      </div>
    ...
    <div class="stats-filter-bar">
      <div class="stats-filter-group">
        ...
        <div class="stats-filter-pills">
          ${[["ielts","\u{1F3AF} IELTS Test"],["10","\u{1F4CA} Practice Test"],["composite","\u{1F9E9} Mixed Skills"]].map(([u,f])=>`
            <button class="stats-filter-pill${_statsScaleFilter===u?" active":""}"
              onclick="_statsScaleFilter='${u}';applyStatsFilter()">
              ${f}
            </button>`).join("")}
        </div>
      </div>
      <div class="stats-filter-group">
        <span class="stats-filter-label">K\u1EF9 n\u0103ng:</span>
        <div class="stats-filter-pills">
          ${["",...FILTERABLE_ASSIGNMENT_SKILLS].map(u=>`
            <button class="stats-filter-pill${_statsSkillFilter===u?" active":""}"
              onclick="_statsSkillFilter='${u}';applyStatsFilter()">
              ${u?U[u]:"T\u1EA5t c\u1EA3"}
            </button>`).join("")}
        </div>
      </div>
      <div class="stats-filter-group">
        ...
        <div class="stats-filter-pills">
          ${[["","T\u1EA5t c\u1EA3"],["active","\u0110ang m\u1EDF"],["closed","\u0110\xE3 \u0111\xF3ng"]].map(([u,f])=>`
            <button class="stats-filter-pill${_statsStatusFilter===u?" active":""}"
              onclick="_statsStatusFilter='${u}';applyStatsFilter()">
              ${f}
            </button>`).join("")}
        </div>
      </div>
      <div class="stats-filter-group">
        ...
        <div class="stats-filter-pills">
          ${[["","T\u1EA5t c\u1EA3"],["exam","\u{1F4DD} Ki\u1EC3m tra"],["practice","\u{1F3A7} Luy\u1EC7n t\u1EADp"]].map(([u,f])=>`
            <button class="stats-filter-pill${_statsModeFilter===u?" active":""}"
              onclick="_statsModeFilter='${u}';applyStatsFilter()">
              ${f}
            </button>`).join("")}
        </div>
      </div>
    ...
    <div class="stats-section-card">
      ...
      ${P.length===0?'<div class="empty-state" style="padding:20px">Kh\xF4ng c\xF3 d\u1EEF li\u1EC7u</div>':`<div class="table-wrap">
          <table class="stats-table">
            <thead><tr>
              <th class="sortable" onclick="sortStudentTable('name')">H\u1ECDc sinh ${B("name")}</th>
              <th class="sortable" onclick="sortStudentTable('submitted')">\u0110\xE3 n\u1ED9p ${B("submitted")}</th>
              <th class="sortable" onclick="sortStudentTable('avg_score')">\u0110i\u1EC3m TB ${B("avg_score")}</th>
              ${o?"":`
                <th class="sortable" style="color:#3b82f6" onclick="sortStudentTable('avg_reading')">Reading ${B("avg_reading")}</th>
                <th class="sortable" style="color:#f59e0b" onclick="sortStudentTable('avg_listening')">Listening ${B("avg_listening")}</th>
                <th class="sortable" style="color:#8b5cf6" onclick="sortStudentTable('avg_writing')">Writing ${B("avg_writing")}</th>
                <th class="sortable" style="color:#22c55e" onclick="sortStudentTable('avg_speaking')">Speaking ${B("avg_speaking")}</th>`}
              <th class="sortable" onclick="sortStudentTable('on_time_rate')">\u0110\xFAng h\u1EA1n ${B("on_time_rate")}</th>
              <th></th>
            </tr></thead>
            <tbody>
              ${P.map(u=>{const f=W(u.submitted,u.total),v=u.on_time_rate!==null?Math.round(u.on_time_rate*100):null;return`<tr>
                  <td><span class="student-avatar">${escapeHtml(u.name.charAt(0).toUpperCase())}</span> ${escapeHtml(u.name)}</td>
                  <td>
                    <div class="stats-mini-bar-wrap">
                      <div class="stats-mini-bar" style="width:${f}%"></div>
                    </div>
                    <span class="stats-mini-label">${u.submitted}/${u.total}</span>
                  </td>
                  <td><span class="stats-score-badge">${q(u.avg_score)}</span></td>
                  ${o?"":`
                    <td style="color:#3b82f6">${q(u.avg_reading)}</td>
                    <td style="color:#f59e0b">${q(u.avg_listening)}</td>
                    <td style="color:#8b5cf6">${q(u.avg_writing)}</td>
                    <td style="color:#22c55e">${q(u.avg_speaking)}</td>`}
                  <td>${v!==null?`<span class="stats-ontime-pill ${v>=80?"good":v>=50?"mid":"bad"}">${v}%</span>`:'<span style="color:var(--gray-400)">\u2014</span>'}</td>
                  <td>
                    <button class="btn btn-sm btn-outline" onclick="toggleStudentStatsRow('${u.id}')">Chi ti\u1EBFt</button>
                  </td>
                </tr>
                <tr id="stats-row-${u.id}" style="display:none">
                  <td colspan="${lt}" style="padding:0">
                    <div class="stats-expand-body">
                      <table class="stats-sub-table">
                            <thead id="stats-sub-thead-${u.id}"><tr>
                              <th class="sortable" onclick="sortStatsSubTable('${u.id}','title')">B\xE0i t\u1EADp ${makeSortIcon("title",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${u.id}','skill')">K\u1EF9 n\u0103ng ${makeSortIcon("skill",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${u.id}','score')">\u0110i\u1EC3m ${makeSortIcon("score",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${u.id}','submitted_at')">Ng\xE0y n\u1ED9p ${makeSortIcon("submitted_at",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th class="sortable" onclick="sortStatsSubTable('${u.id}','on_time')">\u0110\xFAng h\u1EA1n ${makeSortIcon("on_time",_statsSubSortCol,_statsSubSortDir)}</th>
                              <th>Overtime</th>
                            </tr></thead>
                            <tbody id="stats-sub-tbody-${u.id}">${buildStatsSubRows(u.submissions.filter(y=>d.has(y.assignment_id)))}</tbody>
                          </table>
                    </div>
                  </td>
                </tr>`}).join("")}
    ...
    </div>`,A=u=>makeSortIcon(u,_assignTableSortCol,_assignTableSortDir),G=[...c];_assignTableSortCol&&G.sort((u,f)=>{let v,y;switch(_assignTableSortCol){case"title":v=u.title.toLowerCase(),y=f.title.toLowerCase();break;case"skill":v=u.skill||"",y=f.skill||"";break;case"mode":v=u.mode||"",y=f.mode||"";break;case"submitted_rate":v=u.total?u.submitted/u.total:0,y=f.total?f.submitted/f.total:0;break;case"avg_score":v=u.avg_score??-1,y=f.avg_score??-1;break;case"on_time":v=u.on_time??-1,y=f.on_time??-1;break;case"late":v=u.late??-1,y=f.late??-1;break;case"missing":v=u.missing??-1,y=f.missing??-1;break;case"is_active":v=u.is_active?1:0,y=f.is_active?1:0;break;default:return 0}return v<y?_assignTableSortDir==="asc"?-1:1:v>y?_assignTableSortDir==="asc"?1:-1:0});const ct=`
    <div class="stats-section-card">
      ...
      ${c.length===0?'<div class="empty-state" style="padding:20px">Kh\xF4ng c\xF3 b\xE0i t\u1EADp ph\xF9 h\u1EE3p</div>':`<div class="table-wrap">
          <table class="stats-table">
            <thead><tr>
              <th class="sortable" onclick="sortAssignTable('skill')">K\u1EF9 n\u0103ng ${A("skill")}</th>
              <th class="sortable" onclick="sortAssignTable('title')">T\xEAn b\xE0i t\u1EADp ${A("title")}</th>
              <th class="sortable" onclick="sortAssignTable('mode')">Ch\u1EBF \u0111\u1ED9 ${A("mode")}</th>
              <th>Th\u1EDDi gian</th>
              <th class="sortable" onclick="sortAssignTable('submitted_rate')">T\u1EF7 l\u1EC7 n\u1ED9p ${A("submitted_rate")}</th>
              <th class="sortable" onclick="sortAssignTable('avg_score')">\u0110i\u1EC3m TB ${A("avg_score")}</th>
              <th class="sortable" onclick="sortAssignTable('on_time')">\u0110\xFAng h\u1EA1n ${A("on_time")}</th>
              <th class="sortable" onclick="sortAssignTable('late')">Mu\u1ED9n ${A("late")}</th>
              <th class="sortable" onclick="sortAssignTable('missing')">Ch\u01B0a n\u1ED9p ${A("missing")}</th>
              <th class="sortable" onclick="sortAssignTable('is_active')">Tr\u1EA1ng th\xE1i ${A("is_active")}</th>
            </tr></thead>
            <tbody>
              ${G.map(u=>{const f=W(u.submitted,u.total);return`<tr>
                  <td>${skillBadge(u.skill)}</td>
                  <td style="font-weight:600">${escapeHtml(u.title)}</td>
                  <td>${u.mode==="practice"?'<span class="stats-mode-chip practice">\u{1F3A7} Luy\u1EC7n t\u1EADp</span>':'<span class="stats-mode-chip exam">\u{1F4DD} Ki\u1EC3m tra</span>'}</td>
                  <td>${u.time_limit_minutes?`${u.time_limit_minutes} ph\xFAt`:'<span style="color:var(--text-muted)">\u2014</span>'}</td>
                  <td>
                    <div class="stats-mini-bar-wrap">
                      <div class="stats-mini-bar" style="width:${f}%"></div>
                    </div>
                  ...
                  <td><span class="stats-score-badge">${q(u.avg_score)}</span></td>
                  <td>${!u.is_active&&u.deadline?`<span style="color:var(--success)">${u.on_time}</span>`:"\u2014"}</td>
                  <td>${!u.is_active&&u.deadline?`<span style="color:var(--accent)">${u.late}</span>`:"\u2014"}</td>
                  <td>${u.missing!==null?`<span style="color:var(--danger)">${u.missing}</span>`:"\u2014"}</td>
                  <td>${u.is_active?'<span class="badge badge-success">\u0110ang m\u1EDF</span>':'<span class="badge badge-gray">\u0110\xE3 \u0111\xF3ng</span>'}</td>
                </tr>`}).join("")}
            </tbody>
          </table>
        </div>`}
    ...
    ${Z}
    ${rt}
    ${ct}`,ensureChartJsLoaded().then(()=>requestAnimationFrame(()=>{const u=document.getElementById("chart-skill-score");if(u){const x=["reading","listening","writing","speaking"].filter(_=>j[_]!==null);if(x.length>0){const _=new Chart(u.getContext("2d"),{type:"bar",data:{labels:x.map(C=>U[C]),datasets:[{data:x.map(C=>Number(j[C]).toFixed(2)),backgroundColor:x.map(C=>Q[C]+"cc"),borderColor:x.map(C=>Q[C]),borderWidth:1,borderRadius:6}]},options:{indexAxis:"y",responsive:!0,plugins:{legend:{display:!1}},scales:{x:{min:0,max:9,grid:{color:"#f3f4f6"},ticks:{font:{size:11}}},y:{ticks:{font:{size:12}}}}}});_statsCharts.push(_)}else u.insertAdjacentHTML("afterend",'<p style="color:var(--gray-400);font-size:13px;padding:20px 0">Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u \u0111i\u1EC3m</p>'),u.remove()}const f=document.getElementById("chart-score-dist");if(f)if(H.reduce((_,C)=>_+C,0)>0){const _=new Chart(f.getContext("2d"),{type:"bar",data:{labels:["0 \u2013 2","2 \u2013 4","4 \u2013 6","6 \u2013 8","8 \u2013 9"],datasets:[{label:"S\u1ED1 b\xE0i",data:H,backgroundColor:["#fca5a5","#fcd34d","#86efac","#67e8f9","#6ee7b7"],hoverBackgroundColor:["#f87171","#fbbf24","#4ade80","#22d3ee","#34d399"],borderRadius:6}]},options:{responsive:!0,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,ticks:{stepSize:1,font:{size:11}},grid:{color:"#f3f4f6"}},x:{ticks:{font:{size:11}}}},onClick:(C,S)=>{S.length&&showHistogramStudents(S[0].index)},onHover:(C,S)=>{C.native.target.style.cursor=S.length?"pointer":"default"}}});_statsCharts.push(_)}else f.insertAdjacentHTML("afterend",'<p style="color:var(--gray-400);font-size:13px;padding:20px 0">Ch\u01B0a c\xF3 b\xE0i n\xE0o \u0111\u01B0\u1EE3c ch\u1EA5m \u0111i\u1EC3m</p>'),f.remove();const v=document.getElementById("chart-trend");if(v){const x=[...s].reverse().filter(k=>!(_statsTrendSkill&&k.skill!==_statsTrendSkill||_statsModeFilter&&k.mode!==_statsModeFilter||_statsScaleFilter&&(k.scoring_scale||"10")!==_statsScaleFilter)),_=x.map(k=>k.title.length>18?k.title.slice(0,16)+"\u2026":k.title),C=i.map((k,dt)=>{const J=z[dt%z.length];return{label:k.name,_studentId:k.id,data:x.map(ut=>{const V=k.submissions.find(pt=>pt.assignment_id===ut.id);return V&&V.overall_score!==null?Number(V.overall_score):null}),borderColor:J,backgroundColor:J+"22",borderWidth:2,pointRadius:5,pointHoverRadius:7,fill:!1,tension:.3,spanGaps:!0}});if(x.length<1){v.style.display="none";const k=document.getElementById("trend-empty-msg");k&&(k.style.display="")}const S=new Chart(v.getContext("2d"),{type:"line",data:{labels:_,datasets:C},options:{responsive:!0,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!1},tooltip:{filter:k=>k.raw!==null,callbacks:{title:k=>k[0]?.label||"",label:k=>` ${k.dataset.label}: ${k.raw!==null?Number(k.raw).toFixed(1):"\u2014"}`}}},scales:{y:{min:0,max:9,ticks:{font:{size:11}},grid:{color:"#f3f4f6"}},x:{ticks:{font:{size:10},maxRotation:30}}}}});_statsCharts.push(S)}const y=document.getElementById("chart-timeline");if(y&&n.length>=2){const x=new Chart(y.getContext("2d"),{type:"line",data:{labels:n.map(_=>{const C=new Date(_.week);return`${C.getDate()}/${C.getMonth()+1}`}),datasets:[{label:"L\u01B0\u1EE3t n\u1ED9p",data:n.map(_=>_.count),borderColor:"#0f766e",backgroundColor:"#0f766e22",borderWidth:2,pointRadius:4,fill:!0,tension:.3}]},options:{responsive:!0,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,ticks:{stepSize:1,font:{size:11}},grid:{color:"#f3f4f6"}},x:{ticks:{font:{size:11}}}}}});_statsCharts.push(x)}const M=document.getElementById("chart-ontime");if(M&&D.length>0){const x=new Chart(M.getContext("2d"),{type:"bar",data:{labels:D.map(_=>_.title.length>22?_.title.slice(0,20)+"\u2026":_.title),datasets:[{label:"\u0110\xFAng h\u1EA1n",data:D.map(_=>_.on_time),backgroundColor:"#86efac",borderRadius:4},{label:"N\u1ED9p mu\u1ED9n",data:D.map(_=>_.late),backgroundColor:"#fcd34d",borderRadius:4},{label:"Ch\u01B0a n\u1ED9p",data:D.map(_=>_.missing||0),backgroundColor:"#fca5a5",borderRadius:4}]},options:{indexAxis:"y",responsive:!0,plugins:{legend:{position:"bottom",labels:{font:{size:11}}}},scales:{x:{stacked:!0,ticks:{stepSize:1,font:{size:11}},grid:{color:"#f3f4f6"}},y:{stacked:!0,ticks:{font:{size:11}}}}}});_statsCharts.push(x)}})).catch(()=>{const u=t.querySelector(".stats-charts-row");u&&u.insertAdjacentHTML("beforebegin",'<div class="empty-state" style="margin-bottom:16px"><p>Kh\xF4ng th\u1EC3 t\u1EA3i bi\u1EC3u \u0111\u1ED3 l\xFAc n\xE0y. B\u1EA1n v\u1EABn c\xF3 th\u1EC3 xem d\u1EEF li\u1EC7u d\u1EA1ng b\u1EA3ng b\xEAn d\u01B0\u1EDBi.</p></div>')})}function toggleStudentStatsRow(t){const e=document.getElementById(`stats-row-${t}`);e&&(e.style.display=e.style.display==="none"?"":"none")}window.toggleStudentStatsRow=toggleStudentStatsRow;function buildStatsSubRows(t){return t.length===0?'<tr><td colspan="5" style="color:var(--gray-400);padding:12px">Ch\u01B0a n\u1ED9p b\xE0i n\xE0o</td></tr>':(_statsSubSortCol?[...t].sort((n,i)=>{let s,o;if(_statsSubSortCol==="title")s=(n.assignment_title||"").toLowerCase(),o=(i.assignment_title||"").toLowerCase();else if(_statsSubSortCol==="skill")s=n.skill||"",o=i.skill||"";else if(_statsSubSortCol==="score")s=n.overall_score??-1,o=i.overall_score??-1;else if(_statsSubSortCol==="submitted_at")s=n.submitted_at||"",o=i.submitted_at||"";else if(_statsSubSortCol==="on_time")s=n.on_time===null?-1:n.on_time?1:0,o=i.on_time===null?-1:i.on_time?1:0;else return 0;return s<o?_statsSubSortDir==="asc"?-1:1:s>o?_statsSubSortDir==="asc"?1:-1:0}):t).map(n=>`<tr>
      ...
      <div class="table-wrap assign-table-wrap">
        <table>
          <thead><tr>
            ...
            <th class="sortable" id="assign-th-title" onclick="sortAssignList('title')">T\xEAn b\xE0i t\u1EADp</th>
            <th>\u0110\u1EC1</th>
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            ...
            <th class="sortable" id="student-th-name" onclick="sortClassStudentsTable('full_name')">H\u1ECD t\xEAn</th>
...
`),s=new Blob([i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=`students_${t}.csv`,a.click(),URL.revokeObjectURL(o),toast(`\u0110\xE3 xu\u1EA5t ${n.length} h\u1ECDc sinh ra CSV`)}window.bulkExportCSV=bulkExportCSV;function buildStudentRows(t,e){return t.map(n=>`
  ...
  `)}window.changeDeadline=changeDeadline;async function saveDeadline(t,e){const n=$("#new-deadline-input")?.value;if(!n){toast("Vui l\xF2ng ch\u1ECDn th\u1EDDi gian","error");return}const i=new Date(n).toISOString();btnLoading(e);try{await api.patch(`/assignments/${t}`,{deadline:i}),closeModal(),toast("\u0110\xE3 c\u1EADp nh\u1EADt h\u1EA1n n\u1ED9p"),_cachedCls?.id&&await showClassDetail({id:_cachedCls.id})}catch(s){toast("L\u1ED7i: "+(s.error||s.message),"error"),btnLoading(e,!1)}}window.saveDeadline=saveDeadline;async function deleteAssignment(t,e,n){if(confirm("Xo\xE1 b\xE0i t\u1EADp n\xE0y? T\u1EA5t c\u1EA3 b\xE0i n\u1ED9p s\u1EBD b\u1ECB xo\xE1 theo.")){btnLoading(n);try{await api.delete(`/assignments/${t}`),toast("\u0110\xE3 xo\xE1 b\xE0i t\u1EADp"),showClassDetail({id:e})}catch(i){btnReset(n),toast("L\u1ED7i xo\xE1: "+(i.error||i.message),"error")}}}async function showAssignmentSubmissions({id:t}){_submissionsSortCol="",_submissionsSortDir="desc",setLoading("\u0110ang t\u1EA3i danh s\xE1ch b\xE0i n\u1ED9p...");try{const{assignment:e,students:n}=await api.get(`/assignments/${t}/submissions`);renderAssignmentSubmissions(e,n)}catch(e){toast("L\u1ED7i t\u1EA3i d\u1EEF li\u1EC7u: "+(e.error||e.message),"error"),renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c danh s\xE1ch b\xE0i n\u1ED9p",e,`/assignment/${t}`)}}function exportSubmissionsCSV(t,e){const n=["H\u1ECD t\xEAn","Username","Tr\u1EA1ng th\xE1i","\u0110i\u1EC3m","Th\u1EDDi gian n\u1ED9p"],i=e.map(r=>[r.full_name,r.username,r.submission_id?"\u0110\xE3 n\u1ED9p":"Ch\u01B0a n\u1ED9p",r.overall_score!=null?r.overall_score:"",r.submitted_at?formatDateTime(r.submitted_at):""]),s=[n,...i].map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join(`
`),o=new Blob(["\uFEFF"+s],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(o),l=document.createElement("a");l.href=a,l.download=`${t.title.replace(/[^a-zA-Z0-9_\-]/g,"_")}_diem.csv`,l.click(),URL.revokeObjectURL(a)}window.exportSubmissionsCSV=exportSubmissionsCSV;function buildSubmissionRows(t,e){return t.length===0?`<tr><td colspan="5">
    ...
    <div class="table-wrap">
      <table>
        <thead>
          <tr>
            <th class="sortable" data-sub-col="full_name" onclick="sortSubmissionsTable('full_name')">H\u1ECDc sinh ${o("full_name")}</th>
            <th class="sortable" data-sub-col="status" onclick="sortSubmissionsTable('status')">Tr\u1EA1ng th\xE1i ${o("status")}</th>
            <th class="sortable" data-sub-col="score" onclick="sortSubmissionsTable('score')">\u0110i\u1EC3m ${o("score")}</th>
            <th class="sortable" data-sub-col="submitted_at" onclick="sortSubmissionsTable('submitted_at')">Th\u1EDDi gian n\u1ED9p ${o("submitted_at")}</th>
            <th>B\xE0i l\xE0m</th>
          </tr>
        </thead>
        <tbody>${a}</tbody>
        ...
        <div style="margin-bottom:16px;padding:12px;background:var(--gray-50);border-radius:12px;border:1px solid var(--gray-200)">
          ...
          ${p.map((g,h)=>`
            <div style="${m?"margin-bottom:10px":""}">
              ${m?`<div style="font-size:12px;color:var(--gray-500);margin-bottom:4px">${escapeHtml(g.name||"Ph\u1EA7n "+(h+1))} <span id="audio-dur-${h}" style="color:var(--gray-400)"></span></div>`:""}
              ${h===0?'<div id="waveform-container" class="waveform-container"><div class="waveform-loading">\u0110ang t\u1EA3i waveform...</div></div>':""}
              <audio ${h===0?'id="waveform-audio"':`id="track-audio-${h}"`} controls src="${escapeHtml(g.url||"")}" preload="metadata" style="width:100%;height:36px;outline:none;${h===0?"margin-top:6px":""}"></audio>
            </div>`).join("")}
        </div>`)}$("#app").innerHTML=`
    <nav class="breadcrumb">
      <a class="breadcrumb-item" onclick="navigate('/classes')">L\u1EDBp h\u1ECDc</a>
          ...
          ${a.map(p=>`
            <div class="prev-attempt-card">
              <div class="prev-attempt-header">
                <span class="prev-attempt-label">L\u1EA7n ${p.attempt_number}</span>
                <span class="prev-attempt-score">${p.overall_score!=null?`${p.overall_score} Band`:"Ch\u01B0a c\xF3 \u0111i\u1EC3m"}</span>
                <span class="prev-attempt-date">${formatDateTime(p.submitted_at)}</span>
              </div>
              ${p.teacher_feedback?.overall?`<div class="prev-attempt-overall">${escapeHtml(p.teacher_feedback.overall)}</div>`:'<div class="prev-attempt-overall" style="color:var(--gray-400);font-style:italic">Kh\xF4ng c\xF3 nh\u1EADn x\xE9t t\u1ED5ng</div>'}
            </div>`).join("")}
        </div>`:""}
  ...
  `}function _buildFolderTreeItems(t,e){return _allFolders.filter(n=>n.parent_id===t).sort((n,i)=>n.display_order-i.display_order||n.name.localeCompare(i.name)).map(n=>{const i=_getFolderSubtreeIds(n.id),s=_allQuestions.filter(r=>i.has(r.folder_id)).length,o=_currentFolderFilter===n.id,a=_allFolders.some(r=>r.parent_id===n.id),l=escapeHtml(n.name).replace(/'/g,"&#39;");return`
        ...
        ${_buildFolderTreeItems(n.id,e+1)}`}).join("")}function setFolderFilter(t){_currentFolderFilter=t,renderQuestions()}window.setFolderFilter=setFolderFilter;async function createFolderPrompt(t){const e=window.prompt(t?"T\xEAn th\u01B0 m\u1EE5c con:":"T\xEAn th\u01B0 m\u1EE5c m\u1EDBi:");if(e?.trim())try{const n=await api.post("/question-folders",{name:e.trim(),parent_id:t});_allFolders.push(n),renderQuestions(),toast('\u0110\xE3 t\u1EA1o th\u01B0 m\u1EE5c "'+n.name+'"')}catch(n){toast("L\u1ED7i: "+(n.error||n.message),"error")}}window.createFolderPrompt=createFolderPrompt;async function renameFolderPrompt(t,e){const n=window.prompt("\u0110\u1ED5i t\xEAn th\u01B0 m\u1EE5c:",e);if(!(!n?.trim()||n.trim()===e))try{const i=await api.patch(`/question-folders/${t}`,{name:n.trim()}),s=_allFolders.findIndex(o=>o.id===t);s>=0&&(_allFolders[s]=i),renderQuestions()}catch(i){toast("L\u1ED7i: "+(i.error||i.message),"error")}}window.renameFolderPrompt=renameFolderPrompt;async function deleteFolderConfirm(t,e){const n=_allFolders.filter(o=>o.parent_id===t).length,i=_allQuestions.filter(o=>o.folder_id===t).length;let s=`Xo\xE1 th\u01B0 m\u1EE5c "${e}"?`;if(n>0&&(s+=`
\u2022 ${n} th\u01B0 m\u1EE5c con c\u0169ng s\u1EBD b\u1ECB xo\xE1.`),i>0&&(s+=`
\u2022 ${i} \u0111\u1EC1 s\u1EBD \u0111\u01B0\u1EE3c chuy\u1EC3n v\u1EC1 Ch\u01B0a ph\xE2n lo\u1EA1i.`),!!confirm(s))try{await api.delete(`/question-folders/${t}`);const o=_getFolderSubtreeIds(t);_allFolders=_allFolders.filter(a=>!o.has(a.id)),_allQuestions.forEach(a=>{o.has(a.folder_id)&&(a.folder_id=null)}),o.has(_currentFolderFilter)&&(_currentFolderFilter=null),renderQuestions(),toast("\u0110\xE3 xo\xE1 th\u01B0 m\u1EE5c")}catch(o){toast("L\u1ED7i: "+(o.error||o.message),"error")}}window.deleteFolderConfirm=deleteFolderConfirm;function openMoveQuestionModal(t){const e=_allQuestions.find(i=>i.id===t);if(!e)return;const n=(i,s)=>_allFolders.filter(o=>o.parent_id===i).sort((o,a)=>o.display_order-a.display_order||o.name.localeCompare(a.name)).map(o=>`
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class="sortable" data-q-col="title" onclick="sortQuestions('title')">Ti\xEAu \u0111\u1EC1 <span style="font-size:11px;font-weight:400;color:var(--text-muted)">(click \u0111\u1EC3 xem nhanh)</span> ${makeSortIcon("title",_questionSortCol,_questionSortDir)}</th>
                <th>Tags</th>
                <th>Chi ti\u1EBFt</th>
                <th class="sortable" data-q-col="created_at" onclick="sortQuestions('created_at')">Ng\xE0y t\u1EA1o ${makeSortIcon("created_at",_questionSortCol,_questionSortDir)}</th>
                <th>Thao t\xE1c</th>
              </tr>
            </thead>
...
`)){const s=i.match(/^([^:\n]+?):\s/);s&&!e.has(s[1])&&(e.add(s[1]),n.push(s[1]))}return n}function _nextSpeakerLabel(){const t=new Set(_speakerNames.map(e=>e.original));for(const e of"ABCDEFGHIJKLMNOPQRSTUVWXYZ")if(!t.has(e))return e;return"?"}function _hasSpeakerPattern(t){return/^[A-Za-z][^:\n]*:\s/m.test(t)}function _refreshSpeakerNames(){const t=$("#listening-script");if(!t||!_hasSpeakerPattern(t.value))return;const e=_parseSpeakersFromTranscript(t.value),n=new Map(_speakerNames.map(i=>[i.original,i]));_speakerNames=e.map(i=>n.get(i)||{original:i,replace:""}),_speakerNames.length===0&&(_speakerNames=[{original:"A",replace:""},{original:"B",replace:""}])}function _renderSpeakerRenameUI(){const t=$("#speaker-rename-section");if(!t)return;const e=$("#listening-script");if(!(e&&_hasSpeakerPattern(e.value))){t.style.display="none";return}t.style.display="";const i=$("#speaker-rename-list");if(!i)return;const s="padding:4px 8px;border:1px solid var(--border);border-radius:5px;font-size:13px;background:var(--surface,var(--bg-subtle));color:var(--text);outline:none;width:100%";i.innerHTML=_speakerNames.map((o,a)=>`
    ...
    `}`),setTimeout(()=>{($("#stu-names")||$("#stu-existing-username"))?.focus()},50)}function switchStudentTab(t){_addStudentTab=t,renderAddStudentModal()}async function submitCreateStudent(t){const e=parseStudentNameLines($("#stu-names")?.value);if(e.length===0){toast("Vui l\xF2ng nh\u1EADp \xEDt nh\u1EA5t 1 h\u1ECDc sinh","error");return}btnLoading(t);try{const n=await api.post("/students",{class_id:_addStudentClassId,students:e.map(s=>({full_name:s}))}),i=Array.isArray(n.created)?n.created:[];if(i.length===0)throw new Error("Kh\xF4ng nh\u1EADn \u0111\u01B0\u1EE3c t\xE0i kho\u1EA3n \u0111\xE3 t\u1EA1o");closeModal(),openStudentCredentialsModal(i.length===1?"T\xE0i kho\u1EA3n h\u1ECDc sinh \u0111\xE3 \u0111\u01B0\u1EE3c t\u1EA1o":`\u0110\xE3 t\u1EA1o ${i.length} t\xE0i kho\u1EA3n h\u1ECDc sinh`,i,i.length===1?"student_account":"student_accounts"),toast(`\u0110\xE3 t\u1EA1o ${i.length} t\xE0i kho\u1EA3n h\u1ECDc sinh!`),showClassDetail({id:_addStudentClassId})}catch(n){btnReset(t),toast("L\u1ED7i: "+(n.error||n.message||"Kh\xF4ng th\u1EC3 t\u1EA1o h\u1ECDc sinh"),"error")}}async function submitAddExistingStudent(t){const e=$("#stu-existing-username")?.value.trim();if(!e){toast("Vui l\xF2ng nh\u1EADp username","error");return}btnLoading(t);try{await api.post("/student-classes",{class_id:_addStudentClassId,username:e}),closeModal(),toast("\u0110\xE3 th\xEAm h\u1ECDc sinh v\xE0o l\u1EDBp!"),showClassDetail({id:_addStudentClassId})}catch(n){btnReset(t),toast("L\u1ED7i: "+(n.error||"Kh\xF4ng th\u1EC3 th\xEAm h\u1ECDc sinh"),"error")}}function openResetPasswordModal(t,e,n){confirm(`C\u1EA5p m\u1EADt kh\u1EA9u m\u1EDBi cho "${e}"?

M\u1EADt kh\u1EA9u c\u0169 s\u1EBD h\u1EBFt hi\u1EC7u l\u1EF1c ngay sau khi \u0111\u1ED5i.`)&&submitResetPassword(t,n)}async function submitResetPassword(t,e){btnLoading(e);try{const n=await api.post(`/students/${t}/reset-password`,{});if(!n?.credentials)throw new Error("Kh\xF4ng nh\u1EADn \u0111\u01B0\u1EE3c m\u1EADt kh\u1EA9u m\u1EDBi");openStudentCredentialsModal("M\u1EADt kh\u1EA9u m\u1EDBi c\u1EE7a h\u1ECDc sinh",[n.credentials],"student_password_reset"),toast("\u0110\xE3 c\u1EA5p m\u1EADt kh\u1EA9u m\u1EDBi!")}catch(n){btnReset(e),toast("L\u1ED7i: "+(n.error||n.message||"Kh\xF4ng th\u1EC3 \u0111\u1ED5i m\u1EADt kh\u1EA9u"),"error");return}btnReset(e)}async function removeStudentFromClass(t,e,n){if(confirm(`Xo\xE1 h\u1ECDc sinh kh\u1ECFi l\u1EDBp n\xE0y?

T\xE0i kho\u1EA3n h\u1ECDc sinh v\u1EABn c\xF2n, ch\u1EC9 r\u1EDDi kh\u1ECFi l\u1EDBp n\xE0y.`)){btnLoading(n);try{await api.delete(`/student-classes?student_id=${t}&class_id=${e}`),toast("\u0110\xE3 xo\xE1 h\u1ECDc sinh kh\u1ECFi l\u1EDBp"),showClassDetail({id:e})}catch(i){btnReset(n),toast("L\u1ED7i: "+(i.error||i.message),"error");return}btnReset(n)}}async function showProfileFields(){setLoading("\u0110ang t\u1EA3i...");try{const t=await api.get("/profile-fields");renderProfileFieldsPage(t)}catch(t){toast("L\u1ED7i: "+(t.error||t.message),"error"),renderRouteError("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c h\u1ED3 s\u01A1 h\u1ECDc sinh",t,"/profile-fields")}}function renderProfileFieldsPage(t){const e={text:"V\u0103n b\u1EA3n ng\u1EAFn",textarea:"V\u0103n b\u1EA3n d\xE0i",select:"Ch\u1ECDn \u0111\xE1p \xE1n",date:"Ng\xE0y sinh"},n={notification_email:"Email th\xF4ng b\xE1o"},i=t.length===0?'<div class="pf-empty"><div class="pf-empty-icon">\u{1F4CB}</div><div>Ch\u01B0a c\xF3 c\xE2u h\u1ECFi n\xE0o. Th\xEAm c\xE2u h\u1ECFi \u0111\u1EA7u ti\xEAn b\xEAn tr\xEAn!</div></div>':`<div class="table-wrap"><table class="pf-table">
...
C\xE1c c\xE2u tr\u1EA3 l\u1EDDi c\u1EE7a h\u1ECDc sinh c\u0169ng s\u1EBD b\u1ECB xo\xE1.`))try{await api.delete(`/profile-fields/${t}`),toast("\u0110\xE3 xo\xE1 c\xE2u h\u1ECFi"),showProfileFields()}catch(e){toast("L\u1ED7i: "+(e.error||e.message),"error")}}window.deleteProfileField=deleteProfileField;async function openStudentProfileModal(t,e){openModal(`\u{1F464} H\u1ED3 s\u01A1 \u2014 ${e}`,'<div style="text-align:center;padding:24px;color:var(--gray-400)">\u0110ang t\u1EA3i...</div>');try{const{student:n,fields:i,answers:s}=await api.get(`/students/${t}/profile-answers`),o=i.length===0?`<div class="pf-modal-empty">
    ...
    </tr>`).join(""):'<tr><td colspan="7" style="text-align:center;color:var(--gray-400);padding:32px">Ch\u01B0a c\xF3 \u0111\u1EC1 n\xE0o.</td></tr>'}function setSharedSkillFilter(t){_sharedSkillFilter=t,renderSharedPool()}window.setSharedSkillFilter=setSharedSkillFilter;async function deleteSharedQuestion(t,e){if(confirm(`Xo\xE1 \u0111\u1EC1 "${e}" kh\u1ECFi Kho \u0111\u1EC1 luy\u1EC7n t\u1EADp?`))try{await api.delete(`/shared-pool/${t}`),_sharedQuestions=_sharedQuestions.filter(n=>n.id!==t),renderSharedPool(),toast("\u0110\xE3 xo\xE1 \u0111\u1EC1","success")}catch(n){toast("L\u1ED7i xo\xE1 \u0111\u1EC1: "+(n.error||n.message),"error")}}window.deleteSharedQuestion=deleteSharedQuestion;let _sharedStatsModal=null,_sharedStatsChart=null,_sharedStatsRows=[],_sharedStatsMode="avg";async function showSharedPoolStats(t,e){if(_sharedStatsModal&&(_sharedStatsModal.remove(),_sharedStatsModal=null),_sharedStatsChart){try{_sharedStatsChart.destroy()}catch{}_sharedStatsChart=null}const n=document.createElement("div");n.className="modal-overlay",n.innerHTML=`
