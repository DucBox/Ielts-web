...
`),s=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(s),a=document.createElement("a");a.href=o,a.download=t,a.click(),URL.revokeObjectURL(o)}async function downloadAudioFile(t,e,n){if(!t)return;const i=n?n.innerHTML:"";try{n&&(n.disabled=!0,n.innerHTML="\u23F3");const s=await fetch(t,{cache:"no-store"});if(!s.ok)throw new Error("fetch failed");const o=await s.blob(),a=(t.split("?")[0].match(/\.([a-zA-Z0-9]+)$/)||[])[1],l=(o.type.split("/")[1]||"").split(";")[0],r=a||l||"webm",c=(e||"audio").replace(/\.[a-zA-Z0-9]{2,5}$/,""),d=URL.createObjectURL(o),u=document.createElement("a");u.href=d,u.download=`${c}.${r}`,document.body.appendChild(u),u.click(),u.remove(),URL.revokeObjectURL(d)}catch(s){toast("T\u1EA3i file th\u1EA5t b\u1EA1i: "+(s.message||"l\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"),"error")}finally{n&&(n.disabled=!1,n.innerHTML=i)}}window.downloadAudioFile=downloadAudioFile;function buildStudentCredentialsFilename(t="student_accounts"){const e=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");return`${t}_${e}.csv`}function openStudentCredentialsModal(t,e,n="student_accounts"){const i=(Array.isArray(e)?e:[]).map(s=>({full_name:String(s?.full_name||"").trim(),username:String(s?.username||"").trim(),password:String(s?.password||"")})).filter(s=>s.full_name&&s.username&&s.password);_oneTimeStudentCredentials=i.length>0?{rows:i,fileName:buildStudentCredentialsFilename(n)}:null,openModal(t,`
    ...
    </div>`,K=c.filter(p=>!p.is_active&&p.deadline),N=_closedAssignsSearch?K.filter(p=>p.title.toLowerCase().includes(_closedAssignsSearch)):K,X=3,D=_closedAssignsExpanded?N:N.slice(0,X),it=N.length-D.length,st=K.length===0?"":`
    <div class="stats-section-card">
      ...
      ${N.length===0?'<p style="color:var(--gray-400);font-size:13px;padding:8px 0 4px">Kh\xF4ng t\xECm th\u1EA5y b\xE0i t\u1EADp ph\xF9 h\u1EE3p</p>':`<canvas id="chart-ontime" height="${Math.max(80,D.length*38)}"></canvas>`}
      ${N.length>X?`
...
`),o=new Blob(["\uFEFF"+s],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(o),l=document.createElement("a");l.href=a,l.download=`${t.title.replace(/[^a-zA-Z0-9_\-]/g,"_")}_diem.csv`,l.click(),URL.revokeObjectURL(a)}window.exportSubmissionsCSV=exportSubmissionsCSV;function buildSubmissionRows(t,e){return t.length===0?`<tr><td colspan="5">
    ...
    <div class="annotation-edit-actions">
      ...
      <button class="annotation-cancel-btn" onclick="refreshAnnotationsList()">Hu\u1EF7</button>
        ...
        <button type="button" class="btn btn-sm btn-danger" onclick="stopVoiceNoteRecording()">\u23F9 D\u1EEBng</button>`;else if(e.status==="uploading"){const s=e.pct<100&&e.eta!=null?` \xB7 ETA ${_fmtEta(e.eta)}`:"";i=`<div class="upload-progress-row" style="width:100%">
    ...
    </div>`}).join("")}}window.renderVoiceNotesList=renderVoiceNotesList;async function startVoiceNoteRecording(t){if(_voiceNoteRecordIdx>=0){toast("\u0110ang ghi \xE2m b\u1EA3n kh\xE1c, h\xE3y d\u1EEBng tr\u01B0\u1EDBc","warning");return}if(_gradingVoiceNotes[t]){_voiceNoteRecordIdx=t,_gradingVoiceNotes[t].status="recording",renderVoiceNotesList();try{const e=await navigator.mediaDevices.getUserMedia({audio:!0});_voiceNoteChunks=[];const n=MediaRecorder.isTypeSupported("audio/webm")?"audio/webm":"audio/ogg";_voiceNoteMediaRecorder=new MediaRecorder(e,{mimeType:n}),_voiceNoteMediaRecorder.ondataavailable=i=>{i.data.size>0&&_voiceNoteChunks.push(i.data)},_voiceNoteMediaRecorder.onstop=()=>{e.getTracks().forEach(o=>o.stop());const i=_voiceNoteMediaRecorder.mimeType||"audio/webm",s=new Blob(_voiceNoteChunks,{type:i});_onVoiceNoteRecordingDone(_voiceNoteRecordIdx,s,i)},_voiceNoteMediaRecorder.start(250),_voiceNoteRecordSeconds=0,clearInterval(_voiceNoteRecordTimer),_voiceNoteRecordTimer=setInterval(()=>{_voiceNoteRecordSeconds++;const i=Math.floor(_voiceNoteRecordSeconds/60),s=_voiceNoteRecordSeconds%60,o=document.getElementById("vn-record-timer");o&&(o.textContent=`${i}:${s.toString().padStart(2,"0")}`)},1e3)}catch(e){_gradingVoiceNotes[t].status="idle",_voiceNoteRecordIdx=-1,renderVoiceNotesList(),e.name==="NotAllowedError"||e.name==="SecurityError"?showMicPermissionDeniedModal(t):e.name==="NotFoundError"?toast("Kh\xF4ng t\xECm th\u1EA5y microphone tr\xEAn m\xE1y n\xE0y","error"):toast("Kh\xF4ng th\u1EC3 truy c\u1EADp microphone: "+e.message,"error")}}}window.startVoiceNoteRecording=startVoiceNoteRecording;function showMicPermissionDeniedModal(t){openModal("\u{1F399}\uFE0F Kh\xF4ng th\u1EC3 truy c\u1EADp microphone",`
    <div style="display:flex;flex-direction:column;gap:12px;line-height:1.6;color:var(--text)">
      <p>Tr\xECnh duy\u1EC7t \u0111\xE3 <strong>t\u1EEB ch\u1ED1i quy\u1EC1n micro</strong> cho trang n\xE0y tr\u01B0\u1EDBc \u0111\xF3, n\xEAn kh\xF4ng t\u1EF1 b\u1EADt l\u1EA1i popup xin quy\u1EC1n \u0111\u01B0\u1EE3c n\u1EEFa.</p>
      <p style="margin-bottom:0">C\xE1ch c\u1EA5p l\u1EA1i quy\u1EC1n:</p>
      <ol style="padding-left:20px;margin:0">
        <li>B\u1EA5m v\xE0o bi\u1EC3u t\u01B0\u1EE3ng \u{1F512} (ho\u1EB7c \u24D8) b\xEAn tr\xE1i thanh \u0111\u1ECBa ch\u1EC9 URL c\u1EE7a tr\xECnh duy\u1EC7t</li>
        <li>T\xECm m\u1EE5c <strong>Microphone</strong> v\xE0 ch\u1ECDn <strong>Allow / Cho ph\xE9p</strong></li>
        <li>T\u1EA3i l\u1EA1i trang r\u1ED3i b\u1EA5m "Ghi \xE2m" l\u1EA1i</li>
      </ol>
